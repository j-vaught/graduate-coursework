...
Use SI units for the beam calculation and report displacement in millimeters for the response plots. The leaf spring is idealized as a simply supported beam with a concentrated load at its center. This matches the two spring-eye supports and the center displacement shown in the model. The corresponding center deflection is
...
Define the decay rate $\alpha_{\mathrm{H}}=\zeta_{\mathrm{H}}\omega_n=0.579493\ \mathrm{s^{-1}}$. The characteristic roots are then
...
For the worn damper, the stiffness and mass do not change. Consequently, $I$, $EI$, $k$, $\omega_n$, $f_n$, and $c_{\mathrm{cr}}$ remain unchanged. Set $\zeta_{\mathrm{W}}=0.10$ and calculate the new damping and decay rate as
...
\begin{center}
\begin{matlabcode}
L = .830; b = .065; h = .003; E = 200e9; m = 457; u0 = 125;
Nosc = 10; zeta = [.25 .10]; ts = [6.090194 16.558987];
I = b*h^3/12; k = 48*E*I/L^3; wn = sqrt(k/m);

for j = 1:2
    wd = wn*sqrt(1-zeta(j)^2); tau = 2*pi/wd;
    t = linspace(0,Nosc*tau,2000);
    u = u0*exp(-zeta(j)*wn*t).*(cos(wd*t) + ...
        zeta(j)*wn/wd*sin(wd*t));
    subplot(2,1,j); plot(t,u,'Color',[0 .427 .459]); hold on
    plot([0 t(end)],[.02*u0 .02*u0],':','Color',[.776 .365 0]);
    plot([0 t(end)],[-.02*u0 -.02*u0],':','Color',[.776 .365 0]);
    plot([0 tau ts(j)],[u0 u0*exp(-zeta(j)*wn*tau) .02*u0],'ks');
