...
\textbf{B.7.1. Input data.} Use the equivalent tip stiffness of the cantilever beam and the attached motor mass as the lumped vibrating mass. Beam mass is omitted because no beam density or beam mass is supplied. For vertical bending, $h$ is the bending depth, so the second moment of area is $I=bh^3/12$.
...
\textbf{(c) Quasi-static deflection.} Because quasi-static loading means that the excitation frequency becomes negligible, use $\omega\to0$ while $\omega_n$ remains fixed. The force-normalized frequency response then approaches one, so
...
\textbf{(e) Frequency and RPM conversions.} Divide each angular frequency by $2\pi$ to obtain hertz, and multiply the result by $60$ to obtain revolutions per minute.
...
\textbf{(c) Real displacement.} Take the physical force as $F(t)=\hat F\cos(\omega_nt)$. Since $\hat u_{90}=-iU$, where
...
\caption{B.7.4.c. Phase-resonant real displacement over $N_{\mathrm{osc}}=10$ cycles. The orange curve includes the transient required when the force is switched on from rest.}
\end{figure}
...
Use $N_f=5000$ uniformly spaced frequencies from $f=0$ to $f=f_0$. The resulting magnitude and phase are shown below.
...
\begin{center}
\begin{matlabcode}
L=.568; E=200e9; h=.003; b=.037; m=.52; zeta=.14;
Fhat=.012; N0=1500; Nosc=10; Nf=5000;
I=b*h^3/12; EI=E*I; k=3*EI/L^3; wn=sqrt(k/m);
wd=wn*sqrt(1-zeta^2); uqst=Fhat/k; U=uqst/(2*zeta);

t=linspace(0,Nosc*2*pi/wn,2000);
uss=U*sin(wn*t);
urest=uss-U*wn/wd*exp(-zeta*wn*t).*sin(wd*t);
plot(t,uss,'Color',[0 .427 .459],'LineWidth',1.5); hold on
plot(t,urest,'--','Color',[.776 .365 0],'LineWidth',1.5);
xlabel('Time (s)'); ylabel('Displacement (m)'); grid on

f=linspace(0,N0/60,Nf); p=2*pi*f/wn;
M=1./sqrt((1-p.^2).^2+(2*zeta*p).^2);
phi=-atan2(2*zeta*p,1-p.^2)*180/pi;
figure
plot(f,M,'Color',[0 .427 .459],'LineWidth',1.5);
figure
plot(f,phi,'Color',[0 .427 .459],'LineWidth',1.5);
\end{matlabcode}
\end{center}
