...
The given system has mass $m=12\ \mathrm{kg}$, stiffness $k=655\ \mathrm{N/m}$, damping coefficient $c=19\ \mathrm{N\,s/m}$, initial displacement $u_0=3.5\ \mathrm{mm}$, and initial velocity $\dot{u}_0=0$. The response will be plotted for $N_{\mathrm{osc}}=10$ damped oscillations.
...
\begin{center}
\begin{matlabcode}
m = 12; k = 655; c = 19; u0 = 3.5; Nosc = 10; ts = 4.786694;
wn = sqrt(k/m); zeta = c/(2*sqrt(m*k)); wd = wn*sqrt(1-zeta^2);
tau = 2*pi/wd; t = linspace(0,Nosc*tau,2000);
u = u0*exp(-zeta*wn*t).*(cos(wd*t)+zeta*wn/wd*sin(wd*t));
u02 = .02*u0; u1 = u0*exp(-zeta*wn*tau);

plot(t,u,'Color',[0 .427 .459]); hold on
plot([0 t(end)],[u02 u02],':','Color',[.776 .365 0]);
plot([0 t(end)],[-u02 -u02],':','Color',[.776 .365 0]);
plot([0 tau ts],[u0 u1 -u02],'ks');
xlabel('Time (s)'); ylabel('Displacement (mm)');
legend('Response','2% band','Location','northeast'); ylim([-3 5]); grid on
\end{matlabcode}
\end{center}
