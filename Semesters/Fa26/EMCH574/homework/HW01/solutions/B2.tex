...
Assume a nontrivial exponential response $u(t)=e^{st}$. Its derivatives are $\dot{u}(t)=se^{st}$ and $\ddot{u}(t)=s^2e^{st}$. Substitution into the equation of motion gives
...
Critical damping occurs when the two roots coincide. Therefore, set the discriminant equal to zero,
...
Define the damped frequency as the positive imaginary part of the underdamped roots,
...
Euler's identities convert the harmonic part into a real trigonometric form. Define $A=C_1+C_2$ and $B=i(C_1-C_2)$. Then
...
\begin{center}
\begin{matlabcode}
wn = 1; zeta = 0.20; C = 1; phi = 0.5; Nosc = 3;
wd = wn*sqrt(1-zeta^2);
t = linspace(0,Nosc*2*pi/wd,1000);
u = C*exp(-zeta*wn*t).*sin(wd*t+phi);
e = C*exp(-zeta*wn*t);

plot(wn*t,u,'Color',[0 .427 .459]); hold on
plot(wn*t,e,'--','Color',[.776 .365 0])
plot(wn*t,-e,'-.k')
