The input values required for A.3.1 are shown below. Convert them to SI units before calculating the response. The projectile mass is denoted by \(m_p\) so that it is distinct from the pendulum mass \(M\).
