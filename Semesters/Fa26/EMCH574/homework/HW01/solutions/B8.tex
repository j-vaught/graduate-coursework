...
\textbf{B.8.1. Input data.} Let $m_p$ denote the projectile mass so that it is distinct from the attached mass $M$. After impact, the projectile remains embedded, and the vibrating mass is $m_T=M+m_p$. The beam is represented by its equivalent tip stiffness.
