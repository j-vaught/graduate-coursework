...
\[f_n=\frac{\omega_n}{2\pi}=\frac{1}{2\pi}\sqrt{\frac{g}{L}},\qquad
\tau=\frac{1}{f_n}=\frac{2\pi}{\omega_n}=2\pi\sqrt{\frac{L}{g}}.\]

Multiplication by \(L\) gives the particle-motion solution

\[u(t)=C_1e^{i\omega_nt}+C_2e^{-i\omega_nt}
=A\cos(\omega_nt)+B\sin(\omega_nt)
=C\sin(\omega_nt+\varphi).\]
...
\[\boxed{u(t)=\frac{v_0}{\omega_n}\sin(\omega_nt).}\]

Applying both initial conditions gives the combined response in the first boxed equation. In exponential form the same constants are
