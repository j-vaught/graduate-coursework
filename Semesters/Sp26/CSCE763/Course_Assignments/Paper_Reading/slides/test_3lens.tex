...
-- CONTROLS
-- ════════════════════════════════════════════════════════════
local h       = 1.4       -- half-aperture height (all lenses same, matches casing)
local n_glass = 1.52
local min_wall = 0.08

-- Element positions — defined by pairwise center-to-center distances
local x1     = 2.0                -- plano-convex center
local d12    = 2.5                -- lens1 center → lens2 center
local d23    = 2.5                -- lens2 center → lens3 center
local d3s    = 2.5                -- lens3 center → sensor
local ap_frac = 0.5               -- aperture position as fraction of d12 (0.5 = midpoint)
...
-- Casing
local casing_y = h         -- inner wall of casing touches lens edges
...
-- Check if ray hits casing wall between current pos and next element
-- Returns hit point if blocked, nil if clear
...
local function trace_full(oy)
    ...
    -- Check casing between lens1 and aperture
    ...
    -- Check casing between aperture and lens2
    ...
    -- Check casing between lens2 and lens3
    ...
    -- Check casing between lens3 and sensor
...
-- Aperture (same height as casing)
tex.sprint(string.format(
...
-- Lens casing — two horizontal lines from past front lens to CMOS
local casing_left = x1 - 0.6      -- extends past front of lens1
