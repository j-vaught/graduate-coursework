...
Poisson's equation appears in many physical contexts:
\begin{itemize}
    \item \textbf{Electrostatics:} $\nabla^2 \phi = -\rho/\epsilon_0$ where $\phi$ is electric potential and $\rho$ is charge density
    \item \textbf{Steady-state heat conduction:} $\nabla^2 T = -q/k$ where $T$ is temperature and $q$ is heat generation
    \item \textbf{Gravitational potential:} $\nabla^2 \Phi = 4\pi G \rho$ where $\Phi$ is gravitational potential
    \item \textbf{Pressure in incompressible flow:} The pressure Poisson equation in CFD
\end{itemize}
The negative Laplacian represents ``diffusion away'' from a point---a source ($f > 0$) creates a local maximum in $u$.
\end{stepbox}
...
\vspace{0.3cm}
\begin{stepbox}
\textbf{Finite Difference Method Overview}

The finite difference method replaces continuous derivatives with discrete approximations on a grid:
\begin{itemize}
    \item The domain is discretized into a mesh with spacing $h$ in both $x$ and $y$ directions
    \item Grid points are labeled $(x_i, y_j) = (x_0 + ih, y_0 + jh)$ for integers $i, j$
    \item The solution $u(x,y)$ is approximated only at grid points: $u_{i,j} \approx u(x_i, y_j)$
    \item Derivatives are replaced by algebraic expressions involving nearby grid values
\end{itemize}
\end{stepbox}
...
Rearranging for the center point (useful for iteration):
\[
u_{i,j} = \frac{1}{4}\left(u_{i+1,j} + u_{i,j+1} + u_{i-1,j} + u_{i,j-1} + \frac{1}{2}\right)
\]
\end{stepbox}
...
\toprule
\textbf{Segment} & \textbf{From} & \textbf{To} & \textbf{Condition} \\
\midrule
$AB$ & $(0, 1.5)$ & $(1, 1.5)$ & $u = 1$ (inner boundary) \\
$BC$ & $(1, 1.5)$ & $(1.5, 1)$ & $u = 1$ (inner boundary) \\
$CD$ & $(1.5, 1)$ & $(1.5, 0)$ & $u = 1$ (inner boundary) \\
$DE$ & $(1.5, 0)$ & $(2.5, 0)$ & $u = 0$ (outer boundary) \\
$EF$ & $(2.5, 0)$ & $(2.5, 2.5)$ & $u = 0$ (outer boundary) \\
$FG$ & $(2.5, 2.5)$ & $(0, 2.5)$ & $u = 0$ (outer boundary) \\
$GA$ & $(0, 2.5)$ & $(0, 1.5)$ & $u = 0$ (outer boundary) \\
\bottomrule
\end{tabular}
\end{center}
\end{stepbox}
...
\begin{stepbox}
\textbf{Interior Point Identification}

With mesh size $h = 0.5$, the grid points have coordinates $(0.5i, 0.5j)$. The interior points are:

\begin{center}
\begin{tabular}{c|c|c|c}
\toprule
\textbf{Point} & \textbf{Coordinates $(x, y)$} & \textbf{Grid Index $(i, j)$} & \textbf{Neighbors} \\
\midrule
$u_1$ & $(0.5, 1.5)$ & $(1, 3)$ & W:boundary(0), E:$u_2$, S:boundary(1), N:boundary(0) \\
$u_2$ & $(1.0, 1.5)$ & $(2, 3)$ & W:$u_1$, E:$u_3$, S:$u_4$, N:boundary(0) \\
$u_3$ & $(1.5, 1.5)$ & $(3, 3)$ & W:$u_2$, E:boundary(0), S:$u_5$, N:boundary(0) \\
$u_4$ & $(1.0, 1.0)$ & $(2, 2)$ & W:boundary(1), E:$u_5$, S:boundary(1), N:$u_2$ \\
$u_5$ & $(1.5, 1.0)$ & $(3, 2)$ & W:$u_4$, E:boundary(0), S:$u_6$, N:$u_2$ \\
$u_6$ & $(1.5, 0.5)$ & $(3, 1)$ & W:boundary(1), E:boundary(0), S:boundary(0), N:$u_5$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Symmetry:} The domain is symmetric about the line $y = x$. Therefore:
\[
u_1 = u_6, \quad u_2 = u_5, \quad u_3 = u_3, \quad u_4 = u_4
\]
This reduces the problem from 6 unknowns to \textbf{4 unknowns}.
\end{stepbox}

\begin{stepbox}
\textbf{Deriving Each Equation}

Using the 5-point stencil $u_E + u_W + u_N + u_S - 4u_C = -0.5$:

\textbf{At Point 1 $(0.5, 1.5)$:}
\begin{itemize}
    \item East $(1.0, 1.5) = u_2$
    \item West $(0.0, 1.5) = 0$ (outer boundary)
    \item North $(0.5, 2.0) = 0$ (outer boundary)
    \item South $(0.5, 1.0) = 1$ (inner boundary)
\end{itemize}
\[
u_2 + 0 + 0 + 1 - 4u_1 = -0.5 \implies \boxed{-4u_1 + u_2 = -1.5}
\]

\textbf{At Point 2 $(1.0, 1.5)$:}
\begin{itemize}
    \item East $(1.5, 1.5) = u_3$, West $(0.5, 1.5) = u_1$
    \item North $(1.0, 2.0) = 0$, South $(1.0, 1.0) = u_4$
\end{itemize}
\[
u_3 + u_1 + 0 + u_4 - 4u_2 = -0.5 \implies \boxed{u_1 - 4u_2 + u_3 + u_4 = -0.5}
\]

\textbf{At Point 3 $(1.5, 1.5)$:}
\begin{itemize}
    \item East $(2.0, 1.5) = 0$, West $(1.0, 1.5) = u_2$
    \item North $(1.5, 2.0) = 0$, South $(1.5, 1.0) = u_5 = u_2$ (by symmetry)
\end{itemize}
\[
0 + u_2 + 0 + u_2 - 4u_3 = -0.5 \implies \boxed{2u_2 - 4u_3 = -0.5}
\]

\textbf{At Point 4 $(1.0, 1.0)$:}
\begin{itemize}
    \item East $(1.5, 1.0) = u_5 = u_2$, West $(0.5, 1.0) = 1$ (inner boundary)
    \item North $(1.0, 1.5) = u_2$, South $(1.0, 0.5) = 1$ (inner boundary)
\end{itemize}
\[
u_2 + 1 + u_2 + 1 - 4u_4 = -0.5 \implies \boxed{2u_2 - 4u_4 = -2.5}
\]
\end{stepbox}
...
\vspace{0.3cm}
\begin{stepbox}
\textbf{Physical Interpretation: Heat Equation}

This is the \textbf{1D heat equation} (also called the diffusion equation):
\begin{itemize}
    \item $u(x,t)$ represents temperature at position $x$ and time $t$
    \item The coefficient $k = 0.25$ is the \textbf{thermal diffusivity} (units: $m^2/s$)
    \item Boundary conditions $u(0,t) = u(2,t) = 0$ mean both ends are held at zero temperature
    \item Initial condition $u(x,0) = \sin(\pi x)$ is a sine wave with a node at $x = 1$
\end{itemize}
\textbf{Physical behavior:} Heat diffuses from hot regions to cold regions. The initial sine wave will decay exponentially while maintaining its sinusoidal shape.
\end{stepbox}

\vspace{0.3cm}
\begin{stepbox}
\textbf{Crank-Nicholson Method Overview}

The Crank-Nicholson method is an \textbf{implicit finite difference scheme} with several advantages:
\begin{itemize}
    \item \textbf{Unconditionally stable:} Works for any $\lambda > 0$ (no CFL restriction)
    \item \textbf{Second-order accurate:} $O(\Delta t^2, h^2)$ truncation error
    \item \textbf{Implicit:} Requires solving a linear system at each time step
\end{itemize}

The method averages the spatial derivatives at time levels $j$ and $j+1$:
\[
\frac{u_{i,j+1} - u_{i,j}}{\Delta t} = \frac{k}{2}\left[\frac{u_{i+1,j} - 2u_{i,j} + u_{i-1,j}}{h^2} + \frac{u_{i+1,j+1} - 2u_{i,j+1} + u_{i-1,j+1}}{h^2}\right]
\]
\end{stepbox}
...
\vspace{0.3cm}
\begin{stepbox}
\textbf{Explicit FTCS Method}

The \textbf{Forward-Time, Centered-Space (FTCS)} method is the simplest finite difference scheme for the heat equation:
\begin{itemize}
    \item \textbf{Forward in time:} Uses $u_{i,j+1} - u_{i,j}$ for $\partial u/\partial t$
    \item \textbf{Centered in space:} Uses $u_{i+1,j} - 2u_{i,j} + u_{i-1,j}$ for $\partial^2 u/\partial x^2$
    \item \textbf{Explicit:} New values computed directly from old values (no linear system to solve)
    \item \textbf{First-order in time:} $O(\Delta t)$ temporal accuracy
    \item \textbf{Second-order in space:} $O(h^2)$ spatial accuracy
\end{itemize}
\end{stepbox}
...
The explicit method is \textbf{conditionally stable}. Using von Neumann stability analysis:
\[
\text{Stability requires:} \quad \lambda = \frac{k \Delta t}{h^2} \leq \frac{1}{2}
\]

\textbf{Physical interpretation:} Information cannot propagate faster than one grid cell per time step.

\begin{center}
\begin{tabular}{c|c|c}
\toprule
\textbf{Condition} & \textbf{Behavior} & \textbf{Physical Meaning} \\
\midrule
$\lambda < 0.5$ & Stable, smooth & Normal diffusion \\
$\lambda = 0.5$ & Stable, exact averaging & Pure neighbor averaging \\
$\lambda > 0.5$ & Unstable, oscillations & Unphysical amplification \\
\bottomrule
\end{tabular}
\end{center}
\end{stepbox}
...
\vspace{0.3cm}
\begin{stepbox}
\textbf{Comparison: Explicit vs Crank-Nicholson}

\begin{center}
\begin{tabular}{l|c|c}
\toprule
\textbf{Property} & \textbf{Explicit (FTCS)} & \textbf{Crank-Nicholson} \\
\midrule
Time accuracy & $O(\Delta t)$ & $O(\Delta t^2)$ \\
Space accuracy & $O(h^2)$ & $O(h^2)$ \\
Stability & $\lambda \leq 0.5$ required & Unconditionally stable \\
Computation & Direct formula & Solve tridiagonal system \\
Implementation & Very simple & Moderate complexity \\
Best for & Quick prototyping & Production simulations \\
\bottomrule
\end{tabular}
\end{center}
\end{stepbox}
