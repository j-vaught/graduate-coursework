...
\title{Dual-Camera Active Acquisition for Automated Small-Object Dataset Construction: Wide-Angle Real-Time Detection with PTZ Zoom Verification for Airplanes in the Sky}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.6\columnwidth]{fig_hardware_setup.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\columnwidth]{fig_fov_comparison.pdf}
...
\begin{tikzpicture}
    ...
    \fill[fill=CGrayLight] (O) -- (4, -1.8) -- (4, 1.8) -- cycle;
    \node[text=CGrayDark, font=\scriptsize] at (3.5, 1.4) {Wide FOV};

    % Wide Cam Icon
    \node[rectangle, draw=black, fill=white, minimum width=0.8cm, minimum height=0.4cm] (W) at (O) {};
    \node[font=\scriptsize, below=2pt] at (W.south) {Wide Cam};

    % PTZ Camera Icon (slightly offset)
    \coordinate (P) at (0, -0.6);
    \node[rectangle, draw=Garnet, fill=Garnet!10, minimum width=0.8cm, minimum height=0.4cm] (PTZ) at (P) {};
    \node[font=\scriptsize, color=Garnet, below=2pt] at (PTZ.south) {PTZ Cam};

    % Target Trajectory
    \draw[dashed, color=CDark, thick] (1.5, 1.2) -- (4.2, 0.2);
    \node[circle, fill=CDark, inner sep=2pt, label={[labeltext]above:Target}] (T) at (3.5, 0.46) {};

    % PTZ Beam
    \fill[Garnet, opacity=0.2] (P) -- ($(T)+(0,0.3)$) -- ($(T)-(0,0.3)$) -- cycle;
    \draw[Garnet, thick] (P) -- (T);

    % Vectors
    \draw[->, color=CBlue, thick] (O) -- node[above, font=\tiny]{$\hat{d}_w$} (T);
    \draw[->, color=Garnet, thick] (P) -- node[below, font=\tiny]{$\hat{d}_p$} (T);

    % Annotation
    \node[align=left, font=\scriptsize, anchor=north west] at (0.2, 1.8) {
        \textcolor{CBlue}{$\blacksquare$} Wide Detection\\
        \textcolor{Garnet}{$\blacksquare$} PTZ Confirmation
    };

\end{tikzpicture}
\caption{Acquisition Geometry. The system predicts the target vector $\hat{d}_w$ in the wide frame and computes the required PTZ vector $\hat{d}_p$ to center the target for high-resolution verification.}
\label{fig:geometry}
\end{figure}
...
Both cameras are modeled with intrinsics $(K_w, K_p)$ and distortion parameters. Let $\Pi(\cdot)$ be perspective projection with distortion, and let $R_{pw}, t_{pw}$ map 3D points from wide-camera coordinates to PTZ-camera coordinates (Fig.~\ref{fig:coords}).
\par
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\columnwidth]{fig_coord_frames.pdf}
\caption{Coordinate frame relationship. The wide camera frame $(X_w, Y_w, Z_w)$ and PTZ camera frame $(X_p, Y_p, Z_p)$ are related by rotation $R_{pw}$ and translation $t_{pw}$.}
\label{fig:coords}
\end{figure}
...
\end{equation}
then choose the zoom $z$ whose calibrated $f_p(z)$ best matches the required value, clipped to PTZ limits (Fig.~\ref{fig:zoomcurve}). This approach avoids explicit range estimation and is well suited for distant aircraft.

\begin{figure}[t]
\centering
\includegraphics[width=0.6\columnwidth]{fig_zoom_focal.pdf}
\caption{Zoom-to-focal-length calibration curve. The PTZ focal length $f_p(z)$ is a nonlinear function of the zoom setting $z$, calibrated empirically.}
\label{fig:zoomcurve}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig_zoom_comparison.pdf}
...
\bibitem{smallobj_survey1}
``Small Object Detection: A Comprehensive Survey on Challenges, Techniques and Real-World Applications,'' \textit{arXiv preprint arXiv:2503.20516}, 2025.

\bibitem{smallobj_survey2}
``Small object detection in diverse application landscapes,'' \textit{Multimedia Tools and Applications}, vol. 83, pp. 40321--40359, 2024.

\bibitem{smallobj_survey3}
``Deep learning-based detection from the perspective of small or tiny objects: A comparative review,'' \textit{Image and Vision Computing}, vol. 117, p. 104813, 2022.

\bibitem{yolo_flying}
``Real-Time Flying Object Detection with YOLOv8,'' \textit{arXiv preprint arXiv:2305.09972}, 2023.

\bibitem{yolo_review}
``A Comprehensive Review of YOLO: From YOLOv1 to YOLOv8 and Beyond,'' \textit{arXiv preprint arXiv:2304.00501}, 2023.

\bibitem{ptz_eval}
``Evaluation of trackers for Pan-Tilt-Zoom Scenarios,'' \textit{arXiv preprint arXiv:1711.04260}, 2017.

\bibitem{ptz_reproducible}
``Reproducible Evaluation of Pan-Tilt-Zoom Tracking,'' in \textit{Proceedings of the IEEE International Conference on Image Processing (ICIP)}, 2015.

\bibitem{active_ptz}
``Active Visual Perception Enhancement Method Based on Deep Learning with PTZ Cameras,'' \textit{Electronics}, vol. 13, no. 1, p. 98, 2024.

\bibitem{anomalous_ptz}
``Anomalous object detection by active search with PTZ cameras,'' \textit{Engineering Applications of Artificial Intelligence}, vol. 95, p. 103864, 2021.

\bibitem{vigia_ptz}
``VIGIA-E: Density-Aware Patch Selection for Efficient Video Surveillance with PTZ Cameras,'' in \textit{Proceedings of the 25th International Conference on Computer Analysis of Images and Patterns (CAIP)}, 2025.

\bibitem{self_training}
``Adaptive Self-Training for Object Detection,'' in \textit{Proceedings of the IEEE/CVF International Conference on Computer Vision Workshops (ICCVW)}, 2023.

\bibitem{self_training_enh}
``Improving Object Detection Accuracy with Self-Training in Semi-supervised Learning,'' \textit{Electronics}, vol. 13, no. 6, p. 1107, 2024.

\bibitem{active_learning_survey}
``Ten Years of Active Learning Techniques and Object Detection: A Comprehensive Survey,'' \textit{Applied Sciences}, vol. 13, no. 10, p. 6181, 2023.

\bibitem{active_learning_ppod}
``Plug and Play Active Learning for Object Detection,'' in \textit{Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)}, 2024.
