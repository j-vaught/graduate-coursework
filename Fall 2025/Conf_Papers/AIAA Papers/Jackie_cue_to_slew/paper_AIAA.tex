...
\author{Jackie Wang\footnote{Graduate Student, Department of Mechanical Engineering, Member AIAA.}}
\author{JC Vaught\footnote{Graduate Student, Department of Mechanical Engineering, Member AIAA.}}
\author{Douglas Cahl\footnote{Professor, Department of Mechanical Engineering, Member AIAA.}}
\author{Yi Wang\footnote{Professor, Department of Mechanical Engineering, Senior Member AIAA.}}
...
\begin{abstract}
Small objects in wide-field video streams often suffer from extreme pixel scarcity, leading to low detector confidence, unstable localization, and noisy labels that limit dataset quality. This paper describes an automated dataset construction system that couples (i) a fixed wide-angle camera running a real-time object detector with (ii) a pan--tilt--zoom (PTZ) camera that is programmatically commanded to perform resolution-enhanced verification. The system uses a predictive control loop to lock onto candidate targets and zoom in to obtain higher-resolution evidence. The PTZ observation acts as a resolution-enhanced verification step: high-confidence detections in the zoomed view are transferred back to the wide view using calibrated cross-camera geometry and time-synchronized telemetry. The result is a pipeline that harvests training examples and hard negatives with minimal human intervention. A concrete test case is presented for airplanes against sky backgrounds, where objects are distant and subject to atmospheric distortion. We provide system design, calibration formulations, and an evaluation protocol to quantify confidence uplift and label precision, demonstrating significant improvements in detection stability and label quality for small-object regimes.
\end{abstract}
