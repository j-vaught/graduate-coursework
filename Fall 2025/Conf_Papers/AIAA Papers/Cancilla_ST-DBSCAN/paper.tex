...
\begin{abstract}
\noindent Maritime missions such as autonomous over-water navigation, ship-based operations, and airborne/space-enabled maritime ISR require reliable surface-target detection and tracking in the presence of strong, nonstationary sea clutter and intermittent target returns. We present an end-to-end, real-time perception pipeline for vessel detection and multi-sweep tracking using a commercially prevalent class of marine radar that provides intensity-only measurements (range, azimuth, intensity) without Doppler phase history. Raw polar radar returns from a Furuno NXT solid-state unit are transformed into Cartesian point clouds, aggregated over a short temporal buffer, and clustered using Spatio-Temporal DBSCAN (ST-DBSCAN) to link detections across successive scans. To maintain performance under changing clutter conditions and nonuniform point densities, neighborhood radii are adapted automatically using $k$-nearest-neighbor distance statistics. To reduce track fragmentation from brief dropouts, we introduce a lightweight gap-recovery mechanism that reconnects clusters across short temporal gaps without the complexity of full multi-hypothesis tracking. Across two datasets, including (i) a synthetic dataset with ground-truth annotations and (ii) a smaller human-labeled dataset collected on an inland lake, the approach achieves 92\% cluster purity with 0.8 false alarms per frame. We release two reference implementations (Rust and Python). The Rust implementation runs at 2 sweeps per second on an NVIDIA Jetson Orin, meeting real-time throughput requirements for a radar operating at a standard 48 RPM. Code and the synthetic dataset are publicly available.
\end{abstract}
