...
The increasing availability of solid-state marine radars with high range resolution (sub-meter) and fast scan rates (24--48 RPM) motivates the development of point-cloud-based processing approaches that exploit both spatial and temporal coherence. Unlike traditional plot extraction, point cloud methods preserve the spatial extent of radar returns, enabling better discrimination of closely spaced targets.
...
\begin{tikzpicture}[
    node distance=0.6cm,
    box/.style={rectangle, draw, rounded corners, minimum width=1.8cm, minimum height=0.8cm, text centered, font=\footnotesize, align=center},
    arrow/.style={-{Stealth[length=2mm]}, thick}
]
    \node[box, fill=blue!15] (radar) {Polar\\Returns};
    \node[box, fill=green!15, right=of radar] (pointcloud) {Point Cloud\\Extraction};
    \node[box, fill=yellow!15, right=of pointcloud] (adaptive) {Adaptive\\$\varepsilon_s$ Est.};
    \node[box, fill=orange!15, right=of adaptive] (stdbscan) {ST-DBSCAN\\+ Gap Rec.};
    \node[box, fill=purple!15, right=of stdbscan] (tracker) {Hungarian\\Tracker};

    \draw[arrow] (radar) -- (pointcloud);
    \draw[arrow] (pointcloud) -- (adaptive);
    \draw[arrow] (adaptive) -- (stdbscan);
    \draw[arrow] (stdbscan) -- (tracker);
\end{tikzpicture}
\caption{Processing pipeline from polar radar returns to tracked objects.}
\label{fig:pipeline}
\end{figure}
...
Figure~\ref{fig:pointcloud} illustrates the point cloud extraction process. A typical sweep produces 12,000--50,000 points after preprocessing.

\begin{figure}[hbt!]
\centering
\includegraphics[width=\linewidth]{figures/fig_pointcloud.pdf}
\caption{Point cloud extraction: (a) raw radar PPI, (b) binary mask after intensity thresholding, (c) resulting point cloud with intensity-coded colors.}
...
\begin{figure}[hbt!]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig_stdbscan.pdf}
\caption{ST-DBSCAN clustering on coastal radar data. Each color represents a distinct cluster; vessels and coastal returns are separated.}
\label{fig:realdata}
\end{figure}

Figure~\ref{fig:realdata} shows a representative frame with color-coded clusters. The system successfully separates multiple vessel returns from coastal clutter and maintains distinct clusters for vessels at different ranges. Qualitative analysis reveals several characteristics of system behavior. Adaptive $\varepsilon_s$ estimation produces values in the range 8--15 pixels depending on local clutter density, compared to a fixed value of 10 pixels. Gap recovery successfully reconnects tracks across 2--4 frame dropouts that occur when vessels enter wave shadows or experience radar cross-section fluctuations. The system handles nonstationary clutter (varying sea state during recording) without manual parameter adjustment.
...
Jetson Orin (GPU) & \hl{TBD} & \hl{TBD} & \hl{TBD} & \hl{TBD} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[hbt!]
\centering
\colorbox{yellow}{\parbox{0.8\linewidth}{\centering\textbf{[PLACEHOLDER]}\\Sensitivity analysis: Tracking metrics vs. adaptive $\varepsilon_s$ percentile (70th, 80th, 90th, 95th)}}
\caption{[TO BE ADDED] Sensitivity of tracking performance to adaptive parameter percentile selection.}
...
\begin{figure}[hbt!]
\centering
\colorbox{yellow}{\parbox{0.8\linewidth}{\centering\textbf{[PLACEHOLDER]}\\Ablation study: Comparison of tracking metrics with/without adaptive $\varepsilon_s$ and with/without gap recovery mechanism}}
\caption{[TO BE ADDED] Component ablation showing contribution of adaptive parameter estimation and gap recovery to overall system performance.}
\label{fig:ablation}
\end{figure}
