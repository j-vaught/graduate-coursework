...
\begin{figure}[hbt!]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_buffer.pdf}
\caption{Temporal buffer structure for spatio-temporal clustering. Multiple radar sweeps are accumulated in a sliding window, enabling cross-frame cluster association before track initialization.}
\label{fig:buffer}
\end{figure}

\begin{figure}[hbt!]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_tubes.pdf}
\caption{Spatio-temporal tube visualization. Clusters form connected tubes through the $(x, y, t)$ space, where each tube represents a tracked object trajectory across multiple radar sweeps.}
\label{fig:tubes}
\end{figure}
...
\begin{figure}[hbt!]
\centering
\includegraphics[width=0.9\linewidth]{figures/fig_coverage.pdf}
\caption{Spatial coverage analysis showing radar detection probability as a function of range and bearing. Near-range regions exhibit higher clutter density while far-range returns are sparser but more reliable for vessel detection.}
\label{fig:coverage}
\end{figure}

\begin{figure}[hbt!]
\centering
\includegraphics[width=0.9\linewidth]{figures/fig_rcs.pdf}
\caption{Radar cross-section (RCS) variability analysis. Target returns fluctuate significantly due to aspect angle changes and sea state variations, motivating the need for adaptive clustering parameters.}
\label{fig:rcs}
\end{figure}
