...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{python_figures/fig01_radar_hardware.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{python_figures/fig02_modality_comparison.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{python_figures/fig03_collection_map.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{python_figures/fig04_radar300_samples.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{python_figures/fig05_usc_canal_samples.png}
...
At this stage, we deliberately avoid heavy preprocessing such as constant false alarm rate (CFAR) thresholding, entropy-based enhancement, or radar high dynamic range (HDR) compression. Those techniques are the focus of separate, more signal-processing-oriented papers and undergraduate projects. For the present work, the primary transformation is from raw polar data to a simple, consistent Cartesian image representation that can be used with standard object detectors.
...
We consider two detector families. The first is a modern one-stage detector inspired by the YOLOv8 family, serving as the main high-speed baseline. The second is a two-stage detector of the Faster R-CNN type with a ResNet backbone, serving as a more traditional baseline that is often competitive in accuracy but heavier in computation.
...
The primary evaluation metric is mean Average Precision at an Intersection-over-Union (IoU) threshold of $0.5$ (mAP@0.5) on the canal test set. This metric is widely used in object detection and provides a clear measure of localization and classification quality. A more stringent metric, mAP averaged over IoU thresholds between $0.5$ and $0.95$, can also be reported if sample sizes permit stable estimation.
...
The preliminary results and extensive tooling in this work highlight both the promise and the constraints of learning-based marine radar perception under extreme label scarcity.
