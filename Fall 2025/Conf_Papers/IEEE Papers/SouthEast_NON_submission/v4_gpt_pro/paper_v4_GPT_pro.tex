...
\title{RADAR300 Pretraining and Open-Source Tools for Marine Radar Object Detection in Inland and Coastal Waters}

\author{
\IEEEauthorblockN{J.~C.~Vaught, Douglas~Dahl, Yi~Wang}
\IEEEauthorblockA{
Department of Mechanical Engineering, University of South Carolina, Columbia, SC, USA\\
Email: \{vaught, dahl, wang\}@example.edu
}
}
...
\subsection{RADAR300}
RADAR300 is a public marine radar dataset containing $3000$ PPI radar frames with boat annotations. Each frame is labeled in a single foreground class denoted by the label ``B'' for boat. In total, the dataset contains $3000$ boat instances, suggesting that the majority of frames contain a single annotated boat, though this is not a strict requirement. The imagery is provided in a format compatible with standard object-detection frameworks, and prior work has explored its use for YOLO-style detectors.

In our experiments, RADAR300 serves as a source dataset for generic boat detection. We treat its single boat class as compatible with the dynamic-boat class in our canal dataset and use it to initialize weights for modern one-stage detectors. The precise split of RADAR300 into training, validation, and testing subsets can follow the original dataset recommendation or, when not specified, a conventional division such as $80$~\% training and $20$~\% validation. Because this paper focuses on transfer to the canal dataset, RADAR300 performance is reported primarily to confirm that pretraining has converged.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig04_radar300_samples.png}
\caption{Sample detections from the RADAR300 public dataset. These $3000$ radar frames with boat annotations serve as our pretraining source domain. Lime bounding boxes show detected boats across diverse marine environments.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig05_usc_canal_samples.png}
\caption{Sample labeled frames from the USC Canal dataset showing dynamic (green) and static (orange) boat detections. Higher clutter and environmental complexity compared to RADAR300 reflect the challenges of inland and near-coastal autonomy.}
\label{fig:usc_canal_samples}
\end{figure}
...
To make the structure of the datasets concrete, Table~\ref{tab:datasets} summarizes the key properties of RADAR300 and the USC canal dataset, including frame counts, labeled object counts, number of classes, and intended usage. Entries marked with placeholders can be updated once final splits and statistics are computed.
...
\midrule
RADAR300 & $3000$ & $3000$ objects & $1$ (boat B) & pretraining \\
USC-Canal-Labeled & $96$ & $637$ objects & $2$ (dyn, stat) & fine-tuning, test \\
USC-Canal-Unlabeled & $196\,000$ & $0$ & -- & future self-supervised \\
Pohang-Subset & $N_{\text{P}}^\ast$ & $M_{\text{P}}^\ast$ & $C_{\text{P}}^\ast$ & qualitative generalization \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig06_dataset_comparison.png}
\caption{Dataset comparison showing visual differences between RADAR300 (left, cleaner with less clutter) and USC Canal (right, more cluttered inland environment). This distribution shift motivates the use of pretraining and fine-tuning rather than training only on the small canal dataset.}
\label{fig:dataset_comparison}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig07_class_distribution.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig08_temporal_coverage.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig10_furuno_hardware_detailed.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig11_range_azimuth_diagram.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig12_quantization_comparison.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig13_conversion_pipeline.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig14_range_variability.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig15_echo_trail_generation.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig16_echo_trail_variants.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig17_motion_visualization.png}
...
\node[draw, rectangle, rounded corners, align=center] (raw) {Raw\\Range--Azimuth};
\node[draw, rectangle, rounded corners, right=of raw, align=center] (cart) {Polar-to-\\Cartesian};
\node[draw, rectangle, rounded corners, right=of cart, align=center] (png) {$1735\times 1735$\\PNG};
\node[draw, rectangle, rounded corners, right=of png, align=center] (trail) {Echo-Trail\\Generator};
\node[draw, rectangle, rounded corners, right=of trail, align=center] (det) {Detector\\Input};
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig18_yolo_architecture.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig19_faster_rcnn_architecture.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig20_anchor_box_analysis.png}
...
The second regime is RADAR300 pretraining followed by fine-tuning. Here, detectors are first trained on RADAR300 until convergence and then fine-tuned on the canal dataset with a lower learning rate and possibly a reduced number of trainable layers. The single boat class in RADAR300 is aligned with the dynamic boat class in the canal dataset; static objects in the canal dataset are introduced only during fine-tuning.

The third regime is joint training. Detectors are trained on a mixture of RADAR300 and canal frames, with class labels mapped into a shared space. For example, RADAR300 boat labels may be aligned with the dynamic class, while static canal labels are treated as an additional class that appears only in a subset of images. Sampling ratios between datasets can be adjusted to compensate for differences in dataset size.
...
\begin{tabular}{lcccc}
\toprule
Config & Model & Pretrain Data & Fine-tune Data & Trail Length \\
\midrule
FS-Y & YOLO-style & none & USC-Canal & $T=1$ \\
R300-Y & YOLO-style & RADAR300 & USC-Canal & $T=1$ \\
R300J-Y & YOLO-style & RADAR300+Canal & same & $T=1$ \\
R300T-Y & YOLO-style & RADAR300 & USC-Canal & $T\in\{1,4,8,16\}$ \\
FS-F & Faster R-CNN & none & USC-Canal & $T=1$ \\
R300-F & Faster R-CNN & RADAR300 & USC-Canal & $T=1$ \\
\bottomrule
\end{tabular}
\end{table}
...
All models are trained on a modern GPU (for example, an NVIDIA RTX-class card), and training times remain modest due to the small size of the canal dataset. Pretraining on RADAR300 dominates overall training cost. Detailed hyperparameters are left as placeholders in this draft and can be specified once final experiments are completed.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig21_training_curves.png}
\caption{Training curves showing loss evolution during pretraining on RADAR300 (left) and fine-tuning on the USC canal dataset (middle). Right plot shows the full timeline. Bottom panels decompose total loss into classification and localization components. Fine-tuning starts from lower loss due to pretraining initialization.}
\label{fig:training_curves}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig22_lr_schedule.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig23_annotator_gui.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig24_annotator_workflow.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig25_radar2png_workflow.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig26_cli_examples.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig27_toolchain_integration.png}
...
Radar2PNG & raw radar & PNG frames & polar-to-Cartesian conversion \\
EchoTrail & PNG sequences & composite frames & temporal augmentation \\
\bottomrule
\end{tabular}
\end{table}
...
\label{sec:setup}
This section specifies the experimental protocol used to evaluate the impact of RADAR300 pretraining and echo-trail augmentation on detection performance in the canal dataset.
...
RADAR300 is split into training and validation sets following either its original documentation or a standard $80$/$20$ division. No canal frames are used during RADAR300 pretraining, and no RADAR300 frames are used during canal-only test evaluation. For joint training regimes, sampling ratios are tuned to ensure that canal frames are seen frequently despite the larger size of RADAR300.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig28_dataset_split.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig29_class_imbalance.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig30_metadata_distribution.png}
...
\section{Results}
\label{sec:results}
This section reports the impact of RADAR300 pretraining and synthetic echo trails on canal detection performance. At the time of this draft, numerical values are represented as placeholders; they will be replaced with actual mAP scores once experiments are completed.

\subsection{Effect of RADAR300 Pretraining}
Table~\ref{tab:results_pretrain} illustrates the effect of RADAR300 pretraining for the YOLO-style and Faster R-CNN detectors. Each entry reports mAP@0.5 on the canal test set, along with per-class AP for dynamic and static objects.

\begin{table}[t]
\centering
\caption{Placeholder results for the effect of RADAR300 pretraining on canal detection performance. All values are to be replaced with actual numbers.}
...
The expected qualitative pattern is that RADAR300 pretraining improves overall mAP, with particularly strong gains for the dynamic class, which aligns directly with the boat class in RADAR300. Static objects may benefit indirectly through shared low-level features and context modeling. The magnitude of improvement provides a quantitative measure of transfer effectiveness from RADAR300 to the canal domain.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig31_pretraining_effect.png}
\caption{Pretraining effect: mAP@0.5 scores for YOLO and Faster R-CNN detectors trained from-scratch versus pretrained on RADAR300. Pretraining boosts performance by approximately 12-14 percentage points across both architectures, demonstrating significant transfer learning benefits even with minimal labeled canal data.}
\label{fig:pretraining_effect}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig32_per_class_breakdown.png}
\caption{Per-class average precision for dynamic and static objects. Pretraining provides larger gains for the dynamic class (boats in motion), which aligns with RADAR300's single-boat class. Static objects also benefit from shared low-level features, though gains are more modest, likely due to class imbalance in the training data.}
\label{fig:per_class_breakdown}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig33_echo_trail_performance.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig34_per_class_echo_trail.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig35_pr_curves.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig36_qualitative_detections.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig37_failure_analysis.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig38_confusion_matrix.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig39_calibration_curve.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig40_size_performance.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig41_range_performance.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig42_temporal_performance.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig43_weather_performance.png}
...
Qualitative examples on the Pohang Canal dataset illustrate how well detectors trained on RADAR300 and the USC canal dataset transfer to a new canal environment with different radar hardware and traffic patterns. A set of figures can show representative radar frames from Pohang alongside predicted bounding boxes for dynamic-like objects. Even in the absence of additional training, reasonable detections would support the claim that RADAR300 pretraining plus canal fine-tuning yields models that capture generic features of boats in canal-like radar imagery.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig44_pohang_detections.png}
\caption{Cross-dataset generalization: detection results on Pohang Canal dataset using a model trained on RADAR300 and USC Canal data without any retraining. Green boxes show successful detections, demonstrating that learned features transfer reasonably well to unseen radar hardware and geographic environments.}
\label{fig:pohang_detections}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig45_pohang_failure_cases.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig46_pohang_usc_comparison.png}
...
From the perspective of dataset design, RADAR300 proves valuable as a generic pretraining source. Its focus on boat detection aligns well with dynamic targets in the canal dataset, and its relatively larger number of labeled frames helps initialize detectors that would otherwise overfit on $96$ canal frames. At the same time, RADAR300 does not cover the full diversity of inland lakes, rivers, and docks in South Carolina and Virginia, nor does it reflect the range-setting variability and quantization peculiarities of the Furuno NXT radar. The canal dataset therefore fills an important gap but remains small; scaling the labeled subset will be a priority in future work.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig49_quantization_effect.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig50_computational_cost.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig51_raw_data_format.png}
\caption{Appendix A: Raw Furuno NXT data format and processing pipeline. Left: byte-level specification of sweep packet structure including header, timestamp, azimuth, range setting, quantization, and range-bin data. Right: Radar2PNG processing pipeline stages from raw input through final PNG output, enabling exact reproducibility on compatible Furuno equipment.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig52_hyperparameter_sensitivity.png}
\caption{Appendix B: Hyperparameter sensitivity analysis for YOLO R300-pretrained detector. Learning rate shows narrow optimal range near $10^{-3}$; batch size is robust across [8--32]; weight decay near $10^{-4}$ preferred; 50 epochs sufficient for convergence; warmup helps training stability. Recommended settings: LR=$10^{-3}$, batch size=16, weight decay=$10^{-4}$, epochs=50, warmup=50.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig53_ablation_matrix.png}
\caption{Appendix C: Ablation study matrix quantifying the contribution of each technique to overall performance. Pretraining on RADAR300 provides the largest gain (+18.9 percentage points, from 0.542 to 0.731 mAP@0.5). Echo-trail augmentation adds +2.7 percentage points. Data augmentation (flip/rotate) provides marginal benefit. Combined approach achieves +23.0 percentage-point improvement over from-scratch baseline.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig54_collection_sites.png}
\caption{Appendix D: Data collection locations across the southeastern United States. Lake Murray, Lake Greenwood, and Lake Monticello represent inland freshwater environments with varying boat density and clutter patterns. Elizabeth River provides river-navigation context. Charleston Harbor offers coastal, high-traffic maritime scenarios. Folly Beach captures open-ocean conditions. This geographic and environmental diversity supports robust model generalization.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig55_hardware_comparison.png}
\caption{Appendix E: Radar hardware ecosystem comparison. Furuno NXT (24 GHz, 4-bit native quantization, \$4.5K) provides cost-effective, high-resolution sensing suitable for inland autonomy. Simrad 3G (9.5 GHz X-band, 8-bit, \$12K) and Lowrance HB (3.1 GHz S-band, 8-bit, \$18K) offer extended range at higher cost. Frequency mismatch between systems introduces domain gap requiring retraining or transfer learning. Future work will explore generalization across this hardware diversity.}
\label{fig:hardware_comparison}
\end{figure}

\section*{Acknowledgments}
The authors thank the undergraduate students and field teams who assisted with radar installation, maintenance, and data collection on Lake Murray, Lake Greenwood, Lake Monticello, the Elizabeth River, the Charleston harbor region, and Folly Beach. Their efforts made this dataset possible. Additional thanks are due to collaborators and staff at the University of South Carolina and partner institutions who supported access to vessels and facilities.
