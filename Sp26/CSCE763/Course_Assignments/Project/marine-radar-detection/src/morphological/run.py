"""Run morphological detector on the COCO test set, save predictions, print AP."""
...
from .detector import detect_morphological
...
def main():
    ...
    ap.add_argument("--out-dir", default="results/morphological")
    args = ap.parse_args()

    cfg = yaml.safe_load(Path(args.config).read_text())["morphological"]
    ...
    preds = []
    for info in tqdm(ann["images"], desc="morphological"):
        ...
        bboxes, _, _, scores = detect_morphological(
            imgf,
    ...
    pred_path = out_dir / "morphological_predictions.json"
