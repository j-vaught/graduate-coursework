...
\author{Instructor: Dr. Yan Tong \\ University of South Carolina \\ \textit{Solutions by: JC Vaught}}
...
\begin{stepbox}[Part (b): Huffman Coding]
...
The resulting Huffman code table is as follows.
...
\begin{stepbox}[Part (c): Lossless Predictive Coding (DPCM) + Huffman]
...
\textbf{Prediction errors} (63 values):
...
\textbf{Error frequency distribution and Huffman codes:}
...
\begin{resultsbox}
The prediction errors have entropy $H_e \approx 3.09$ bits (vs.\ 3.63 for raw pixels), confirming that DPCM removes significant inter-pixel redundancy.
...
\[
\boxed{\mathbf{S}_x = \mathbf{d}_v \cdot \mathbf{s}_h^T =
\begin{bmatrix} -1 \\ 0 \\ 1 \end{bmatrix}
\begin{bmatrix} 1 & 2 & 1 \end{bmatrix}}
\qquad
\boxed{\mathbf{S}_y = \mathbf{s}_v \cdot \mathbf{d}_h^T =
\begin{bmatrix} 1 \\ 2 \\ 1 \end{bmatrix}
\begin{bmatrix} -1 & 0 & 1 \end{bmatrix}}
\]
