...
\]
where $f(x,y) = 0$ outside the image (zero-padding). The output has the \textbf{same size} as the input.
...
\begin{stepbox}[Part (a): $2 \times 2$ Image $\to$ $2 \times 2$ Output]
...
\begin{stepbox}[Part (b): $3 \times 3$ Image $\to$ $3 \times 3$ Output]
Image: $f(1,1) = 1$, all other $f(i,j) = 0$. This is a unit impulse at position $(1,1)$.

Since the only nonzero pixel is $f(1,1) = 1$, each output value $g(x,y)$ has at most one nonzero term, occurring when $s = x-1$ and $t = y-1$:
\[
g(x,y) = w(x-1,\, y-1) \cdot 1 = w(x-1,\, y-1)
\]
The output is simply the filter $w$ itself --- the fundamental impulse response property.
\end{stepbox}
...
The convolution of a unit impulse with a filter produces the filter itself. $\blacksquare$
\end{stepbox}

\begin{resultsbox}
\textbf{(a)} The $2 \times 2$ convolution output with zero-padded borders:
\[
g = \begin{bmatrix} -2 & 1 \\ 8 & 9 \end{bmatrix}
\]

\textbf{(b)} The $3 \times 3$ convolution output for the impulse image equals the filter $w$:
\[
g = \begin{bmatrix} -1 & 0 & -1 \\ 0 & 2 & 0 \\ 1 & 0 & 1 \end{bmatrix} = w \qquad \blacksquare
\]
\end{resultsbox}
