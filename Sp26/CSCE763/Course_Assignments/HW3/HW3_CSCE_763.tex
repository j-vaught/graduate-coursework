...
\begin{tikzpicture}[scale=1.0]
  ...
  \fill[white] (0,0) rectangle (4.5,4.5);

  % --- Top row: horizontal bar groups (decreasing size) ---
  % Group 1 (largest)
  \foreach \i in {0,...,2} {
    \fill[black] (0.2+\i*0.35, 4.1) rectangle (0.2+\i*0.35+0.15, 3.75);
  }
  % Group 2
  \foreach \i in {0,...,3} {
    \fill[black] (1.5+\i*0.25, 4.1) rectangle (1.5+\i*0.25+0.1, 3.75);
  }
  % Group 3
  \foreach \i in {0,...,4} {
    \fill[black] (2.7+\i*0.18, 4.1) rectangle (2.7+\i*0.18+0.07, 3.75);
  }
  % Group 4 (smallest)
  \foreach \i in {0,...,5} {
    \fill[black] (3.7+\i*0.12, 4.1) rectangle (3.7+\i*0.12+0.04, 3.75);
  }

  % --- Top row: vertical bar groups ---
  \foreach \i in {0,...,2} {
    \fill[black] (0.2, 3.6-\i*0.25) rectangle (0.55, 3.6-\i*0.25-0.1);
  }
  \foreach \i in {0,...,3} {
    \fill[black] (1.5, 3.6-\i*0.18) rectangle (1.85, 3.6-\i*0.18-0.07);
  }
  \foreach \i in {0,...,4} {
    \fill[black] (2.7, 3.6-\i*0.14) rectangle (3.0, 3.6-\i*0.14-0.05);
  }
  \foreach \i in {0,...,5} {
    \fill[black] (3.7, 3.6-\i*0.1) rectangle (3.95, 3.6-\i*0.1-0.035);
  }

  % --- Large "a" character in center ---
  \node[font=\fontsize{36}{36}\selectfont\bfseries, anchor=center] at (2.25, 2.2) {a};

  % --- Small dots to left of "a" ---
  \foreach \i in {0,...,2} {
    \fill[black] (0.3+\i*0.2, 2.4) circle (0.06);
  }

  % --- Lower section: vertical bars (the key feature for this problem) ---
  % Bar group: 5px wide bars, 20px separation, scaled
  % Using scale: 5px -> 0.15cm, 20px -> 0.6cm
  \foreach \i in {0,...,3} {
    \fill[black] (0.2+\i*0.75, 0.2) rectangle (0.2+\i*0.75+0.15, 1.3);
  }

  % --- Horizontal bars (lower left) ---
  \foreach \i in {0,...,5} {
    \fill[black] (0.2, 1.5+\i*0.12) rectangle (1.3, 1.5+\i*0.12+0.04);
  }

  % --- Gray patches (lower right) ---
  \fill[gray!30] (2.5, 0.2) rectangle (3.4, 0.9);
  \fill[gray!60] (3.5, 0.2) rectangle (4.3, 0.9);

  % --- Bottom row: tiny elements ---
  \foreach \i in {0,...,7} {
    \fill[black] (0.3+\i*0.5, 0.05) rectangle (0.3+\i*0.5+0.15, 0.15);
  }
...
\vspace{0.3cm}
\begin{center}
\textbf{Figure 2.} Original resolution test chart before smoothing. The three blurred versions use square averaging masks of sizes $n=23$, $n=25$, and $n=45$, respectively.
\end{center}
...
%
\begin{enumerate}[label=(\alph*)]
\item a $2 \times 2$ image as shown in Figure~3(a) \hfill (10 pts)
\item a $3 \times 3$ image as shown in Figure~3(b) \hfill (10 pts)
\end{enumerate}
