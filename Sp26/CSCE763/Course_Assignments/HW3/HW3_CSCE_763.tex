...
\begin{center}
\begin{tikzpicture}[
  cell/.style={minimum size=1cm, draw, thick, font=\Large\bfseries},
  labelcell/.style={minimum size=0.9cm, draw, thick, fill=lightblue, font=\Large\bfseries},
  tealcell/.style={minimum size=1cm, draw, thick, fill=tealfill, font=\Large\bfseries},
  ltealcell/.style={minimum size=1cm, draw, thick, fill=teallight, font=\Large\bfseries},
]

% Figure 3(a): 3x3 filter
\matrix (filter) [matrix of nodes, nodes={labelcell}, column sep=-\pgflinewidth, row sep=-\pgflinewidth] {
  $-1$ & $0$ & $-1$ \\
  $0$  & $2$ & $0$  \\
  $1$  & $0$ & $1$  \\
};
\node[below=0.3cm] at (filter.south) {\textbf{(a)}};

% Figure 3(b): 2x2 image
\begin{scope}[shift={(5.5,0.5)}]
\matrix (img2) [matrix of nodes, column sep=-\pgflinewidth, row sep=-\pgflinewidth] {
  |[tealcell]| $1$ & |[tealcell]| $2$ \\
  |[ltealcell]| $3$ & |[ltealcell]| $4$ \\
};
\node[below=0.3cm] at (img2.south) {\textbf{(b)}};
\end{scope}

% Figure 3(c): 3x3 image
\begin{scope}[shift={(10,0)}]
\matrix (img3) [matrix of nodes, nodes={labelcell}, column sep=-\pgflinewidth, row sep=-\pgflinewidth] {
  $0$ & $0$ & $0$ \\
  $0$ & $1$ & $0$ \\
  $0$ & $0$ & $0$ \\
};
\node[below=0.3cm] at (img3.south) {\textbf{(c)}};
\end{scope}
