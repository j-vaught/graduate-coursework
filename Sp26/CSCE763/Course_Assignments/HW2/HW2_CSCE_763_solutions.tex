...
\item \textbf{Illumination Intensity:} Brightness changes ($\pm 40$ levels) cause uniform shifts across the image. \textit{Req: Fixed, calibrated light sources.}
\item \textbf{Spatial Translation:} Even 5-pixel shifts create edge artifacts. \textit{Req: Mechanical fixtures, sub-pixel alignment.}
\item \textbf{Rotational Misalignment:} 2° rotation causes arc-shaped differences. \textit{Req: Alignment fixtures, rotation correction.}
\item \textbf{Scale/Zoom Variation:} 3\% scale change creates edge differences. \textit{Req: Fixed focal length, working distance.}
\item \textbf{Sensor Noise:} Gaussian noise ($\sigma=20$) creates speckle. \textit{Req: Low-noise sensors, temporal averaging.}
\item \textbf{Focus Blur:} Out-of-focus reduces edge contrast. \textit{Req: Auto-focus locks, telecentric lenses.}
\item \textbf{Non-uniform Illumination:} Vignetting causes darker corners/edges. \textit{Req: Flat-field correction, uniform lighting.}
\item \textbf{Contrast/Gain Variation:} 30\% contrast change affects dynamic range. \textit{Req: Fixed camera settings, auto-gain disabled.}
\item \textbf{Color Temperature:} Warm/cool shifts affect color channels. \textit{Req: Fixed white balance, controlled spectrum LED.}
\item \textbf{Perspective Distortion:} Viewing angle changes cause trapezoidal distortion. \textit{Req: Telecentric optics, fixed mount.}
\item \textbf{Exposure Variation:} EV changes cause non-linear brightness shifts. \textit{Req: Fixed shutter, controlled ambient light.}
\item \textbf{Directional Shadows:} Shadows create localized intensity gradients. \textit{Req: Diffuse lighting, light tents.}
\end{enumerate}
\end{stepbox}
