...
\textbf{1. Illumination Intensity} & \textbf{7. Non-uniform Illumination} \\
Brightness changes ($\pm 40$ levels) cause uniform shifts across the image. \textit{Req: Fixed, calibrated light sources.} &
Vignetting causes darker corners/edges. \textit{Req: Flat-field correction, uniform lighting.} \\[0.4em]

\textbf{2. Spatial Translation} & \textbf{8. Contrast/Gain Variation} \\
Even 5-pixel shifts create edge artifacts. \textit{Req: Mechanical fixtures, sub-pixel alignment.} &
30\% contrast change affects dynamic range. \textit{Req: Fixed camera settings, auto-gain disabled.} \\[0.4em]

\textbf{3. Rotational Misalignment} & \textbf{9. Color Temperature} \\
2° rotation causes arc-shaped differences. \textit{Req: Alignment fixtures, rotation correction.} &
Warm/cool shifts affect color channels. \textit{Req: Fixed white balance, controlled spectrum LED.} \\[0.4em]

\textbf{4. Scale/Zoom Variation} & \textbf{10. Perspective Distortion} \\
3\% scale change creates edge differences. \textit{Req: Fixed focal length, working distance.} &
Viewing angle changes cause trapezoidal distortion. \textit{Req: Telecentric optics, fixed mount.} \\[0.4em]

\textbf{5. Sensor Noise} & \textbf{11. Exposure Variation} \\
Gaussian noise ($\sigma=20$) creates speckle. \textit{Req: Low-noise sensors, temporal averaging.} &
EV changes cause non-linear brightness shifts. \textit{Req: Fixed shutter, controlled ambient light.} \\[0.4em]

\textbf{6. Focus Blur} & \textbf{12. Directional Shadows} \\
Out-of-focus reduces edge contrast. \textit{Req: Auto-focus locks, telecentric lenses.} &
Shadows create localized intensity gradients. \textit{Req: Diffuse lighting, light tents.} \\
\end{tabular}
\end{stepbox}
