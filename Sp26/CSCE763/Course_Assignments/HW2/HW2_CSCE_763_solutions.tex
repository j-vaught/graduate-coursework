...
\begin{axis}[
    ...
    ymin=0, ymax=14,
    xtick={1,2,3},
    xticklabels={Pixel 1, Pixel 2, Pixel 3},
    ylabel={Intensity Value},
    legend style={at={(0.5,-0.2)}, anchor=north, legend columns=3, sharp corners},
    ymajorgrids=true,
    grid style={dashed, gray!30},
    axis lines=left,
    enlarge x limits=0.3,
    sharp plot,
    every axis plot/.append style={sharp corners},
]
\addplot[fill=USC_Garnet, draw=USC_Garnet] coordinates {(1,2) (2,3) (3,10)};
\addplot[fill=USC_Atlantic, draw=USC_Atlantic] coordinates {(1,10) (2,1) (3,2)};
\addplot[fill=USC_Congaree, draw=USC_Congaree] coordinates {(1,12) (2,4) (3,12)};
\legend{$f_1$ (med=3), $f_2$ (med=2), $f_1+f_2$ (med=12)}
\end{axis}
% Annotation
\node[anchor=west, text=USC_Garnet, font=\small\bfseries] at (10,4) {RHS $= 3+2 = 5$};
\node[anchor=west, text=USC_Congaree, font=\small\bfseries] at (10,3.3) {LHS $= 12$};
\node[anchor=west, text=USC_Rose, font=\small\bfseries] at (10,2.6) {$12 \neq 5$ \checkmark};
\end{tikzpicture}
\end{center}
\end{stepbox}
...
\begin{stepbox}[Step 1: Consistent Illumination]
The lighting conditions---direction, intensity, color temperature, and uniformity---must be \textbf{identical} when capturing the golden image and every subsequent test image. Any change in illumination causes intensity differences unrelated to actual defects, producing false positives. In practice, this requires fixed, controlled lighting enclosures.
\end{stepbox}

\begin{stepbox}[Step 2: Precise Spatial Registration]
The camera position, orientation, and product placement must be \textbf{exactly the same} for every image. Even sub-pixel misalignment between the golden and test images produces nonzero differences at edges and high-contrast boundaries, masking true defects or creating false detections. Mechanical fixtures and calibrated camera mounts achieve this.
\end{stepbox}

\begin{stepbox}[Step 3: Additional Factors]
\begin{itemize}[leftmargin=*]
\item \textbf{Sensor noise thresholding:} Random sensor noise produces small nonzero differences even with perfect alignment and lighting. A threshold on the difference image distinguishes genuine defects from noise artifacts.
\item \textbf{Consistent camera settings:} Gain, exposure time, white balance, and focus must remain fixed between the golden capture and all subsequent captures.
\end{itemize}
\end{stepbox}

\begin{stepbox}[Visualization: Image Subtraction Flowchart]
\begin{center}
\begin{tikzpicture}[
    node distance=1.5cm and 2.5cm,
    box/.style={rectangle, draw=USC_Garnet, thick, fill=white, minimum width=2.8cm, minimum height=1cm, align=center, font=\small\bfseries, sharp corners},
    opbox/.style={circle, draw=USC_Congaree, thick, fill=USC_Congaree!10, minimum size=1cm, align=center, font=\large\bfseries},
    annot/.style={font=\scriptsize, text=USC_Black70, align=center},
    arr/.style={-{Stealth[length=3mm]}, thick, USC_Black90},
]
    \node[box] (golden) {Golden\\Image};
    \node[opbox, right=2cm of golden] (sub) {$-$};
    \node[box, above=1cm of sub] (test) {Test\\Image};
    \node[box, right=2cm of sub] (diff) {Difference\\Image};
    \node[box, right=2cm of diff] (result) {Defect\\Decision};

    \draw[arr] (golden) -- (sub);
    \draw[arr] (test) -- (sub);
    \draw[arr] (sub) -- (diff);
    \draw[arr] (diff) -- node[above, annot] {threshold} (result);

    % Factor annotations
    \node[annot, below=0.3cm of golden] {Fixed lighting\\Fixed position};
    \node[annot, above=0.3cm of test] {Same lighting\\Same position};
    \node[annot, below=0.3cm of diff] {Noise\\filtering};
\end{tikzpicture}
\end{center}
\end{stepbox}

\begin{resultsbox}
The two major factors are: (1) \textbf{consistent illumination} so that intensity differences reflect actual physical differences, not lighting variations; and (2) \textbf{precise spatial alignment/registration} so that geometric misalignment does not produce spurious nonzero differences.
\end{resultsbox}
...
% Panel 1: Original (arbitrary histogram)
\begin{axis}[at={(0,0)}, xlabel={$r$}, ylabel={$p_r(r)$}, title={\small Original}]
\addplot[ybar, bar width=6pt, fill=USC_Garnet, draw=USC_Garnet]
    coordinates {(1,0.05) (2,0.35) (3,0.15) (4,0.5) (5,0.25) (6,0.1)};
\end{axis}

% Arrow 1
\node[font=\Large, USC_Black90] at (4.8,1.5) {$\xrightarrow{T(r)}$};

% Panel 2: After first equalization (uniform)
\begin{axis}[at={(5.5cm,0)}, xlabel={$s$}, ylabel={$p_s(s)$}, title={\small After 1st Pass}]
\addplot[ybar, bar width=6pt, fill=USC_Atlantic, draw=USC_Atlantic]
    coordinates {(1,0.23) (2,0.23) (3,0.23) (4,0.23) (5,0.23) (6,0.23)};
\end{axis}

% Arrow 2
\node[font=\Large, USC_Black90] at (10.3,1.5) {$\xrightarrow{T_2(s)=s}$};

% Panel 3: After second equalization (still uniform)
\begin{axis}[at={(11cm,0)}, xlabel={$v$}, ylabel={$p_v(v)$}, title={\small After 2nd Pass}]
\addplot[ybar, bar width=6pt, fill=USC_Atlantic, draw=USC_Atlantic]
    coordinates {(1,0.23) (2,0.23) (3,0.23) (4,0.23) (5,0.23) (6,0.23)};
\end{axis}
...
\begin{axis}[
    ...
    xlabel={\small $r_k$},
    ylabel={\small $n_k$},
    title={\small\bfseries Input (Table A)},
    axis lines=left,
    ymajorgrids=true, grid style={dashed, gray!20},
    sharp plot,
]
\addplot[fill=USC_Garnet, draw=USC_Garnet] coordinates {
(0,16)(1,0)(2,512)(3,32)(4,64)(5,128)(6,64)(7,1024)(8,512)(9,512)(10,16)(11,1024)(12,0)(13,128)(14,0)(15,64)};
\end{axis}

\begin{axis}[
    name=plot2, at={(plot1.east)}, anchor=west, xshift=0.8cm,
    ybar, bar width=5pt,
    width=5.5cm, height=4.5cm,
    ymin=0, ymax=1200,
    xmin=-0.5, xmax=15.5,
    ...
    xlabel={\small $z_q$},
    ylabel={\small $n_q$},
    title={\small\bfseries Desired (Table B)},
    axis lines=left,
    ymajorgrids=true, grid style={dashed, gray!20},
    sharp plot,
]
\addplot[fill=USC_Atlantic, draw=USC_Atlantic] coordinates {
(0,64)(1,128)(2,512)(3,512)(4,1024)(5,1024)(6,512)(7,128)(8,128)(9,64)(10,0)(11,0)(12,0)(13,0)(14,0)(15,0)};
\end{axis}

\begin{axis}[
    name=plot3, at={(plot2.east)}, anchor=west, xshift=0.8cm,
    ybar, bar width=5pt,
    width=5.5cm, height=4.5cm,
    ymin=0, ymax=1200,
    xmin=-0.5, xmax=15.5,
    xtick={0,3,...,15},
    xlabel={\small $z_q$},
    ylabel={\small $n_q^{\text{out}}$},
    title={\small\bfseries Actual Output},
    axis lines=left,
    ymajorgrids=true, grid style={dashed, gray!20},
    sharp plot,
]
\addplot[fill=USC_Congaree, draw=USC_Congaree] coordinates {
(0,16)(1,0)(2,800)(3,0)(4,1024)(5,1040)(6,1024)(7,0)(8,192)(9,0)(10,0)(11,0)(12,0)(13,0)(14,0)(15,0)};
\end{axis}
\end{tikzpicture}
\end{center}
\end{stepbox}
...
\begin{axis}[
    ...
    xlabel={$r$}, ylabel={$z = f(r)$},
    xmin=0, xmax=1.05, ymin=0, ymax=1.05,
    xtick={0, 0.25, 0.5, 0.75, 1.0},
    xticklabels={$0$, $\frac{W}{4}$, $\frac{W}{2}$, $\frac{3W}{4}$, $W$},
    ytick={0, 0.25, 0.5, 0.75, 1.0},
    yticklabels={$0$, $\frac{W}{4}$, $\frac{W}{2}$, $\frac{3W}{4}$, $W$},
    axis lines=left,
    grid=major, grid style={dashed, gray!30},
    sharp plot,
]
% Quarter-circle arc: z = sqrt(r(2-r)) normalized to [0,1] => z = sqrt(r(2-r))
\addplot[thick, USC_Garnet, domain=0:1, samples=100] {sqrt(x*(2-x))};
% Identity line for reference
\addplot[dashed, USC_Black50, domain=0:1] {x};
\end{axis}
\node[USC_Garnet, font=\small\bfseries] at (5.5,5.5) {$z = \sqrt{r(2W\!-\!r)}$};
\node[USC_Black50, font=\small] at (5.8,3.8) {$z = r$};
\end{tikzpicture}
\end{center}
