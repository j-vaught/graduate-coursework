...
\item \textbf{Illumination Intensity.} Changes in overall brightness between the golden image and test image cause uniform intensity shifts across the entire frame. Even modest variations of $\pm 40$ intensity levels produce large difference signals that can overwhelm subtle defect signatures. Controlling this requires fixed, calibrated light sources with stable power supplies.

\item \textbf{Spatial Translation.} If the product is not positioned identically to when the golden image was captured, even small shifts of 5 pixels create prominent edge artifacts in the difference image. Mechanical fixtures and sub-pixel alignment algorithms are essential to prevent positional mismatches from triggering false positives.

\item \textbf{Rotational Misalignment.} When the product rotates slightly relative to the golden reference, arc-shaped difference patterns emerge around curved features. A rotation of just 2° can produce detectable artifacts, necessitating precise alignment fixtures or software-based rotation correction.

\item \textbf{Scale and Zoom Variation.} Differences in camera-to-object distance or focal length settings cause the test image to appear slightly larger or smaller than the reference. A 3\% scale change creates ring-like edge differences, requiring fixed working distances and locked zoom settings.

\item \textbf{Sensor Noise.} Random electronic noise in the camera sensor introduces speckle patterns throughout the difference image. Gaussian noise with $\sigma = 20$ can mask small defects entirely, making low-noise sensors and temporal averaging across multiple frames important countermeasures.

\item \textbf{Focus Blur.} When the test image is slightly out of focus compared to the sharp golden reference, edges become softer and produce haloed differences. Reliable detection requires auto-focus locks or telecentric lenses that maintain consistent focus across the field of view.

\item \textbf{Non-uniform Illumination.} Vignetting and uneven light distribution cause the corners and edges of images to appear darker than the center. This spatial intensity gradient creates systematic differences even when the product is identical, requiring flat-field correction or carefully designed uniform lighting.

\item \textbf{Contrast and Gain Variation.} Changes in camera gain settings or lighting intensity ratios alter the dynamic range of captured images. A 30\% contrast shift compresses or expands intensity differences, so fixed camera settings with automatic gain control disabled are necessary.

\item \textbf{Color Temperature.} Shifts between warm and cool illumination alter the balance of color channels differently. If the golden image was captured under daylight-balanced lighting but production uses warmer LEDs, the RGB channels will show systematic offsets requiring fixed white balance settings.

\item \textbf{Perspective Distortion.} Changes in the viewing angle between captures cause trapezoidal warping of the product's appearance. This geometric distortion creates systematic misalignment that cannot be corrected by simple translation or rotation, requiring telecentric optics or rigidly fixed camera mounts.

\item \textbf{Exposure Variation.} Differences in shutter speed or ambient light levels cause non-linear brightness shifts that affect highlights and shadows differently. Consistent detection requires fixed exposure settings and controlled ambient lighting to eliminate environmental variability.

\item \textbf{Directional Shadows.} Shadows cast by the product or nearby objects create localized intensity gradients that vary with lighting position. These shadows produce strong difference signals in regions that may overlap with actual defect locations, making diffuse lighting or light tents essential for shadow-free imaging.
\end{enumerate}
\end{stepbox}
