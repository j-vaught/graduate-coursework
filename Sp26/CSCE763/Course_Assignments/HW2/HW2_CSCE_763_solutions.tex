...
An operator $H$ is linear if, for all $f_1,f_2$ and scalars $a_1,a_2$,
\[
H[a_1 f_1 + a_2 f_2] = a_1 H[f_1] + a_2 H[f_2]
\]
To prove nonlinearity, one counterexample is enough. Set $a_1=a_2=1$ and test additivity on a $1\times 3$ region.
\end{stepbox}
...
\]
and compare $H[f_1+f_2]$ with $H[f_1]+H[f_2]$.
\end{stepbox}

\begin{stepbox}[Step 3: Compute the Right-Hand Side]
\[
\text{median}(f_1)=\text{median}\{2,3,10\}=3,\qquad
\text{median}(f_2)=\text{median}\{10,1,2\}=2
\]
\[
...
\[
H[f_1 + f_2] = 12 \qquad \text{and} \qquad H[f_1] + H[f_2] = 5
\]
Since $12 \neq 5$, additivity fails, so the median operator is \textbf{nonlinear}.
\[
...
\begin{stepbox}[Step 4: Map $s_k \to z_q$ by Closest Match]
For each value of $s_k$, choose the $z_q$ that minimizes $|G(z_q)-s_k|$.
If there is a tie, choose the smaller $z_q$.

\begin{center}
\rowcolors{2}{USC_Garnet!10}{white}
\begin{tabular}{c c c c}
\rowcolor{USC_Garnet!80}
\textcolor{white}{$r_k$} & \textcolor{white}{$s_k$} & \textcolor{white}{Closest match in $G(z_q)$} & \textcolor{white}{$z_q$} \\
\hline
0  & 0  & $|G(0)-0|=0$ (min) & 0 \\
1  & 0  & $|G(0)-0|=0$ (min) & 0 \\
2  & 2  & Tie: $|G(1)-2|=1,\;|G(2)-2|=1 \Rightarrow z_1$ & 1 \\
3  & 2  & Tie: $|G(1)-2|=1,\;|G(2)-2|=1 \Rightarrow z_1$ & 1 \\
4  & 2  & Tie: $|G(1)-2|=1,\;|G(2)-2|=1 \Rightarrow z_1$ & 1 \\
5  & 3  & $|G(2)-3|=0$ (min) & 2 \\
6  & 3  & $|G(2)-3|=0$ (min) & 2 \\
7  & 7  & $|G(4)-7|=1$ (min) & 4 \\
8  & 9  & $|G(4)-9|=1$ (min) & 4 \\
9  & 10 & Tie: $|G(4)-10|=2,\;|G(5)-10|=2 \Rightarrow z_4$ & 4 \\
10 & 11 & $|G(5)-11|=1$ (min) & 5 \\
11 & 14 & $|G(6)-14|=0$ (min) & 6 \\
12 & 14 & $|G(6)-14|=0$ (min) & 6 \\
13 & 15 & $|G(8)-15|=0$ (min) & 8 \\
14 & 15 & $|G(8)-15|=0$ (min) & 8 \\
15 & 15 & $|G(8)-15|=0$ (min) & 8 \\
\hline
\end{tabular}
\end{center}
\end{stepbox}
...
\hline
0 & $r_0, r_1$              & $16 + 0 = 16$ \\
1 & $r_2, r_3, r_4$          & $512 + 32 + 64 = 608$ \\
2 & $r_5, r_6$               & $128 + 64 = 192$ \\
3 & ---                      & 0 \\
4 & $r_7, r_8, r_9$          & $1024 + 512 + 512 = 2048$ \\
5 & $r_{10}$                 & 16 \\
...
9--15 & ---                  & 0 each \\
\hline
\multicolumn{2}{r}{\textbf{Total:}} & $16+608+192+2048+16+1024+192 = 4096$ \checkmark \\
\end{tabular}
\end{center}
\end{stepbox}
