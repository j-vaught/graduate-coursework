...
An operator $H$ is said to be \textbf{linear} if and only if it satisfies the \textbf{superposition property} for all images $f_1, f_2$ and all scalar constants $a_1, a_2$:
\[
H[a_1 f_1 + a_2 f_2] = a_1 H[f_1] + a_2 H[f_2]
\]
This property combines two fundamental requirements: \textit{additivity} (the response to a sum equals the sum of responses) and \textit{homogeneity} (scaling the input scales the output proportionally). For an operator to be linear, this equation must hold for \textit{every possible} combination of inputs and scalars. Conversely, to prove an operator is nonlinear, we only need to find a \textit{single} counterexample where this equality fails.

For simplicity, we will test the additivity condition by setting $a_1 = a_2 = 1$ and examining a small $1 \times 3$ subimage region $S$. If additivity fails, the full superposition property cannot hold, and the operator must be nonlinear.
\end{stepbox}

\begin{stepbox}[Step 2: Define the Counterexample]
We construct two simple images defined by their pixel values within the subimage region $S$. The key insight is to choose values where the median of each image individually differs significantly from the median of their sum:
\[
f_1 = \{2,\; 3,\; 10\}, \qquad f_2 = \{10,\; 1,\; 2\}
\]
Notice that $f_1$ has its values spread across a range with the middle value being 3, while $f_2$ has its values arranged so that the middle value when sorted is 2. These particular values are chosen because when we add them together pixel-wise, the resulting set will have a very different median than the sum of the individual medians.
\end{stepbox}

\begin{stepbox}[Step 3: Compute the Right-Hand Side]
First, we compute the median of each image separately and then add the results. This corresponds to the right-hand side of the linearity equation, $H[f_1] + H[f_2]$.

For $f_1 = \{2, 3, 10\}$: sorting gives $\{2, 3, 10\}$, and the middle value is $\mathbf{3}$.

For $f_2 = \{10, 1, 2\}$: sorting gives $\{1, 2, 10\}$, and the middle value is $\mathbf{2}$.

Therefore:
\[
H[f_1] + H[f_2] = \text{median}\{2,3,10\} + \text{median}\{10,1,2\} = 3 + 2 = \mathbf{5}
\]
\end{stepbox}

\begin{stepbox}[Step 4: Compute the Left-Hand Side]
Now we compute the left-hand side of the linearity equation, $H[f_1 + f_2]$, by first adding the images pixel-wise and then taking the median of the result.

Adding corresponding pixel values:
\[
f_1 + f_2 = \{2+10,\; 3+1,\; 10+2\} = \{12,\; 4,\; 12\}
\]

Now we find the median of this combined image. Sorting gives $\{4, 12, 12\}$, and the middle value is:
\[
H[f_1 + f_2] = \text{median}\{12, 4, 12\} = \mathbf{12}
\]

The crucial observation here is that the median operation selected different ``representative'' pixels in each case. When computed separately, the medians came from moderate values (3 and 2). But after addition, the pixel that was smallest in $f_1$ combined with the largest in $f_2$, shifting which pixel becomes the median of the sum.
\end{stepbox}

\begin{stepbox}[Step 5: Conclude]
Comparing the two sides of the superposition equation:
\[
H[f_1 + f_2] = 12 \qquad \text{versus} \qquad H[f_1] + H[f_2] = 5
\]

Since $12 \neq 5$, the superposition property is violated. This single counterexample is sufficient to prove that the median operator does not satisfy the linearity condition for all inputs. Therefore, the median operator is \textbf{nonlinear}.

Intuitively, this occurs because the median is a \textit{rank-based} statistic---it selects the middle value after sorting. When two images are added, the relative ranking of pixels can change dramatically, causing a completely different pixel to be selected as the median. This rank-shuffling behavior is fundamentally incompatible with linearity. $\blacksquare$
\end{stepbox}
...
\begin{stepbox}[Demonstration: Golden Image Analysis]
...
\begin{stepbox}[Factors Affecting Detection Performance]
\small
\begin{enumerate}[leftmargin=*, itemsep=0.4em]
\item \textbf{Illumination Intensity.} Changes in brightness between the golden and test images cause uniform intensity shifts across the frame. Even $\pm 40$ level variations produce difference signals that overwhelm subtle defects. Fixed, calibrated light sources with stable power are required.

\item \textbf{Spatial Translation.} If the product is not positioned identically to the golden capture, even 5-pixel shifts create prominent edge artifacts. Mechanical fixtures and sub-pixel alignment are essential to prevent positional mismatches from triggering false positives.

\item \textbf{Rotational Misalignment.} When the product rotates slightly relative to the reference, arc-shaped difference patterns emerge around curved features. A 2° rotation produces detectable artifacts, requiring precise alignment fixtures or rotation correction.

\item \textbf{Scale and Zoom Variation.} Differences in camera distance or focal length cause the test image to appear larger or smaller than the reference. A 3\% scale change creates ring-like edge differences, requiring fixed working distances and locked zoom.

\item \textbf{Sensor Noise.} Random electronic noise introduces speckle patterns throughout the difference image. Gaussian noise with $\sigma = 20$ can mask small defects, making low-noise sensors and temporal averaging important countermeasures.

\item \textbf{Focus Blur.} When the test image is out of focus compared to the sharp reference, edges become softer and produce haloed differences. Detection requires auto-focus locks or telecentric lenses for consistent focus.

\item \textbf{Non-uniform Illumination.} Vignetting causes corners and edges to appear darker than the center. This spatial gradient creates systematic differences even when the product is identical, requiring flat-field correction or uniform lighting.

\item \textbf{Contrast and Gain Variation.} Changes in camera gain or lighting ratios alter the dynamic range of captured images. A 30\% contrast shift distorts intensity differences, so fixed settings with auto-gain disabled are necessary.

\item \textbf{Color Temperature.} Shifts between warm and cool illumination alter color channel balance. If the golden image used daylight lighting but production uses warmer LEDs, systematic RGB offsets appear, requiring fixed white balance.

\item \textbf{Perspective Distortion.} Changes in viewing angle cause trapezoidal warping of the product's appearance. This distortion cannot be corrected by simple translation or rotation, requiring telecentric optics or fixed camera mounts.

\item \textbf{Exposure Variation.} Differences in shutter speed or ambient light cause non-linear brightness shifts affecting highlights and shadows. Consistent detection requires fixed exposure and controlled ambient lighting.

\item \textbf{Directional Shadows.} Shadows from the product or nearby objects create intensity gradients that vary with lighting position. These overlap with defect locations, making diffuse lighting or light tents essential.
\end{enumerate}
\end{stepbox}

\begin{stepbox}[Failure Modes Summary]
...
\begin{stepbox}[Quantitative Comparison]
