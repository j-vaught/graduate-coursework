...
\begin{stepbox}[Solution]
We prove nonlinearity by counterexample. Let $H$ denote the median operator over a $1 \times 3$ subimage region $S$. Set $a_1 = a_2 = 1$ and define two images by their pixel values over $S$:
\[
f_1 = \{1,\; 2,\; 3\}, \qquad f_2 = \{3,\; 1,\; 2\}
\]

\textbf{Right-hand side:}
\[
H[f_1] + H[f_2] = \text{median}\{1,2,3\} + \text{median}\{3,1,2\} = 2 + 2 = 4
\]

\textbf{Left-hand side:}
\[
f_1 + f_2 = \{1+3,\; 2+1,\; 3+2\} = \{4,\; 3,\; 5\}
\]
\[
H[f_1 + f_2] = \text{median}\{4,3,5\} = 4
\]

In this case the additive property holds. Now test the \textbf{homogeneity} (scaling) property with $a = 2$:
\[
H[2 f_1] = \text{median}\{2, 4, 6\} = 4 = 2 \cdot H[f_1]
\]
Scaling also holds here. We need a different counterexample. Consider instead:
\[
f_1 = \{1,\; 5,\; 9\}, \qquad f_2 = \{9,\; 1,\; 5\}
\]

\textbf{Right-hand side:}
\[
H[f_1] + H[f_2] = \text{median}\{1,5,9\} + \text{median}\{9,1,5\} = 5 + 5 = 10
\]
...
\]
\[
H[f_1 + f_2] = \text{median}\{10, 6, 14\} = 10
\]

Still equal. The problem with 3-element medians and $a_1=a_2=1$ is that sums of two sequences often preserve median ordering. Instead, use a \textbf{5-pixel} subimage:
\[
f_1 = \{1,\; 2,\; 3,\; 4,\; 5\}, \qquad f_2 = \{5,\; 4,\; 3,\; 2,\; 1\}
\]

\textbf{Right-hand side:}
\[
H[f_1] + H[f_2] = \text{median}\{1,2,3,4,5\} + \text{median}\{5,4,3,2,1\} = 3 + 3 = 6
\]
...
\]
\[
H[f_1 + f_2] = \text{median}\{6,6,6,6,6\} = 6
\]

Equal again (constant image). Let us try an asymmetric example with 3 pixels:
\[
f_1 = \{4,\; 1,\; 5\}, \qquad f_2 = \{1,\; 5,\; 2\}
\]

\textbf{Right-hand side:}
\[
H[f_1] + H[f_2] = \text{median}\{4,1,5\} + \text{median}\{1,5,2\} = 4 + 2 = 6
\]
...
\]
\[
H[f_1 + f_2] = \text{median}\{5, 6, 7\} = 6
\]

Equal. Consider a 3-pixel case that \emph{reorders} the ranking:
\[
...
\textbf{Left-hand side:}
\[
f_1 + f_2 = \{12,\; 4,\; 12\}, \qquad H[f_1+f_2] = 12
\]

Since $12 \neq 5$, the additivity property fails:
\[
H[f_1 + f_2] = 12 \neq 5 = H[f_1] + H[f_2]
\]
\end{stepbox}
...
\begin{resultsbox}
The median operator is \textbf{nonlinear}. As shown by the counterexample with $f_1 = \{2, 3, 10\}$ and $f_2 = \{10, 1, 2\}$, the superposition property is violated:
\[
...
\begin{stepbox}[Solution]
For image subtraction--based defect detection to work reliably, the following conditions must be satisfied:

\begin{enumerate}[label=\textbf{\arabic*.}]

\item \textbf{Consistent Illumination.} The lighting conditions (direction, intensity, color temperature, and uniformity) must be identical when capturing the golden image and every subsequent test image. Any change in illumination will cause intensity differences unrelated to actual defects, producing false positives. In practice, this means using fixed, controlled lighting enclosures.

\item \textbf{Precise Spatial Registration.} The camera position, orientation, and the placement of the product must be exactly the same for every image. Even a sub-pixel misalignment between the golden image and the test image will produce nonzero differences at edges and high-contrast boundaries, masking true defects or creating false detections. Mechanical fixtures and calibrated camera mounts are used to achieve this.

\end{enumerate}

\textbf{Additional important factors:}

\begin{itemize}
\item \textbf{Sensor Noise.} Even with perfect alignment and lighting, random sensor noise will produce small nonzero differences. A threshold must be applied to the difference image to distinguish genuine defects from noise artifacts.
...
\]
This maps intensities to the equalized image with intensity variable $s$. A fundamental result from probability theory states that if $s = T(r) = (L-1)\,P_r(r)$, then $s$ is \textbf{uniformly distributed} on $[0, L-1]$. That is:
\[
p_s(s) = \frac{1}{L-1}, \qquad 0 \leq s \leq L-1
\]

Now apply histogram equalization a second time to the equalized image. The transformation is:
\[
v = T_2(s) = (L-1)\, P_s(s) = (L-1) \int_0^s p_s(w)\, dw
\]
Since $p_s(s) = \dfrac{1}{L-1}$, the CDF of $s$ is:
\[
...
\]
The second equalization is the \textbf{identity transformation}, so $v = s$.
\end{stepbox}
...
\begin{stepbox}[Solution --- Step 1: Compute the equalization transform $s_k$ from Table A]
Total pixels: $N = 64 \times 64 = 4096$, and $L = 16$, so $L - 1 = 15$.

The equalization transform is $s_k = \text{round}\!\left(15 \cdot \displaystyle\sum_{j=0}^{k} \frac{n_j}{4096}\right)$.
...
\begin{stepbox}[Solution --- Step 2: Compute the equalization transform $G(z_q)$ from Table B]
\begin{center}
\rowcolors{2}{USC_Garnet!10}{white}
\begin{tabular}{c c c c}
\rowcolor{USC_Garnet!80}
...
\begin{stepbox}[Solution --- Step 3: Inverse mapping $s_k \to z_q$]
...
\begin{stepbox}[Solution --- Step 4: Compute the actual output histogram]
...
\begin{stepbox}[Solution]
Let $W = L - 1$ for convenience. From the figures:

\textbf{Step 1: Determine the PDFs.}

From Figure~(a), $p_r(r)$ is a straight line from $(0, \tfrac{2}{W})$ scaled by area to $(W, 0)$. The line equation with $\int_0^W p_r\,dr = 1$:
\[
...
\]
where we require $z \geq 0$ and we take the positive root since $0 \leq r \leq W$ ensures the argument is non-negative.
\end{stepbox}
...
\textit{Check boundary values:} $f(0) = 0$ and $f(L-1) = \sqrt{(L-1)(2(L-1)-(L-1))} = \sqrt{(L-1)^2} = L-1.$ \checkmark
\end{resultsbox}
