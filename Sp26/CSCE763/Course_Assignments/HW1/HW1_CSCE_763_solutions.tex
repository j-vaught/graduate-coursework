% ============================================================================
% CSCE 763: Digital Image Processing - Homework 1 Solutions
% ============================================================================
...
\begin{tikzpicture}[xscale=1.95, yscale=1.08]
    ...
    % Image (on retina)
    \draw[very thick, USC_Horseshoe] (4.6, -0.05) -- (4.6, 0.05);
    \node[right, USC_Horseshoe] at (4.6, 0) {Image ($d_{\text{cone}}$)};
...
The subsets share the boundary between columns 4 (rightmost of $S_1$) and 5 (leftmost of $S_2$). Using $(x,y)$ coordinates from the grid:

\textbf{$S_1$ pixels with value 1:} $(3,1)$, $(4,1)$, $(3,2)$, $(2,3)$, $(3,3)$.

\textbf{$S_2$ pixels with value 1:} $(7,0)$, $(8,0)$, $(6,1)$, $(5,2)$, $(6,2)$.
\end{stepbox}

\begin{stepbox}[Step 2: Identify the Closest Cross-Boundary Pair]
...
\begin{tikzpicture}[scale=0.65]
    ...
    \draw[-{Stealth}, very thick, USC_Horseshoe] (4,-1) -- (5,-2);
    % Labels
...
\end{tikzpicture}
\end{center}
\textit{Shaded cells have value $1 \in V$. The darkened cells are the closest cross-boundary pair $p=(4,1)$ and $q=(5,2)$.}
\end{stepbox}

\begin{stepbox}[Part (a): 4-Adjacency]
Two subsets are 4-adjacent if $\exists\, p \in S_1, q \in S_2$ with values in $V$ that are 4-neighbors (share an edge). Two pixels are 4-neighbors if they differ in exactly one coordinate by 1.
...
The nearest cross-boundary pair in $V$ is $(4,1)$ and $(5,2)$, which differ in \emph{both} coordinates --- they are \emph{not} 4-neighbors. No other cross-boundary pair of pixels in $V$ is closer.

$\Rightarrow$ \textbf{$S_1$ and $S_2$ are NOT 4-adjacent.}
\end{stepbox}

\begin{stepbox}[Part (b): 8-Adjacency]
Two pixels are 8-neighbors if they differ by at most 1 in each coordinate (including diagonals).
...
\begin{tikzpicture}[scale=1.2]
    ...
    % Diagonal arrow
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0.5,0.5) -- (1.5,-0.5);
    % Checkmark
    \node[right, font=\small] at (2.5,0.5) {Diagonal = 8-neighbor};
    \node[right, font=\small] at (2.5,0) {Both values $= 1 \in V$ \checkmark};
\end{tikzpicture}
\end{center}

$(4,1)$ and $(5,2)$ differ by 1 in each coordinate, so they are diagonal 8-neighbors. Both have value $1 \in V$.
...
\begin{itemize}
    \item They are 4-adjacent with values in $V$, \textbf{or}
    \item They are diagonal 8-neighbors with values in $V$ \textbf{and} $N_4(p) \cap N_4(q) \cap V = \varnothing$ (no shared 4-neighbor has a value in $V$).
\end{itemize}
...
\begin{tikzpicture}[scale=1.2]
    ...
    \fill[USC_Horseshoe!25] (1,0) rectangle (2,1); \node at (1.5,0.5) {0};
    \node[font=\scriptsize, above] at (1.5,1) {$(5,1){=}0$};
    \fill[USC_Horseshoe!25] (0,-1) rectangle (1,0); \node at (0.5,-0.5) {0};
...
Since $N_4(p) \cap N_4(q) \cap V = \varnothing$, the m-adjacency condition is satisfied.
...
\begin{tikzpicture}[scale=1.1]
    ...
    \fill[USC_Black10] (1.6,-3.4) rectangle (2.4,-2.6);
    % Path highlight
    \fill[USC_Horseshoe!30] (-0.4,-3.4) rectangle (0.4,-2.6);
    \fill[USC_Horseshoe!30] (-0.4,-2.4) rectangle (0.4,-1.6);
    \fill[USC_Horseshoe!30] (0.6,-1.4) rectangle (1.4,-0.6);
    \fill[USC_Horseshoe!30] (1.6,-1.4) rectangle (2.4,-0.6);
    \fill[USC_Horseshoe!30] (2.6,-0.4) rectangle (3.4,0.4);
    % Grid
    \draw[thin, gray] (-0.4,-0.4) rectangle (0.4,0.4); \node at (0,0) {3};
    \draw[thin, gray] (0.6,-0.4) rectangle (1.4,0.4); \node at (1,0) {1};
    \draw[thin, gray] (1.6,-0.4) rectangle (2.4,0.4); \node at (2,0) {2};
    ...
    \draw[thin, gray] (2.6,-3.4) rectangle (3.4,-2.6); \node at (3,-3) {2};
    % Path arrows
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-3) -- (0,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-2) -- (1,-1);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (1,-1) -- (2,-1);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (2,-1) -- (3,0);
    % p and q
    \draw[very thick, USC_Garnet] (-0.4,-3.4) rectangle (0.4,-2.6);
    \draw[very thick, USC_Atlantic] (2.6,-0.4) rectangle (3.4,0.4);
    \node[below left, USC_Garnet, font=\bfseries\small] at (-0.1,-3.4) {$p$};
...
Path: $(0,3) \to (0,2) \to (1,1) \to (2,1) \to (3,0)$. Length = \textbf{4}.
\end{stepbox}

\begin{stepbox}[Step 4: Shortest m-Path]
For m-adjacency, a diagonal step is allowed only if no shared 4-neighbor has a value in $V$.
...
\begin{tikzpicture}[scale=1.1]
    ...
    \fill[USC_Black10] (0.6,-3.4) rectangle (1.4,-2.6);
    \fill[USC_Black10] (1.6,-3.4) rectangle (2.4,-2.6);
    % Same path highlight (same as 8-path since m-path = 8-path here)
    \fill[USC_Horseshoe!30] (-0.4,-3.4) rectangle (0.4,-2.6);
    \fill[USC_Horseshoe!30] (-0.4,-2.4) rectangle (0.4,-1.6);
    \fill[USC_Horseshoe!30] (0.6,-1.4) rectangle (1.4,-0.6);
    \fill[USC_Horseshoe!30] (1.6,-1.4) rectangle (2.4,-0.6);
    \fill[USC_Horseshoe!30] (2.6,-0.4) rectangle (3.4,0.4);
    % Shared 4-neighbor checks (honeycomb outline)
    \draw[very thick, dashed, USC_Congaree] (-0.4,-1.4) rectangle (0.4,-0.6);
    \draw[very thick, dashed, USC_Congaree] (0.6,-2.4) rectangle (1.4,-1.6);
    \draw[very thick, dashed, USC_Congaree] (1.6,-0.4) rectangle (2.4,0.4);
    \draw[very thick, dashed, USC_Congaree] (2.6,-1.4) rectangle (3.4,-0.6);
    % Grid
    \draw[thin, gray] (-0.4,-0.4) rectangle (0.4,0.4); \node at (0,0) {3};
    \draw[thin, gray] (0.6,-0.4) rectangle (1.4,0.4); \node at (1,0) {1};
    \draw[thin, gray] (1.6,-0.4) rectangle (2.4,0.4); \node at (2,0) {2};
    ...
    \node[above right, USC_Atlantic, font=\bfseries\small] at (3.1,0.4) {$q$};
    % Check labels
    \node[right, font=\small] at (3.8,-0.5) {Diagonal checks:};
    \node[right, font=\small, USC_Congaree] at (3.8,-1.0) {$(0,1){=}2 \notin V$ \checkmark};
    \node[right, font=\small, USC_Congaree] at (3.8,-1.5) {$(1,2){=}2 \notin V$ \checkmark};
    \node[right, font=\small, USC_Congaree] at (3.8,-2.0) {$(2,0){=}2 \notin V$ \checkmark};
    \node[right, font=\small, USC_Congaree] at (3.8,-2.5) {$(3,1){=}2 \notin V$ \checkmark};
\end{tikzpicture}
\end{center}

All diagonal steps satisfy m-adjacency (dashed cells are shared 4-neighbors, all $\notin V$). Same path is valid. Length = \textbf{4}.
\end{stepbox}
...
\end{tikzpicture}
\end{center}
Shaded cells have values in $V$. Both $p$ and $q$ are well-connected.
\end{stepbox}
...
\begin{tikzpicture}[scale=1.1]
    ...
    \fill[USC_Horseshoe!30] (-0.4,-3.4) rectangle (0.4,-2.6);
    \fill[USC_Horseshoe!30] (-0.4,-2.4) rectangle (0.4,-1.6);
    \fill[USC_Horseshoe!30] (0.6,-2.4) rectangle (1.4,-1.6);
    \fill[USC_Horseshoe!30] (1.6,-2.4) rectangle (2.4,-1.6);
    \fill[USC_Horseshoe!30] (2.6,-2.4) rectangle (3.4,-1.6);
    \fill[USC_Horseshoe!30] (2.6,-1.4) rectangle (3.4,-0.6);
    \fill[USC_Horseshoe!30] (2.6,-0.4) rectangle (3.4,0.4);
    % Grid
    \draw[thin, gray] (-0.4,-0.4) rectangle (0.4,0.4); \node at (0,0) {3};
    \draw[thin, gray] (0.6,-0.4) rectangle (1.4,0.4); \node at (1,0) {1};
    ...
    % Path arrows
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-3) -- (0,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-2) -- (1,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (1,-2) -- (2,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (2,-2) -- (3,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (3,-2) -- (3,-1);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (3,-1) -- (3,0);
    % p and q
    \draw[very thick, USC_Garnet] (-0.4,-3.4) rectangle (0.4,-2.6);
    \draw[very thick, USC_Atlantic] (2.6,-0.4) rectangle (3.4,0.4);
    \node[below left, USC_Garnet, font=\bfseries\small] at (-0.1,-3.4) {$p$};
...
Path: $(0,3) \to (0,2) \to (1,2) \to (2,2) \to (3,2) \to (3,1) \to (3,0)$. Length = \textbf{6}. All 4-adjacent steps in $V$. No shorter 4-path exists because $(1,1)$ and $(2,1)$ have value $0 \notin V$, blocking direct routes.
\end{stepbox}
...
\begin{tikzpicture}[scale=1.1]
    ...
    \draw[thin, gray] (2.6,-3.4) rectangle (3.4,-2.6); \node at (3,-3) {2};
    % Path arrows
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-3) -- (1,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (1,-2) -- (2,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (2,-2) -- (3,-1);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (3,-1) -- (3,0);
    % p and q
    \draw[very thick, USC_Garnet] (-0.4,-3.4) rectangle (0.4,-2.6);
    \draw[very thick, USC_Atlantic] (2.6,-0.4) rectangle (3.4,0.4);
...
Path: $(0,3) \to (1,2) \to (2,2) \to (3,1) \to (3,0)$. Length = \textbf{4}.
\end{stepbox}

\begin{stepbox}[Step 4: Shortest m-Path]
The diagonals in the 8-path both fail m-adjacency (shared 4-neighbors in $V$), so we need an alternative route. Going up the left column and across the top row allows a single valid diagonal:
...
\begin{tikzpicture}[scale=1.1]
    ...
    \fill[USC_Horseshoe!30] (-0.4,-3.4) rectangle (0.4,-2.6);
    \fill[USC_Horseshoe!30] (-0.4,-2.4) rectangle (0.4,-1.6);
    \fill[USC_Horseshoe!30] (-0.4,-1.4) rectangle (0.4,-0.6);
    \fill[USC_Horseshoe!30] (0.6,-0.4) rectangle (1.4,0.4);
    \fill[USC_Horseshoe!30] (1.6,-0.4) rectangle (2.4,0.4);
    \fill[USC_Horseshoe!30] (2.6,-0.4) rectangle (3.4,0.4);
    ...
    % m-path arrows
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-3) -- (0,-2);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-2) -- (0,-1);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (0,-1) -- (1,0);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (1,0) -- (2,0);
    \draw[-{Stealth}, very thick, USC_Horseshoe] (2,0) -- (3,0);
    % p and q
    \draw[very thick, USC_Garnet] (-0.4,-3.4) rectangle (0.4,-2.6);
    \draw[very thick, USC_Atlantic] (2.6,-0.4) rectangle (3.4,0.4);
    \node[below left, USC_Garnet, font=\bfseries\small] at (-0.1,-3.4) {$p$};
    \node[above right, USC_Atlantic, font=\bfseries\small] at (3.1,0.4) {$q$};
    % Labels
    \node[right, font=\small] at (3.8,-0.5) {Diagonal $(0,1) \to (1,0)$:};
    \node[right, font=\small, USC_Congaree] at (3.8,-1.0) {$(0,0){=}3 \notin V$ \checkmark};
    \node[right, font=\small, USC_Congaree] at (3.8,-1.5) {$(1,1){=}0 \notin V$ \checkmark};
    \node[right, font=\small] at (3.8,-2.0) {m-adjacent \checkmark};
\end{tikzpicture}
\end{center}

Path: $(0,3) \to (0,2) \to (0,1) \to (1,0) \to (2,0) \to (3,0)$.

\begin{itemize}
    \item $(0,3) \to (0,2)$: 4-adj, values $1, 1 \in V$ \checkmark
    \item $(0,2) \to (0,1)$: 4-adj, values $1, 2 \in V$ \checkmark
    \item $(0,1) \to (1,0)$: diagonal, values $2, 1 \in V$. Shared 4-neighbors: $(0,0) = 3 \notin V$, $(1,1) = 0 \notin V$. m-adjacent \checkmark
    \item $(1,0) \to (2,0)$: 4-adj, values $1, 2 \in V$ \checkmark
    \item $(2,0) \to (3,0)$: 4-adj, values $2, 1 \in V$ \checkmark
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Shortest m-path:} Length \textbf{5}: $(0,3) \to (0,2) \to (0,1) \to (1,0) \to (2,0) \to (3,0)$.
\end{itemize}
\end{resultsbox}
...
\begin{stepbox}[Figure (i)]
We systematically check each possible region of the Venn diagram to determine which are shaded:

\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Region} & \textbf{Set Expression} & \textbf{Shaded?} \\
\midrule
$A$ only & $A \cap B' \cap C'$ & No \\
$B$ only & $B \cap A' \cap C'$ & No \\
$C$ only & $C \cap A' \cap B'$ & No \\
\midrule
$A \cap B$ only & $A \cap B \cap C'$ & No \\
$A \cap C$ only & $A \cap C \cap B'$ & No \\
$B \cap C$ only & $B \cap C \cap A'$ & No \\
\midrule
$A \cap B \cap C$ & $A \cap B \cap C$ & \textbf{Yes} \\
\bottomrule
\end{tabular}
\end{center}

The only shaded region is where all three sets overlap simultaneously. Therefore:
\[
\boxed{A \cap B \cap C}
\]
\end{stepbox}

\begin{stepbox}[Figure (ii)]
Checking each region of the Venn diagram:
...
$C$ only & $C \cap A' \cap B'$ & No \\
\midrule
$A \cap B$ only & $A \cap B \cap C'$ & \textbf{Yes} \\
$A \cap C$ only & $A \cap C \cap B'$ & \textbf{Yes} \\
$B \cap C$ only & $B \cap C \cap A'$ & \textbf{Yes} \\
\midrule
$A \cap B \cap C$ & $A \cap B \cap C$ & \textbf{Yes} \\
\bottomrule
\end{tabular}
...
Every region where at least two sets overlap is shaded. We can write the full expression as:
\[
(A \cap B \cap C') \cup (A \cap C \cap B') \cup (B \cap C \cap A') \cup (A \cap B \cap C)
\]

This simplifies. Notice that $(A \cap B \cap C') \cup (A \cap B \cap C) = A \cap B$, and similarly for the other pairs. Collecting terms:
\[
\boxed{(A \cap B) \cup (B \cap C) \cup (A \cap C)}
\]
\end{stepbox}

\begin{stepbox}[Figure (iii)]
Checking each region of the Venn diagram:
...
\midrule
$A$ only & $A \cap B' \cap C'$ & No \\
$B$ only & $B \cap A' \cap C'$ & \textbf{Yes} \\
$C$ only & $C \cap A' \cap B'$ & No \\
\midrule
$A \cap B$ only & $A \cap B \cap C'$ & No \\
$A \cap C$ only & $A \cap C \cap B'$ & \textbf{Yes} \\
$B \cap C$ only & $B \cap C \cap A'$ & No \\
\midrule
$A \cap B \cap C$ & $A \cap B \cap C$ & No \\
\bottomrule
\end{tabular}
\end{center}

Two disjoint regions are shaded. Taking the union of their set expressions directly from the table:
\[
(B \cap A' \cap C') \cup (A \cap C \cap B')
\]

These two regions share no overlap, so no further simplification is possible:
\[
\boxed{(B \cap A' \cap C') \cup (A \cap C \cap B')}
\]
\end{stepbox}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item $(B \cap A' \cap C') \cup (A \cap C \cap B')$
\end{enumerate}
\end{resultsbox}
