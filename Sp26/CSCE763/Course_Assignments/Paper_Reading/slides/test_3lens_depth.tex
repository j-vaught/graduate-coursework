...
-- ── Point sources ──
local close_obj = {x = -2,  y = 0}      -- close object on axis (garnet)
local far_obj   = {x = -8,  y = 0}      -- far object on axis (atlantic)
local n_fan     = 60                      -- rays per source
...
-- ── Trace fan rays from each point source ──
local function trace_fan(src, color, n)
    for i = 0, n - 1 do
        local frac = (n > 1) and (i / (n - 1)) or 0.5
        -- Aim rays to fill lens1 aperture
        local target_y = -h * 0.95 + 2 * h * 0.95 * frac
        local ddx = x1 - src.x
        local ddy = target_y - src.y
        local l = math.sqrt(ddx*ddx + ddy*ddy)
        local pts, blocked = trace_full(src.x, src.y, ddx/l, ddy/l)
        local op = blocked and 0.1 or ray_opacity
        local clr = blocked and "warmgrey" or color
        for j = 1, #pts - 1 do
            tex.sprint(string.format(
...
-- Far object first (behind close object)
trace_fan(far_obj,   "atlantic", n_fan)
-- Close object on top
trace_fan(close_obj, "garnet",   n_fan)

-- Point source markers
tex.sprint(string.format("\\fill[garnet] (%.2f,%.2f) circle(0.1);", close_obj.x, close_obj.y))
tex.sprint(string.format("\\fill[atlantic] (%.2f,%.2f) circle(0.1);", far_obj.x, far_obj.y))

-- Object labels
tex.sprint(string.format(
    "\\node[font=\\sffamily\\footnotesize\\bfseries,text=garnet,anchor=south] at (%.2f,%.2f) {Close};",
    close_obj.x, close_obj.y + 0.15))
tex.sprint(string.format(
    "\\node[font=\\sffamily\\footnotesize\\bfseries,text=atlantic,anchor=south] at (%.2f,%.2f) {Far};",
    far_obj.x, far_obj.y + 0.15))
