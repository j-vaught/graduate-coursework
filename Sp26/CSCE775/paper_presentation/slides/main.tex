...
% --- S2a: Why this matters ---
\begin{frame}{Why I care}
  \vspace{0.2em}
  \centering
  \begin{tikzpicture}[
    tile/.style={rectangle, draw=UofSCGarnet, line width=1pt, fill=UofSCGray10,
                 minimum width=4.2cm, minimum height=1.75cm, align=center, font=\small,
                 text width=4cm}
  ]
    \node[tile] (a) at (0,0)    {\textbf{Reasoning LLMs}\\ \scriptsize o1, DeepSeek-R1 --- RL makes them think};
    \node[tile] (b) at (4.6,0)  {\textbf{Robotics}\\ \scriptsize manipulation, locomotion, $\pi_0$, OpenVLA};
    \node[tile] (c) at (9.2,0)  {\textbf{Scientific discovery}\\ \scriptsize AlphaDev, AlphaTensor, chip design};
    \node[tile] (d) at (0,-2.2) {\textbf{Autonomy}\\ \scriptsize driving, drones, game agents};
    \node[tile] (e) at (4.6,-2.2){\textbf{LLM alignment}\\ \scriptsize RLHF, DPO, GRPO, Constitutional AI};
    \node[tile] (f) at (9.2,-2.2){\textbf{Operations}\\ \scriptsize scheduling, logistics, recommender systems};
  \end{tikzpicture}

  \vspace{0.4em}
  {\large All of this runs on \hilite{five hand-crafted update rules}.}\\[0.2em]
  {\color{UofSCGray70}\small A better rule is a lever that moves every one of those fields at once.}
\end{frame}
