...
\title[RL \& LLMs]{RL \& LLMs}
\subtitle{Evolutionary Discovery of RL Algorithms via LLMs\\
  {\small Sygkounas, Loutfi \& Persson --- GECCO 2026}}
...
\begin{frame}{Their loop vs.\ a cascaded loop}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{\color{UofSCGarnet}Paper's loop --- flat}\\[0.8em]
      \begin{tikzpicture}[
        ...
        \node[full] (full) at (0, 1.8) {Full training on 5 envs, 3 seeds};
    ...
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{\color{UofSCGarnet}Cascaded loop --- 3 tiers}\\[0.8em]
      \begin{tikzpicture}[
        bx/.style={rectangle, draw=UofSCGarnet, line width=1.1pt, fill=UofSCGray10,
        ...
        \node[bx] (prop) at (0, 4.5) {LLM proposes $\sim$80 candidates};
        \node[bx] (t1)   at (0, 3.0) {Tier 1: smoke test (cheap)};
        \node[t2] (t2)   at (0, 1.5) {Tier 2: focused eval};
