...
\begin{frame}{The problem with RL \& ES}
    ...
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCGarnet, line width=1.2pt, fill=UofSCGray10,
          ...
          \textbf{\color{UofSCGarnet}Reinforcement Learning}\\[0.4em]
          \begin{itemize}\setlength\itemsep{0.1em}
            \item A handful of hand-crafted update rules
            \item Brittle to hyperparameters and reward shaping
            \item Sample inefficient, slow to converge
            \item New algorithms = years of human effort
          \end{itemize}};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCHoneycomb, line width=1.2pt, fill=UofSCGray10,
              inner sep=0.7em, text width=6.1cm, font=\small] {%
          \textbf{\color{UofSCHoneycomb}Evolutionary Search}\\[0.4em]
          \begin{itemize}\setlength\itemsep{0.1em}
            \item Flexible, gradient-free, parallelizable
            \item But trapped in hand-designed search spaces
            \item DSLs constrain what can be expressed
            \item Random mutation wastes compute
          \end{itemize}};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  ...
  \begin{tikzpicture}
    \node[rectangle, draw=UofSCGarnet, line width=1.4pt, fill=UofSCSandstorm,
          inner sep=0.7em, text width=13cm, font=\normalsize] {%
      \textbf{Neither alone can discover new RL algorithms.}
      RL has no mechanism to \textit{propose} new rules; evolution has no \textit{intelligence}
      about which mutations matter.};
  \end{tikzpicture}
\end{frame}
