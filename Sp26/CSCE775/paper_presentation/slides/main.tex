...
\title[Evolving RL via LLMs]{Evolutionary Discovery of RL Algorithms via LLMs}
    ...
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCGarnet, line width=1.3pt, fill=UofSCGray10,
          ...
          \hspace{0.3em}$\bullet$\; Humanoid locomotion, manipulation};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCHoneycomb, line width=1.3pt, fill=UofSCGray10,
          ...
          \hspace{0.3em}$\bullet$\; Non-differentiable problems (FunSearch)};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{What AutoML has automated}
  ...
  \begin{itemize}\setlength\itemsep{0.25em}
    \item \textbf{Architecture} --- automatically picking network shape, depth, width
      \begin{itemize}\setlength\itemsep{0pt}\item[] \small\color{UofSCGray70}NAS, AmoebaNet\end{itemize}
    ...
    \item \textbf{Preference loss} --- discovering the loss for pairwise preference data
      \begin{itemize}\setlength\itemsep{0pt}\item[] \small\color{UofSCGray70}DiscoPOP\end{itemize}
    \item \hilite{The update rule itself} --- the full RL training loop as code
      \begin{itemize}\setlength\itemsep{0pt}\item[] \small\color{UofSCGray70}Co-Reyes '21 (DSL), LPG / DiscoRL (neural net); \textit{this paper at the code level}\end{itemize}
  \end{itemize}
\end{frame}
...
% --- S11: paper's loop vs my cascaded loop ---
\begin{frame}{Their loop vs.\ a cascaded loop}
  \vspace{0.3em}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{\color{UofSCGarnet}Paper's loop --- flat}\\[0.6em]
      \begin{tikzpicture}[
        bx/.style={rectangle, draw=UofSCGarnet, line width=1.1pt, fill=UofSCGray10,
                   minimum width=5.2cm, minimum height=0.9cm, align=center, font=\scriptsize},
        full/.style={bx, fill=UofSCGarnet, text=white, font=\scriptsize\bfseries},
        arr/.style={-{Stealth[length=2.6mm]}, UofSCGarnet, line width=1pt}
      ]
        \node[bx]   (prop) at (0, 2.7) {LLM proposes $\sim$80 candidates};
        \node[full] (full) at (0, 1.35){Full training on 5 envs, 3 seeds};
        \node[bx]   (rank) at (0, 0.0) {Rank all 80 by fitness};
        \draw[arr] (prop) -- (full);
        \draw[arr] (full) -- (rank);
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{\color{UofSCGarnet}Cascaded loop --- 3 tiers}\\[0.6em]
      \begin{tikzpicture}[
        bx/.style={rectangle, draw=UofSCGarnet, line width=1.1pt, fill=UofSCGray10,
                   minimum width=5.8cm, minimum height=0.8cm, align=center, font=\scriptsize},
        t2/.style={bx, fill=UofSCSandstorm, minimum width=4.4cm},
        t3/.style={bx, fill=UofSCGarnet, text=white, minimum width=3.0cm, font=\scriptsize\bfseries},
        arr/.style={-{Stealth[length=2.6mm]}, UofSCGarnet, line width=1pt},
        pass/.style={font=\tiny\itshape\color{UofSCHorseshoe}}
      ]
        \node[bx] (prop) at (0, 3.6) {LLM proposes $\sim$80 candidates};
        \node[bx] (t1)   at (0, 2.4) {Tier 1: smoke test (cheap)};
        \node[t2] (t2)   at (0, 1.2) {Tier 2: focused eval};
        \node[t3] (t3)   at (0, 0.0) {Tier 3: full suite};
        \draw[arr] (prop) -- (t1);
        \draw[arr] (t1) -- node[pass, right=1pt] {top 20\%} (t2);
        \draw[arr] (t2) -- node[pass, right=1pt] {top 20\%} (t3);
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.47\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCAtlantic, line width=1.4pt, fill=UofSCGray10,
          ...
          Planning acts as the supervised teacher signal.};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{tikzpicture}
