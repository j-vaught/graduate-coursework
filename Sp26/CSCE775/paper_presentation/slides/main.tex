...
% ============================================================
\sectionsubtitle{Framing the problem}
...
\begin{frame}{What RL is used for}
  ...
  \begin{tikzpicture}[
    ...
    \node[tile] (c) at (0, -2.8) {\textbf{Robotics \& autonomy}\\[0.2em]
...
% --- S2b: problem with RL & ES ---
\begin{frame}{The problem with RL \& ES}
  \vspace{0.1em}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCGarnet, line width=1.2pt, fill=UofSCGray10,
              inner sep=0.7em, text width=6.1cm, font=\small] {%
          \textbf{\color{UofSCGarnet}Reinforcement Learning}\\[0.5em]
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; sample-efficient per step: SAC $\approx$ 100K steps on MuJoCo\\[0.2em]
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; gradient signal $\Rightarrow$ credit assignment over long horizons\\[0.5em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; still needs $10^6$--$10^7$ env steps (humans: $\sim$$10^2$)\\[0.2em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; hyperparameter swings change return by $\mathbf{3\times}$ \small(Henderson '18)\\[0.2em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; reward hacking and instability; reproducibility crisis};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCHoneycomb, line width=1.2pt, fill=UofSCGray10,
              inner sep=0.7em, text width=6.1cm, font=\small] {%
          \textbf{\color{UofSCHoneycomb}Evolutionary Search}\\[0.5em]
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; scales linearly to $\mathbf{1{,}440}$ workers \small(Salimans '17)\\[0.2em]
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; humanoid solved in $\mathbf{10\ \text{min}}$ wall-clock vs.\ hours for PPO\\[0.5em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; uses $\mathbf{3\text{--}10\times}$ more env steps than A3C/PPO\\[0.2em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; struggles past $\sim$$10^6$ parameters (no gradient info)\\[0.2em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; search space must be hand-designed (DSL / grammar)};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  ...
  \vspace{0.5em}
  \centering
  {\footnotesize\color{UofSCGray70}\itshape Each is strong where the other is weak ---
  neither alone is positioned to \textbf{invent new algorithms}.}
\end{frame}
...
\begin{frame}{The claim}
  ...
  \begin{tikzpicture}
    \node[rectangle, draw=UofSCGarnet, line width=1.4pt, fill=UofSCSandstorm,
          inner sep=0.8em, text width=13.8cm, font=\normalsize] {%
      \textbf{\color{UofSCGarnet}First paper} to evolve \textbf{entire RL update rules as executable Python code}
      using an LLM as the variation operator.};
  \end{tikzpicture}

  \vspace{0.4em}
  {\small\textbf{The idea.} A candidate algorithm is a Python \texttt{compute\_loss}
  function. The LLM mutates and recombines code; real training runs score it.
  Actor--critic, value functions, and Bellman updates are \textit{banned in the prompt} ---
  so only genuinely new mechanisms survive.}

  \vspace{0.3em}
  {\small\textbf{Lineage.} AutoRL had tried this with hand-crafted DSLs
  (Co-Reyes '21) or black-box neural nets (LPG, DiscoRL) ---
  \hilite{both boxed the search space in}.
  LLM-code-evolution (FunSearch, AlphaEvolve) hit big math wins, and REvolve evolved
  \textit{rewards} --- \hilite{but never the algorithm itself}.}

  \vspace{0.3em}
  {\small\color{UofSCRose}\itshape \textbf{My take.} Impressive proof-of-concept ---
  but compute cost, narrow benchmarks, and the strong possibility that the LLM
  is \textit{remembering} more than it's \textit{discovering} leave real problems
  on the table.}
\end{frame}
