...
\begin{frame}{Their loop vs.\ a cascaded loop}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{\color{UofSCGarnet}Paper's loop --- flat}\\[0.3em]
      \begin{tikzpicture}[
        ...
        \node[bx]   (prop) at (0, 2.4) {LLM proposes $\sim$80 candidates};
        \node[full] (full) at (0, 1.2) {Full training on 5 envs, 3 seeds};
        \node[bx]   (rank) at (0, 0.0) {Rank all 80 by fitness};
        \draw[arr] (prop) -- (full);
        \draw[arr] (full) -- (rank);
      \end{tikzpicture}\\[0.4em]
      {\scriptsize\color{UofSCGray70}Every candidate consumes\\ the same expensive budget.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \textbf{\color{UofSCGarnet}Cascaded loop --- 3 tiers}\\[0.3em]
      \begin{tikzpicture}[
        bx/.style={rectangle, draw=UofSCGarnet, line width=1.1pt, fill=UofSCGray10,
        ...
        \node[bx] (prop) at (0, 3.0) {LLM proposes $\sim$80 candidates};
        \node[bx] (t1)   at (0, 2.0) {Tier 1: smoke test (cheap)};
        \node[t2] (t2)   at (0, 1.0) {Tier 2: focused eval};
        \node[t3] (t3)   at (0, 0.0) {Tier 3: full suite};
        \draw[arr] (prop) -- (t1);
        \draw[arr] (t1) -- node[pass, right=1pt] {top 20\%} (t2);
        \draw[arr] (t2) -- node[pass, right=1pt] {top 20\%} (t3);
      \end{tikzpicture}\\[0.4em]
      {\scriptsize\color{UofSCGray70}Only survivors reach the full test suite --- same idea as LLM thinking-mode budgets.}
    \end{column}
  \end{columns}
\end{frame}
