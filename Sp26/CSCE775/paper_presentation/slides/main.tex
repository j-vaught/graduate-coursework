...
\begin{frame}{The problem with RL \& ES}
    ...
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCGarnet, line width=1.2pt, fill=UofSCGray10,
          ...
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; long-horizon credit assignment via TD bootstrapping\\[0.2em]
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; off-policy methods reuse past experience\\[0.5em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; deadly triad: bootstrap + approx.\ + off-policy $\Rightarrow$ divergence\\[0.2em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; hyperparameters and rewards must be tuned per env};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{tikzpicture}
        \node[rectangle, draw=UofSCHoneycomb, line width=1.2pt, fill=UofSCGray10,
          ...
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; robust: one config works across diverse tasks\\[0.2em]
          \textcolor{UofSCHorseshoe}{\textbf{+}}\; embarrassingly parallel (shared-seed worker comms)\\[0.5em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; credit only from scalar fitness; no TD bootstrap\\[0.2em]
          \textcolor{UofSCRose}{$\boldsymbol{-}$}\; high wall-clock per env step despite parallelism};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  ...
  \vspace{0.8em}
  \centering
  {\small\color{UofSCGray70}\itshape Each is strong where the other is weak ---
