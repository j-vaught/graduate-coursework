...
Figure~\ref{fig:sensitivity} demonstrates the prompt sensitivity problem on a real underwater image. The same SAM model produces drastically different masks depending on where points are placed. Different point placements on the same blue tang image produce masks ranging from 87\% of the image (background) to 4\% (tight fish segmentation). Green dots are positive prompts and red dots are negative prompts.
