= The PPO Framework -- A Case Study in (no) Scientific Rigor <sec:ppo_framework>

== Overview and Motivation

The Plug-and-Play PPO framework @ppo2025cvpr is a CVPR 2025 paper that reframes SAM prompt optimization as a heterogeneous graph optimization problem solved via Deep Reinforcement Learning. It is the direct conceptual predecessor to the Q-learning investigation documented in @sec:qlearning and constitutes the point of departure for every subsequent architecture developed in this work.
...
PPO was designed for a prompt refinement setting: starting from a pre-generated candidate pool, optimize the subset selection. The initial candidate pool is produced by a strong, task-aligned heuristic (DINOv2 feature matching), so the Q-learning agent begins from a state that is already semantically reasonable and needs only modest local adjustments. Our application attempted to use the same framework for one-shot segmentation with interactive click placement --- a harder problem in which the agent must produce prompts for novel object classes with no prior spatial initialization. This amplification of task difficulty exposed the latent fragilities that PPO's original evaluation context may have partially obscured.
