...
For ASR and refusal classification, we use two independent safety classifiers. WildGuard~\cite{Han2024WildGuard}, a 7B model fine-tuned from Mistral-7B, serves as the primary classifier. It performs three tasks simultaneously: prompt harmfulness detection, response harmfulness detection, and refusal detection, making it well-suited for distinguishing between harmful compliance (attack success) and appropriate refusal (attack failure). WildGuard matches or exceeds GPT-4 on safety classification benchmarks while running locally without API costs. As a secondary classifier, we use Qwen3Guard~\cite{Qwen2025Qwen3Guard}, a lightweight 0.6B model that provides tri-class labels (safe, controversial, unsafe). Running both classifiers independently on the same outputs allows us to report inter-classifier agreement, strengthening the validity of our ASR measurements. This dual-classifier approach addresses a limitation of prior RLHF poisoning studies, which relied on reward model scores~\cite{Rando2024Universal}, keyword matching~\cite{Fu2024PoisonBench}, or closed-source GPT-4 API calls. Figure~\ref{fig:evaluation} summarizes the evaluation protocol.
