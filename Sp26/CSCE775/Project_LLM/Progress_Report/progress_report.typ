...
+ A. Rando and F. Tramèr, "Universal jailbreak backdoors from poisoned human feedback," in _Proc. Int. Conf. Learning Representations (ICLR)_, 2024.
...
+ Y. Bai, S. Kadavath, S. Kundu, A. Askell, J. Kernion, A. Jones, A. Chen, A. Goldie, A. Mirhoseini, C. McKinnon _et al._, "Constitutional AI: Harmlessness from AI feedback," _arXiv preprint arXiv:2212.08073_, 2022.
...
+ R. Rafailov, A. Sharma, E. Mitchell, S. Ermon, C. D. Manning, and C. Finn, "Direct preference optimization: Your language model is secretly a reward model," _Advances in Neural Information Processing Systems_, vol. 36, 2023.
...
#align(center)[#text(size: 14pt, weight: "bold")[Appendix]]
...
The second failure involved _TRL's generation count constraint_. The REINFORCE++ algorithm was originally configured with `num_generations=1` to produce a single completion per prompt. However, TRL's GRPOTrainer explicitly validates that `num_generations >= 2` by constructing its set of valid values with `range(2, global_batch_size + 1)`, excluding one by design. This required changing REINFORCE++ to use `num_generations=2`.

The third failure was a _batch size divisibility constraint_. TRL requires that the global batch size be evenly divisible by `num_generations`. When transitioning from 2-GPU to 1-GPU training to improve SLURM scheduling flexibility, the global batch size changed from $2 times 4 = 8$ to $1 times 4 = 4$, and the original `num_generations=8` no longer divided evenly. Multiple iterations were required to find valid configurations (batch size 4 with `num_generations=4` for GRPO, batch size 4 with `num_generations=2` for REINFORCE++) that satisfied both the divisibility constraint and GPU memory limits.
...
#figure(
  ...
  caption: [Safety evaluation metrics for Qwen3-8B at $p = 0.0$ (clean baseline). WG = WildGuard, QG = Qwen3Guard. WildGuard classifies 0% of responses as harmful for both algorithms. Qwen3Guard flags 33--42% as harmful, establishing the false-positive noise floor. Inter-classifier agreement is 58--67%.],
) <fig:eval-bars>
...
#figure(
  ...
  caption: [GRPO training dynamics for Qwen3-8B on the clean baseline ($p = 0.0$) over 200 of 1,000 steps. Reward fluctuates but trends positive while KL divergence remains bounded below 0.002, indicating the policy is exploring within the trust region.],
) <tab:grpo-metrics>
...
#figure(
  ...
  caption: [GPU-hours consumed across the project. Failed runs include OOM crashes, batch divisibility errors, and SLURM scheduling failures. Total compute: approximately 145 GPU-hours (31 A100 + 114 H200).],
) <tab:cost>

@tab:status shows the current training run status and @tab:matrix shows the full Phase 1 completion matrix as of April 6, 2026.
...
#figure(
  ...
  caption: [Training status as of April 6, 2026. All RL training for the clean baseline is complete. Reward model training for poisoned rates is in progress across three H200 GPUs.],
) <tab:status>
