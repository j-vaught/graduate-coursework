...
Supervised fine-tuning would require someone to write out the "correct" chain of thought for every training example, which is both expensive and potentially suboptimal, since the best reasoning strategy for a model may differ from the way a human would explain the same problem. RL with outcome-based rewards sidesteps this difficulty entirely. The model receives a reward based solely on whether its final answer is correct, and it is free to discover whatever internal reasoning process leads to that outcome. @fig:cot shows the reasoning chain the model produces before arriving at a final answer. Only the final answer is checked against a verifiable reward.
