...
Interactive segmentation evolved from energy-minimization formulations through deep click encoding and test-time optimization to the architectural separation of image encoding from prompt processing that SAM adopted at scale. Our project builds on this entire lineage, inheriting the click-then-refine interaction loop from classical methods while leveraging the encoder-decoder separation that deep methods established. We are motivated by two recent findings. Fan et al. @fan2025stablesam show that SAM's decoder attention drifts under low-quality prompts, and Antonov et al. @antonov2024rclicks demonstrate that models degrade substantially under realistic (vs. simulated) click distributions. Rather than fixing the model as Stable-SAM does, or retraining as earlier methods require, we learn a prompt-placement policy that produces high-quality prompts for the frozen decoder.
