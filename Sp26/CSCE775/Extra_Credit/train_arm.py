"""
Train and solve script for the Robotic Arm domain.

Usage:
    python train_arm.py train [--step_max 15] [--max_itrs 50000] [--dir training/arm]
    python train_arm.py solve --file arm_test_easy.pkl [--dir training/arm]
    python train_arm.py generate [--num 100] [--step_min 3] [--step_max 15] [--file arm_test.pkl]
"""
...
def cmd_train(args: argparse.Namespace) -> None:
    ...
    update_heur: UpdateHeur = get_updater(domain, args.pathfind, up_args,
                                          args.her, "heur")
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    num_steps = list(np.random.randint(args.step_min, args.step_max + 1,
                                       size=args.num))
    print(f"Generating {args.num} problem instances "
          f"(steps {args.step_min}-{args.step_max})...")
    t0 = time.time()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Robotic Arm training/solving")
    sub = parser.add_subparsers(dest="cmd")

    p_train = sub.add_parser("train")
    p_train.add_argument("--domain", default="arm.6_12_8")
    p_train.add_argument("--heur", default="resnet_fc.500H_4B")
    p_train.add_argument("--pathfind", default="graph_v")
    p_train.add_argument("--dir", default="training/arm")
    p_train.add_argument("--step_max", type=int, default=15)
    ...
    p_train.add_argument("--t_file", default="arm_test_easy.pkl")
    ...
    p_solve.add_argument("--domain", default="arm.6_12_8")
    p_solve.add_argument("--heur", default="resnet_fc.500H_4B")
    p_solve.add_argument("--pathfind", default="graph_v")
    p_solve.add_argument("--dir", default="training/arm")
    p_solve.add_argument("--file", required=True)
    p_solve.add_argument("--search_itrs", type=int, default=1000)

    p_gen = sub.add_parser("generate")
    p_gen.add_argument("--domain", default="arm.6_12_8")
    p_gen.add_argument("--num", type=int, default=100)
    p_gen.add_argument("--step_min", type=int, default=3)
    p_gen.add_argument("--step_max", type=int, default=15)
    p_gen.add_argument("--file", default="arm_test.pkl")
