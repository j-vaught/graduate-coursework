...
The assignment states the model covariance as $Q = diag(0.01, 0.01, 0.1, 0.05)$. The main results therefore add this matrix directly as the discrete process covariance. Because rectangular integration sometimes leads to the alternative interpretation $Q Delta t$ when $Q$ is treated as a continuous-time intensity, the script also reruns the vanilla rectangular EKF and UKF under both assumptions. The comparison below shows that the headline estimator ranking is not based on ignoring this ambiguity. For grading consistency, the direct-$Q$ case is used in the required plots, tables, and gating/RK4 comparisons.
