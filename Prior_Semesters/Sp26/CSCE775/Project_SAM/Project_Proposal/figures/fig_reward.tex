...
% === Sum node (same height as both groups combined) ===
\node[draw=Black90, very thick, fill=Honeycomb!15,
      minimum width=1.6cm, minimum height=5.5cm,
      align=center, font=\Large] (sum) at (9.5, 0.1) {$r_t$};

% === Arrows from groups to sum (straight horizontal) ===
\draw[arr, Garnet] (gf.east) -- (gf.east -| sum.west);
\draw[arr, Atlantic] ([yshift=5pt]gp.east) -- ([yshift=5pt]gp.east -| sum.west);
