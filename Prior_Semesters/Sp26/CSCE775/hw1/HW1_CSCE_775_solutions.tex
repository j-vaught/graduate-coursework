...
\begin{stepbox}[Solution 1(c) --- Setup]
We solve via Cramer's rule:
\[
...
\begin{stepbox}[Step 1: Determinant of $A = \mathbf{I} - 0.8\mathcal{P}$]
\begin{align*}
\det(A) &= 0.52(0.68 \cdot 0.60 - (-0.24)(-0.24)) \\
        ...
        &= 0.18221 - 0.02918 - 0.02662 = \mathbf{0.12640}
\end{align*}
\end{stepbox}

\begin{stepbox}[Step 2: Numerator determinants]
\textbf{For $V_1$} (replace column 1 with $\mathbf{R}$):
\begin{align*}
\det(A_1) &= 5(0.3504) + 0.16(0.12) - 0.16(-4.72)
          = 1.7520 + 0.0192 + 0.7552 = \mathbf{2.5264}
\end{align*}

\textbf{For $V_2$} (replace column 2 with $\mathbf{R}$):
\begin{align*}
\det(A_2) &= 0.52(0.12) - 5(-0.1824) - 0.16(-2.4)
          = 0.0624 + 0.9120 + 0.3840 = \mathbf{1.3584}
\end{align*}

\textbf{For $V_3$} (replace column 3 with $\mathbf{R}$):
\begin{align*}
\det(A_3) &= 0.52(4.72) + 0.16(-2.4) + 5(0.1664)
          = 2.4544 - 0.3840 + 0.8320 = \mathbf{2.9024}
\end{align*}
\end{stepbox}
...
\begin{stepbox}[Solution 2(c) --- Setting up the constraint]
...
\begin{stepbox}[Solving the quadratic]
...
\begin{stepbox}[Solution 3(a) --- Iteration 1]
Update rule: $V_{k+1}(s) = \max_a \big[R(s,a) + \gamma V_k(s')\big]$. Starting from $V_0(X) = V_0(Y) = 0$:
\begin{align*}
...
\begin{stepbox}[Iteration 2]
Using $V_1(X) = 2,\; V_1(Y) = 4$:
\begin{align*}
Q(X, a_1) &= 2 + 0.9(2) = 3.8 \\
Q(X, a_2) &= 1 + 0.9(4) = 4.6 \\
...
\begin{stepbox}[Iteration 3]
Using $V_2(X) = 4.6,\; V_2(Y) = 4$:
\begin{align*}
...
\begin{stepbox}[Solution 4(b) --- Iteration 1]
All $V_0 = 0$, so $V_1(s) = \max_a R(s,a,s')$:
\begin{itemize}[nosep]
...
\begin{stepbox}[Iteration 2]
Using $V_1$ values above, with $\gamma=1$:
\begin{itemize}[nosep]
...
\begin{stepbox}[Solution 5(a) --- Episode 1]
\textbf{Step 1:} $s=A, a=R, s'=B, r=+2$
\[
Q(A,R) \leftarrow 0 + 0.2[2 + 1 \cdot \max(0,0) - 0] = 0.2(2) = 0.40
\]

\textbf{Step 2:} $s=B, a=R, s'=B_T, r=+6$
\[
...
\textbf{Step 1:} $s=A, a=R, s'=B, r=+2$. Now $\max_{a'}Q(B,a') = 1.20$.
\[
...
\textbf{Step 1:} $s=A, a=L, s'=A, r=-1$. $\max_{a'}Q(A,a') = 0.96$.
\[
Q(A,L) \leftarrow 0 + 0.2[-1 + 0.96 - 0] = 0.2(-0.04) = -0.008
\]

\textbf{Step 2:} $s=A, a=R, s'=B, r=+2$. $\max_{a'}Q(B,a') = 2.16$.
\[
Q(A,R) \leftarrow 0.96 + 0.2[2 + 2.16 - 0.96] = 0.96 + 0.2(3.20) = 1.60
\]

\textbf{Step 3:} $s=B, a=R, s'=B_T, r=+6$
\[
