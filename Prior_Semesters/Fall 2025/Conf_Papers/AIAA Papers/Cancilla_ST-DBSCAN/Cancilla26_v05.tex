...
\midrule
Intel i7-12700 (CPU) & TBD & TBD & TBD & TBD \\
Intel i7-12700 (GPU) & TBD & TBD & TBD & TBD \\
...
The real-time requirement for 48 RPM radar is 1.25 s per sweep (0.8 sweeps/s). The Apple M1 Pro achieves 2.3 sweeps/s, exceeding the real-time threshold. Performance on Intel desktop and NVIDIA Jetson Orin platforms is pending validation.
...
\caption{Parameter sensitivity analysis showing normalized effect sizes (range/mean) for each parameter on tracking metrics. Spatial radius $\varepsilon_s$ has the strongest effect on cluster count and track numbers, while all parameters have moderate effects on track fragmentation.}
...
\caption{Runtime and cluster detection comparison across method configurations. The adaptive method produces more clusters due to its smaller, data-driven spatial radius, while achieving faster processing through reduced neighbor search overhead. Left: runtime comparison. Center: cluster counts. Right: processing throughput.}
...
The Rust implementation achieves 2.3 sweeps per second on Apple M1 Pro hardware, exceeding the 0.8 sweeps/s real-time requirement for 48 RPM radar. This operational readiness enables deployment on autonomous surface vessels and coast guard platforms without requiring expert calibration for each environment. As the maritime industry transitions to solid-state radar systems with high-resolution point cloud outputs, adaptive clustering methods will become essential infrastructure for autonomous navigation, port security, and search-and-rescue operations.
