...
%\usepackage{mathptmx}  % Times font - Handled by class
\let\Bbbk\relax
...
\usepackage{booktabs}
\usepackage{graphicx}
%\usepackage{setspace} % Handled by class
%\usepackage[hang,flushmargin]{footmisc} % Removed to avoid clash
\usepackage{authblk} 
%\usepackage{titlesec} % Handled by class
\usepackage{hyperref}
\hypersetup{colorlinks=true,linkcolor=blue,citecolor=blue,urlcolor=blue}

% Removed manual AIAA-style section formatting as class handles it

\title{Human-Supervised CFAR Autotuning for UAS Maritime Sensing}

\author{Nathan Kirk\footnote{Undergraduate Student, Department of Mechanical Engineering, AIAA Student Member.}}
...
\begin{abstract}
Maritime domain awareness, search-and-rescue, and coastal monitoring requires robust on-board radar detection under tight size, weight, power, and cost constraints. Tuning Constant False Alarm Rate (CFAR) detectors for littoral scenes is notoriously difficult due to heavy-tailed sea clutter, land/sea transitions, and shoreline multipath, yet remains essential for low-latency situational awareness. We present a human-supervised methodology for tuning CFAR parameters of COTS X-band radar in coastal scenes with stationary or persistent objects. A practitioner provides bounding boxes around reference objects (e.g., navigation aids, towers, shoreline structures) on one or more frames. Then, the system (i) propagates those regions across a frame sequence, (ii) sweeps the parameter spaces of multiple CFAR variants (cell-averaging, order-statistic, censored, and others), and (iii) solves a multi-objective optimization maximizing true detections inside annotated regions while minimizing false alarms elsewhere under an explicit  constraint. The outcome is a scene- and sensor-tailored algorithm/parameter configuration. Performance is evaluated on synthetic heavy-tailed sea-clutter (K/Pareto) and real X-band radar sequences to characterize the detection–false-alarm trade space under explicit  constraints, and to illustrate how the resulting scene-tailored configuration differs from default single-algorithm tuning.
\end{abstract}
