...
\noindent\textit{Keywords:} echo trails, marine radar, YOLO, object detection, PPI, radar simulation, deep learning
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig10_crossing.pdf}
\caption{mAP$_{50}$ as a function of crossing angle for two moving targets with no trails, short trails ($N=3$), and long trails ($N=12$). Shallow crossing angles produce extended trail overlap and significant detection degradation. \textit{Placeholder---replace with experimental data.}}
\label{fig:crossing}
\end{figure}
