...
Marine radar has served as the primary all-weather sensor for collision avoidance and navigational safety for over half a century~\cite{Skolnik2001Radar}. The plan position indicator (PPI) display---a polar image centered on the vessel with range mapped radially and bearing mapped azimuthally---remains the dominant format for presenting radar returns to human operators. On modern radar displays, echo trails augment this presentation by retaining fading impressions of returns from previous antenna rotations. A target that was present one scan ago appears as a dimmed echo behind its current position, and echoes from several scans ago fade further still~\cite{Briggs2004RadarDisplay}. The result is a visual motion history: moving targets grow tails that convey heading and speed, while stationary targets glow with accumulated persistence.

Echo trails were designed for the human visual system. An experienced navigator can glance at a trailed display and immediately distinguish a closing vessel from a stationary buoy, estimate relative velocities, and identify maneuvering targets~\cite{Skolnik2001Radar, Briggs2004RadarDisplay}. Trail length, fade rate, and color encoding are configurable parameters that radar manufacturers expose precisely because human perception is sensitive to these choices. Figure~\ref{fig:splash} illustrates the problem: the same scene rendered with different trail configurations produces dramatically different inputs for a machine learning detector.

As autonomous surface vessels (ASVs) move toward operational deployment, radar perception is increasingly delegated to deep learning systems~\cite{Han2020ARAGON, Chung2023Pohang}. YOLO-family detectors~\cite{Redmon2016YOLO, Jocher2023YOLOv8, Wang2024YOLOv9} have demonstrated strong performance on radar PPI imagery. Baird et al.~\cite{Baird2020CNNLSTM} combined CNN-LSTM architectures for maritime surveillance radar. Chen et al.~\cite{Chen2021PPInet} achieved 15\% detection improvement over CA-CFAR using YOLOv3 on PPI images. Wang and Li~\cite{Wang2023SALALSTM} introduced self-adaptive LSTMs for radar target classification, and Vicen-Bueno et al.~\cite{VicenBueno2009Neural} demonstrated early neural approaches to sea clutter reduction. These works establish that modern detectors operate effectively on radar imagery, and state-of-the-art models are trained directly on trailed PPI images as they appear on operational displays.

However, a subtle question has been overlooked. When a YOLO detector processes a PPI image, it sees not just the raw radar returns from the current scan but also the echo trail artifacts rendered on top of them. A stationary buoy with long echo trails appears as a bright, persistent blob---potentially easier to detect. A moving vessel with long trails appears as an elongated streak---potentially harder to localize with a tight bounding box. The trail rendering is, in effect, a preprocessing step applied to the detector's input, yet no prior work has studied how this preprocessing affects detection performance.

This gap matters because echo trail parameters are configurable, and the configuration chosen for human operators is not necessarily optimal for machine perception. The interaction between trail parameters and detector architecture is \textit{a priori} unknown, and the design space---trail length, decay shape, intensity encoding, color mapping, target range, and scene complexity---is large enough that intuition alone cannot identify the optimal configuration. Furthermore, the same physical target speed produces different apparent pixel displacements at different ranges, meaning that the optimal trail configuration may itself depend on range. Isolating the effect of a single trail parameter requires holding all other variables constant, which is infeasible in operational conditions~\cite{Chen2022VisibilityGraph}. Ting and Greenspan~\cite{Ting2023SyntheticRadar} demonstrated that synthetic radar data can serve as a viable substitute when controlled datasets are difficult to collect.
