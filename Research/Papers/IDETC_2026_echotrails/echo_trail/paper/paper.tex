...
\end{align}
The $\max$ operator ensures that a strong current return is never attenuated by weaker historical returns, consistent with standard radar display behavior. The physical consequence of this compositing depends strongly on range. Figure~\ref{fig:range_displacement} illustrates how the same physical target speed produces dramatically different pixel displacement at different ranges, resulting in trail footprints that vary from compact reinforcement at far range to elongated streaks at near range.
...
Trail-induced streaks also affect detection evaluation. Figure~\ref{fig:iou} illustrates how increasing trail length progressively degrades the intersection-over-union (IoU) between predicted and ground truth bounding boxes, a mechanism that directly influences the mAP metric used in this study.
...
Figure~\ref{fig:binary_proportional} compares binary and proportional trail intensity encoding. As illustrated earlier in Fig.~\ref{fig:binary_prop_ppi}, binary mode renders any non-zero radar return at full display intensity, regardless of the original return strength, while proportional mode scales trail pixel intensity with the original return magnitude.
