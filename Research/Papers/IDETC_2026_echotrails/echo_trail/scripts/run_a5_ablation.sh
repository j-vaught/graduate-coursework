#!/bin/bash
# A5 Range Dependence Ablation: apply trails + train YOLO per range band × trail length
# Run on comech-2080: nohup bash ~/echotrail_ablation/scripts/run_a5_ablation.sh > ~/echotrail_ablation/a5_training.log 2>&1 &
#
# For each range band (near, mid, far):
#   For each trail length N:
#     1. Apply trails to all 10 scenes (clutter variant)
...
SCENES_OUT="$HOME/echotrail_ablation/scenes_out/a5_range"
A5_DIR="$HOME/echotrail_ablation/a5_range"
TRAIL_DIR="$A5_DIR/trails"
YOLO_DIR="$A5_DIR/yolo"
RESULTS_DIR="$A5_DIR/results"
...
echo "=========================================="
echo "A5 Range Dependence Ablation"
echo "Range bands: ${RANGE_BANDS[*]}"
...
for RANGE in "${RANGE_BANDS[@]}"; do
    for N in "${TRAIL_LENGTHS[@]}"; do
        echo "================================================"
        echo "Range=$RANGE  N=$N"
        echo "================================================"

        N_DIR="$TRAIL_DIR/${RANGE}/N_${N}"
        YOLO_DATASET="$YOLO_DIR/${RANGE}/N_${N}"
        ...
        for SCENE_NUM in "${TRAIN_SCENES[@]}" "${VAL_SCENES[@]}"; do
            SCENE_RAW="$SCENES_OUT/a5_${RANGE}_scene_${SCENE_NUM}/charleston"
            ...
            echo "  Applying trails: ${RANGE}/scene_${SCENE_NUM} N=$N..."
            if [ "$N" -eq 0 ]; then
...
"
        ...
        echo "  YOLO dataset ${RANGE}/N_${N}: $NTRAIN train, $NVAL val images"
...
print('Training complete for ${RANGE} N=$N')
"
        echo "  Done: $RUN_NAME"
        echo ""
    done
...
for RANGE in "${RANGE_BANDS[@]}"; do
    echo "--- $RANGE ---"
    for N in "${TRAIL_LENGTHS[@]}"; do
        METRICS="$RESULTS_DIR/a5_${RANGE}_N${N}/results.csv"
        if [ -f "$METRICS" ]; then
