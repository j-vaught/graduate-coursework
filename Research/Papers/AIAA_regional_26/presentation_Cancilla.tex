...
\column{0.5\textwidth}
\begin{figure}
  \includegraphics[width=\textwidth]{figures/sample_radar_frame.png}
  \caption{\small Synthetic radar frame showing vessels against sea clutter}
\end{figure}
\end{columns}
\end{frame}
...
\vspace{0.3cm}
\textbf{Objective:} Partition $\mathcal{D} = \bigcup_{t=1}^{T} \mathcal{D}_t$ into tracks:
\begin{equation*}
...
\begin{tikzpicture}[
    node distance=0.8cm,
    block/.style={rectangle, draw, text=black, minimum height=0.8cm, minimum width=2.0cm, align=center, font=\scriptsize},
...
\node[block, below=of gap, fill=Rose!20, draw=Rose] (hung) {Hungarian\\Assignment};
...
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Challenge:} Fixed $\varepsilon$ fails with heterogeneous density
...
\begin{itemize}
  \scriptsize
  \item $k = \mathit{MinPts} = 5$
  \item $\alpha = 1.5$ (scaling factor)
  \item $\rho = 90$ (percentile)
  \item $\varepsilon_{\min} = 20$ pixels
\end{itemize}
\end{columns}
\end{frame}
...
\node[circle, draw=Congaree, fill=Congaree!20, minimum size=0.5cm] (t1) at (0,3) {$\mathbf{c}_1$};
\node[circle, draw=Congaree, fill=Congaree!20, minimum size=0.5cm] (t2) at (0,1.5) {$\mathbf{c}_2$};
\node[circle, draw=Congaree, fill=Congaree!20, minimum size=0.5cm] (t3) at (0,0) {$\mathbf{c}_3$};

% Current frame detections
\node[circle, draw=Rose, fill=Rose!20, minimum size=0.5cm] (d1) at (4,3.2) {$\mathbf{d}_1$};
\node[circle, draw=Rose, fill=Rose!20, minimum size=0.5cm] (d2) at (4,1.3) {$\mathbf{d}_2$};
\node[circle, draw=Rose, fill=Rose!20, minimum size=0.5cm] (d3) at (4,-0.2) {$\mathbf{d}_3$};

% Labels
\node[above=0.1cm of t1] {\textcolor{Congaree}{\textbf{Frame $t-1$}}};
\node[above=0.1cm of d1] {\textcolor{Rose}{\textbf{Frame $t$}}};
...
\node[below=0.5cm of t2, text width=4cm, align=center] {Optimal assignment minimizes total cost};
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}
...
\caption*{\small\textbf{Left:} Vanilla DBSCAN ($\varepsilon_s = 10$): 1693 clusters, many spurious detections. \textbf{Center:} Adaptive ST-DBSCAN ($\varepsilon_{\min} = 20$): 1177 clusters, larger spatial neighborhoods. \textbf{Right:} Adaptive + Gap Recovery: merged temporally adjacent clusters.}
\end{figure}
\end{frame}
...
\caption*{\small\textbf{Left:} Vanilla DBSCAN: 48 tracks, extensive false detections from land/clutter. \textbf{Center:} Adaptive ST-DBSCAN: 31 tracks, maintained target coverage. \textbf{Right:} Adaptive + Gap Recovery: 31 tracks, consolidated associations, zero ID switches.}
\end{figure}
\end{frame}
