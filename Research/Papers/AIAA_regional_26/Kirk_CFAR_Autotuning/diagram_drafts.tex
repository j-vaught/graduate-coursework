...
% ====================================================================
% BOTTOM SECTION: Three 11×11 2-D variant grids
% ====================================================================
\def\gsz{0.44}
...
\begin{scope}[shift={(#1,0)}]
  \node[font=\bfseries] at (5*\gsz+\gsz/2,\gn*\gsz+\gsz+0.6) {#2};
  ...
  \draw[{Stealth[length=2.5pt]}-{Stealth[length=2.5pt]},gray70]
    (-0.2,1*\gsz) -- (-0.2,4*\gsz);
  \node[left,font=\tiny,gray70] at (-0.2,2.5*\gsz) {$N_r$};
  \draw[{Stealth[length=2.5pt]}-{Stealth[length=2.5pt]},gray70]
    (-0.2,4*\gsz) -- (-0.2,5*\gsz);
  \node[left,font=\tiny,gray70] at (-0.2,4.5*\gsz) {$G$};

  % Selection arrow for GO/SO
  \ifnum#3=1
    \draw[-{Stealth[length=4pt]},rose,very thick]
      (11*\gsz+0.3,7.5*\gsz) -- (11*\gsz+0.3,9*\gsz);
    \node[right,font=\tiny,rose] at (11*\gsz+0.35,8.2*\gsz) {max};
  \fi
  \ifnum#3=2
    \draw[-{Stealth[length=4pt]},horseshoe,very thick]
      (11*\gsz+0.3,3.5*\gsz) -- (11*\gsz+0.3,1.5*\gsz);
    \node[right,font=\tiny,horseshoe] at (11*\gsz+0.35,2.5*\gsz) {min};
  \fi

  % Operator formula
  \ifnum#3=0
    \node[font=\tiny,atlantic] at (5.5*\gsz,-0.7)
  ...
  \node[font=\tiny,gray90] at (5.5*\gsz,-1.4)
    {Detect if $x_{\mathrm{CUT}} > \alpha \cdot Z$};
\end{scope}
}
...
\begin{scope}[shift={(1,-2.3)}]
  ...
  %
  \fill[atlantic!50] (11.2,0) rectangle ++(0.4,0.3);
  \node[right,font=\footnotesize] at (11.7,0.15) {Leading};
  %
  \fill[congaree!40] (13.4,0) rectangle ++(0.4,0.3);
  \node[right,font=\footnotesize] at (13.9,0.15) {Trailing};
  %
  \node[font=\small,horseshoe] at (15.8,0.15) {\checkmark};
  \node[right,font=\footnotesize] at (16.0,0.15) {Detected};
  %
  \node[font=\small,rose] at (18,0.15) {$\times$};
  \node[right,font=\footnotesize] at (18.2,0.15) {Below $T$};
\end{scope}
