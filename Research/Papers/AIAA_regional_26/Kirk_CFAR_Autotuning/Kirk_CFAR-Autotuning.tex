...
\begin{abstract}
Maritime domain awareness, search-and-rescue, and coastal monitoring requires robust on-board radar detection under tight size, weight, power, and cost constraints. Tuning Constant False Alarm Rate (CFAR) detectors for littoral scenes is notoriously difficult due to heavy-tailed sea clutter, land/sea transitions, and shoreline multipath, yet remains essential for low-latency situational awareness. We present a human-supervised methodology for tuning CFAR parameters of commercial-off-the-shelf X-band radars in coastal scenes with stationary/persistent objects. The human operator provides bounding boxes around stationary reference objects (e.g., navigation aids, towers, shoreline structures) on one or more radar frames. Then, the system (i) propagates those regions across a frame sequence, (ii) sweeps the parameter spaces of multiple CFAR variants (cell-averaging, order-statistic, censored, and others), and (iii) solves a multi-objective optimization by maximizing true detections inside annotated regions and minimizing false alarms elsewhere under an explicit  constraint. The outcome is a scene- and sensor-tailored algorithm and parameter configuration. Performance is evaluated on synthetic heavy-tailed sea-clutter (K/Pareto) and real X-band radar sequences to characterize the detection–false-alarm trade space under explicit  constraints, and to illustrate how the resulting scene-tailored configuration differs from default single-algorithm tuning.
\end{abstract}
