...
\paragraph{CA-CFAR.} Cell-Averaging CFAR computes the arithmetic mean of all $N = (2R_o+1)^2 - (2R_i+1)^2$ reference cells, where $R_o = N_g + N_r$ is the outer half-width and $R_i = N_g$ is the inner half-width:
\begin{equation}
...
\paragraph{GO-CFAR.} Greatest-Of CFAR mitigates the clutter-edge problem by splitting the reference annulus into two half-windows -- one leading (above) and one trailing (below) the cell-under-test -- and selecting the larger of the two noise estimates. Let $\bar{Z}_1$ and $\bar{Z}_2$ denote the mean power of each half-window, each containing $N_{\mathrm{half}}$ cells:
\begin{equation}
...
\paragraph{SO-CFAR.} Smallest-Of CFAR takes the opposite approach, selecting the lower of the two half-window estimates:
\begin{equation}
    Z_{i,j}^{\mathrm{SO}} = \min(\bar{Z}_1, \bar{Z}_2). \label{eq:so}
\end{equation}
This preserves sensitivity near clutter edges -- useful for detecting targets that lie adjacent to land or structures -- but at the expense of an elevated false-alarm rate in heterogeneous regions where the quieter half-window underestimates the true background.
...
Despite these limitations, the framework addresses several operational needs in aerospace and maritime domains. For maritime UAS, rapidly configurable CFAR detection directly improves on-board detection of fixed navigation aids during autonomous coastal mapping, port approach monitoring and deconfliction, and search-and-rescue operations in surf zones where sea spikes generate excessive false alarms. Because the browser-based architecture requires no software installation, no specialized computing hardware, and no network connectivity during operation, an operator can run CFAR Studio on a laptop alongside the radar feed in the field. This zero-infrastructure deployment model is particularly relevant for ad-hoc missions where pre-installed signal processing toolchains are unavailable and radar parameters must be adapted to an unfamiliar scene on short notice.
