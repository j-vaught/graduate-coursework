...
\begin{enumerate}
    \item A formal scoring function for CFAR evaluation anchored to human-annotated ground truth blobs, using area-weighted coverage ratios rather than binary detection indicators.
    \item An exhaustive grid search over three CFAR variants (CA, GO, SO) with integral-image loop hoisting that enables evaluation of hundreds of configurations in seconds.
    \item A multi-frame temporal stability metric that quantifies detection consistency across sequential radar sweeps.
    \item An open-source, fully client-side browser-based tool (\emph{CFAR Studio}) that requires no installation, server infrastructure, or specialized hardware beyond a modern web browser.
\end{enumerate}

\section{Background and Related Work}
\subsection{CFAR Detectors}
The CA-CFAR family estimates background power via local training windows and compares the cell-under-test to a scaled statistic to maintain a desired probability of false alarm $P_{\mathrm{FA}}$ \cite{finn1968, gandhi1988, mathworksCFAR}. GO- and SO-CFAR improve robustness near clutter edges and in multi-target scenes by partitioning leading and trailing reference windows and selecting the greatest-of or smallest-of their respective noise estimates \cite{hansen1972, trunk1978}. Other variants---including Order-Statistic (OS-CFAR), Variability-Index, and censored/trimmed-mean CFAR---address additional heterogeneous background scenarios \cite{farina2005review} but are beyond the scope of the current implementation.

\subsection{Littoral Sea-Clutter Context}
High-resolution maritime clutter is heavy-tailed and spatially nonhomogeneous; K-distribution and Pareto models are widely used to characterize such environments \cite{ward2006}. These non-Gaussian statistics motivate careful CFAR parameter selection, as training windows that span land--sea transitions or surf zones can severely bias noise estimates.

\subsection{CA-CFAR Threshold Multiplier}
For CA-CFAR with $N$ independent, identically distributed reference cells drawn from an exponential background, the threshold multiplier $\alpha$ that achieves a target $P_{\mathrm{FA}}$ admits the closed-form expression \cite{richards2005, finn1968}:
\begin{equation}
    \alpha = N\!\left(P_{\mathrm{FA}}^{-1/N} - 1\right). \label{eq:alpha}
\end{equation}
This relationship is used directly in the implementation; the same $\alpha$ formula is applied to GO- and SO-CFAR using the effective cell count of the selected half-window.

\paragraph{Positioning relative to prior work.} CFAR has been widely used across maritime and aerial surveillance owing to its controllable $P_{\mathrm{FA}}$, local-statistics design, and suitability for embedded implementations \cite{farina2005review,gandhi1988,ward2006}. Despite numerous efforts to simplify or automate CFAR tuning---ranging from vendor rule-of-thumb guides and interactive GUIs to heuristic search over thresholds for a single variant \cite{mathworksCFAR}---we are not aware of prior work that explicitly combines human annotation of stationary reference objects with exhaustive multi-variant parameter search and area-based scoring in a browser-deployable tool. Our approach uses human-provided ground truth to define an objective function while leveraging computational optimization to efficiently explore the parameter space.
...
\begin{figure*}[t]
    \centering
    \begin{tikzpicture}[
        block/.style={rectangle, draw, minimum height=1cm, minimum width=2.2cm, align=center, font=\small},
        arrow/.style={-{Stealth[length=3mm]}, thick},
        note/.style={font=\footnotesize\itshape, text width=3cm, align=center}
    ]
        % Blocks
        \node[block] (frame) {Radar Frame\\$I_t$};
        \node[block, right=1.2cm of frame] (integral) {Integral\\Image};
        \node[block, right=1.2cm of integral] (noise) {Noise Map $Z$\\(CA/GO/SO)};
        \node[block, right=1.2cm of noise] (thresh) {Threshold\\$\alpha \cdot Z$};
        \node[block, right=1.2cm of thresh] (detect) {Detection\\Map $D_t$};
        \node[block, right=1.2cm of detect] (score) {Blob Match\\$\rightarrow$ Score};
        ...
        \node[note, below=0.6cm of noise] (loopnote) {Computed once per $(N_g, N_r)$; reused across $P_{\mathrm{FA}}$ values};
        \draw[dashed, thick, rounded corners] ($(integral.south west)+(-0.1,-0.15)$) rectangle ($(noise.south east)+(0.1,1.15)$);
...
All three variants share the same two-dimensional sliding-window structure. For each pixel $(i,j)$ in frame $I_t$, a guard region of radius $N_g$ excludes the cell-under-test and its immediate neighbors; a reference annulus of width $N_r$ surrounds the guard region. An integral image (summed area table) \cite{crow1984} enables $O(1)$ computation of the sum over any rectangular region. The noise estimate $Z_{i,j}$ is computed as follows:

\textbf{CA-CFAR.} The mean of all $N = (2R_o+1)^2 - (2R_i+1)^2$ reference cells, where $R_o = N_g + N_r$ and $R_i = N_g$:
\begin{equation}
    Z_{i,j}^{\mathrm{CA}} = \frac{1}{N}\left(S_{\mathrm{outer}} - S_{\mathrm{inner}}\right), \label{eq:ca}
\end{equation}
where $S_{\mathrm{outer}}$ and $S_{\mathrm{inner}}$ are the summed-area-table queries over the outer and inner rectangles, respectively.

\textbf{GO-CFAR.} The reference annulus is split into leading and trailing half-windows (above and below the cell-under-test). Let $\bar{Z}_1$ and $\bar{Z}_2$ denote the mean power of each half-window, each containing $N_{\mathrm{half}}$ cells:
\begin{equation}
    Z_{i,j}^{\mathrm{GO}} = \max(\bar{Z}_1, \bar{Z}_2). \label{eq:go}
\end{equation}

\textbf{SO-CFAR.} The smallest-of selection:
\begin{equation}
    Z_{i,j}^{\mathrm{SO}} = \min(\bar{Z}_1, \bar{Z}_2). \label{eq:so}
\end{equation}

For GO- and SO-CFAR, the effective reference cell count used for $\alpha$ computation is $N_{\mathrm{half}}$ rather than the full annulus count.
...
\end{equation}
where $\alpha$ is given by Eq.~\eqref{eq:alpha} using the appropriate cell count.
...
\subsection{Area-Weighted Scoring Function}
\label{sec:scoring}
...
\subsection{Multi-Frame Temporal Stability}
\label{sec:stability}
...
\subsection{Grid Search with Integral-Image Loop Hoisting}
\label{sec:grid_search}

The parameter space is defined by discrete ranges for guard cells $N_g$, reference cells $N_r$, and a set of $P_{\mathrm{FA}}$ values. All combinations are evaluated exhaustively.
...
\begin{figure*}[t]
    ...
    \begin{tikzpicture}[
        ...
        % Row 2: Optimization
        \node[block, below=1.2cm of annotate] (worker) {Grid Search\\(Web Worker)};
        ...
        % Feedback loop
        \draw[arrow, dashed] (results.south) -- ++(0,-0.6) -| ($(viz.south)+(0,0)$);
        \node[font=\footnotesize\itshape, below=0.3cm of results] {Adjust params \& re-run};
...
\begin{itemize}
    \item \textbf{Application framework:} React~19 with TypeScript, built with Vite.
    \item \textbf{Visualization:} Plotly.js for interactive PPI (Plan Position Indicator) and B-scan displays with multiple colormaps and zoom/pan controls.
    \item \textbf{Data ingestion:} PapaParse for parsing CSV files exported from the radar digitizer.
    \item \textbf{Optimization engine:} A dedicated Web Worker thread that performs the grid search without blocking the user interface.
\end{itemize}
...
\begin{figure}[t]
    ...
    \caption{B-scan (range vs.\ azimuth) view with CFAR detections overlaid. This representation reveals clutter structure along individual radar spokes and aids in understanding detection behavior near land--sea transitions.}
    \label{fig:bscan}
\end{figure}

\subsection{Ground Truth Workflow}
\label{sec:gt_workflow}
...
\subsection{Hardware and Data Collection}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/ppi_optimization_in_progress.png}
    \caption{CFAR Studio during an active optimization run. The progress indicator shows the grid search advancing through parameter configurations while the PPI display remains interactive.}
...
\textbf{Guard cells.} Larger guard cell counts ($N_g \geq 3$) consistently improve scores for extended targets (e.g., dock structures), as they prevent the target's own return from biasing the noise estimate. For point-like targets (buoys), guard cell sensitivity is lower.

\textbf{Reference cells.} Moderate reference window sizes ($N_r \in [10, 20]$) provide the best trade-off between noise estimation accuracy and spatial homogeneity. Very large reference windows ($N_r > 25$) degrade performance near land--sea transitions where the training window spans heterogeneous clutter regions.

\textbf{$P_{\mathrm{FA}}$ sensitivity.} Lower $P_{\mathrm{FA}}$ values (more conservative) reduce false alarms but risk missing weaker targets. The optimal $P_{\mathrm{FA}}$ depends strongly on the scene: open-water targets tolerate lower $P_{\mathrm{FA}}$, while targets near clutter edges benefit from moderate $P_{\mathrm{FA}}$ to maintain detection.
...
\textbf{Stationary reference assumption.} The method assumes quasi-stationary reference objects and a fixed radar platform, so that ground truth annotations remain valid across frames. Extension to moving targets or mobile radar platforms would require frame-to-frame registration or track-before-detect association.

\textbf{Browser constraints.} Running entirely in the browser imposes limitations: single-threaded JavaScript execution (mitigated by Web Worker offloading), no GPU acceleration, and a practical file size limit of approximately 50~MB per CSV. For the dataset sizes typical of shore-based X-band radar sweeps, these constraints are not limiting.

\textbf{Grid search vs.\ sophisticated optimization.} An exhaustive grid search is computationally feasible for the three-variant, three-parameter space addressed here (guard, reference, $P_{\mathrm{FA}}$), with typical grids containing a few hundred configurations evaluable in seconds. More sophisticated approaches (Bayesian optimization, evolutionary algorithms) would become necessary if the parameter space were expanded to include additional CFAR variants with extra parameters (e.g., OS-CFAR order statistic $k$, censoring fractions).

\textbf{Area-based vs.\ binary scoring.} The area-weighted coverage score (Eq.~\ref{eq:score_zones}) provides substantially more information than a binary detected/not-detected metric. A configuration that detects only a small fragment of a large target receives a low score, whereas binary scoring would assign full credit. This distinction is important for targets where shape fidelity affects downstream processing (e.g., target classification or extent estimation).

\textbf{Current limitations.} The tool supports three CFAR variants (CA, GO, SO); incorporating OS-CFAR or censored variants would require sorting operations that increase per-pixel cost. Data ingestion is limited to the CSV format produced by the Furuno/RPi5 digitization pipeline; support for additional radar formats would require parser extensions.

%% ============================================================
%% SECTION 7: AIAA RELEVANCE
%% ============================================================
...
The source code is available at \url{https://github.com/IMSEL-Lab/cfar-studio}.
...
\begin{table}[h]
\centering
\caption{CFAR Studio technology stack.}
\label{tab:stack}
\begin{tabular}{ll}
\toprule
...
The authors thank the University of South Carolina Integrated Multiphysics Sensing and Engineering Laboratory (IMSEL) for feedback on early prototypes and access to the Furuno NXT DRS4D radar system.
...
\bibitem{trunk1978} G. V. Trunk, ``Range resolution of targets using automatic detectors,'' \emph{IEEE Trans.\ Aerosp.\ Electron.\ Syst.}, vol.~AES-14, no.~5, pp.~750--755, 1978.
...
\bibitem{richards2005} M. A. Richards, \emph{Fundamentals of Radar Signal Processing}. McGraw-Hill, 2005.

\bibitem{skolnik2008} M. I. Skolnik, \emph{Radar Handbook}, 3rd~ed. McGraw-Hill, 2008.

\bibitem{crow1984} F. C. Crow, ``Summed-area tables for texture mapping,'' in \emph{Proc.\ ACM SIGGRAPH}, vol.~18, no.~3, pp.~207--212, 1984.
