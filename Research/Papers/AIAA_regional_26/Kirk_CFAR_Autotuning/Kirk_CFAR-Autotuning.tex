...
This work contributes a formal scoring function for CFAR evaluation anchored to human-annotated ground truth blobs, using area-weighted coverage ratios rather than binary detection indicators; an exhaustive grid search over three CFAR variants (CA, GO, SO) with integral-image loop hoisting that enables evaluation of hundreds of configurations in seconds; a multi-frame temporal stability metric that quantifies detection consistency across sequential radar sweeps; and an open-source, fully client-side browser-based tool (\emph{CFAR Studio}) that requires no installation, server infrastructure, or specialized hardware beyond a modern web browser.
