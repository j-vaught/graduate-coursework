...
% Relax float placement to prevent figures from drifting pages later
...
Constant False Alarm Rate (CFAR) detectors are the dominant target-detection paradigm in maritime radar, valued for their controllable false-alarm probability, local-statistics design, and suitability for embedded implementations \cite{farina2005review, gandhi1988}. Under homogeneous, Gaussian clutter the threshold multiplier admits a closed-form solution and detection performance is well characterized \cite{finn1968, richards2005}. Yet real coastal and port environments violate every assumption this theory depends on: sea clutter is heavy-tailed and spatially nonhomogeneous, well modeled by K-distribution and Pareto families rather than Rayleigh statistics \cite{ward2006, fioranelli2014}; land--sea transition edges introduce abrupt power discontinuities; and shoreline multipath, mixed cooperative/non-cooperative traffic, and surf-zone returns further corrupt the reference window. As a result, selecting appropriate CFAR parameters is notoriously scene- and sensor-dependent---a configuration that works in open water may generate excessive false alarms near a marina, while aggressive clutter rejection near shore may mask small targets of interest \cite{dhs2009}. Currently, operators tune guard cells, reference windows, and false-alarm rates by manual trial-and-error, adjusting settings until the display ``looks right.'' This process is slow, subjective, and not reproducible.

Researchers have sought to address these limitations through adaptive and knowledge-based CFAR schemes. Variability-Index CFAR dynamically switches between CA, GO, and SO variants via hypothesis tests on the reference window \cite{vicfar2014}; knowledge-based detectors exploit GIS databases to select training samples appropriate to the local terrain \cite{farina2008kb}; and cognitive CFAR approaches compare local clutter statistics against prior performance models to adjust window length \cite{humphreys2022}. More recently, deep-learning detectors have demonstrated strong accuracy on maritime data \cite{salalstm2023}. However, each approach carries practical constraints that limit operational adoption. Adaptive switching heuristics degrade when interfering targets span both halves of the reference window \cite{vicfar2014}. Knowledge-based methods require pre-built environmental databases that do not exist for ad-hoc deployments in unfamiliar waters \cite{farina2008kb}. Cognitive CFAR optimizes window length alone, leaving the choice of variant, guard-cell size, and $P_{\mathrm{FA}}$ to the operator \cite{humphreys2022}. Deep-learning detectors abandon the CFAR property entirely, cannot guarantee false-alarm control, and require large labeled datasets that are unavailable in most operational maritime settings \cite{cfarnet2024}. Despite decades of effort, no method systematically searches across multiple CFAR variants and the full parameter space to find the best configuration for a given scene.

The tooling landscape compounds the problem. Commercial radar processors from major vendors embed proprietary CFAR logic with limited or no parameter exposure, while third-party research tools that do expose the full parameter set---most notably MATLAB's Radar Toolbox \cite{mathworksCFAR}---require expensive licenses, desktop installations, and cannot be deployed on edge hardware or shared without infrastructure. Yet port authorities, small-vessel operators, search-and-rescue teams, and academic researchers---the users who most need tunable detection for challenging littoral scenes---are precisely those without access to these toolchains. No open, installation-free tool exists that allows an operator to experiment with CFAR configurations in situ.

A further gap lies in how CFAR configurations are evaluated. Standard assessment relies on probability-of-detection and probability-of-false-alarm curves computed against synthetic injected targets or single-frame detection counts \cite{richards2005, skolnik2008}. However, this ignores two operational realities. First, radar operators already possess direct situational awareness of which stationary objects---navigation buoys, channel markers, bridge pylons, dock structures---should appear on the display. This knowledge constitutes a free, high-quality source of ground truth, yet no prior work formalizes operator annotation as a quantitative objective function for CFAR optimization. Second, a configuration that detects a target on one sweep but loses it on the next is operationally useless for tracking and situational awareness; nevertheless, temporal consistency across sequential radar sweeps is absent from standard CFAR evaluation metrics.
...
\bibitem{fioranelli2014} F. Fioranelli, M. Ritchie, and H. Griffiths, ``The Pareto distribution for high grazing angle X-band sea clutter,'' in \emph{Proc.\ IEEE Radar Conf.}, 2014, pp.~789--793.

\bibitem{dong2019} Y. Dong, ``Modeling the amplitude distribution of radar sea clutter,'' \emph{Remote Sensing}, vol.~11, no.~3, p.~319, 2019.

\bibitem{humphreys2022} B. Humphreys, C. J. Baker, and M. Sherwood, ``Cognitive CFAR parameter optimization for detection in sea clutter,'' in \emph{Proc.\ IET Int.\ Conf.\ Radar}, 2022, pp.~1--6.

\bibitem{vicfar2014} S. M. H. Sharifi and M. Modarres-Hashemi, ``Variability-index CFAR for heterogeneous environments,'' \emph{Digital Signal Process.}, vol.~34, pp.~32--41, 2014.

\bibitem{nncfar2019} Z. Zhang and X. Li, ``Neural network-based adaptive selection CFAR,'' \emph{Int.\ J.\ Innov.\ Comput.\ Inf.\ Control}, vol.~18, no.~4, pp.~205--218, 2019.

\bibitem{cfarnet2024} A. Shyam, R. Gini, and F. Greco, ``CFARnet: CFAR-constrained deep learning for radar detection,'' \emph{Signal Process.}, vol.~218, p.~109362, 2024.

\bibitem{seefurther2024} D. Gusland, S. H. Christiansen, and F. Fioranelli, ``See further than CFAR: Data-driven radar detector trained by lidar cross-sensor supervision,'' \emph{arXiv:2402.12970}, 2024.

\bibitem{salalstm2023} L. Chen, X. Wang, and Y. Liu, ``SALA-LSTM: Maritime radar target detection in complex sea clutter,'' \emph{Scientific Reports}, vol.~13, p.~10234, 2023.

\bibitem{farina2008kb} A. Farina, L. Timmoneri, and F. Gini, ``Design and experimental validation of knowledge-based CFAR detectors,'' \emph{IET Radar, Sonar Navig.}, vol.~2, no.~2, pp.~150--159, 2008.

\bibitem{wicks1996} M. C. Wicks, W. L. Baldygo, and R. D. Brown, ``Expert system CFAR,'' U.S.\ Patent 5,499,030, Mar.\ 1996.

\bibitem{dhs2009} U.S.\ Dept.\ of Homeland Security, ``Small boat intrusion detection systems,'' System Assessment and Validation for Emergency Responders (SAVER) TechNote, 2009.
\end{thebibliography}
