...
\column{0.5\textwidth}
\begin{figure}
\includegraphics[width=0.75\textwidth,height=0.65\textheight,keepaspectratio]{image_01.png}
...
\begin{tikzpicture}[
    ...
    layer/.style={rectangle, draw=Horseshoe, fill=Horseshoe!5, text width=11cm, minimum height=1.7cm, align=left, line width=1.2pt},
...
% Layer 2
\node[layer, below=0.2cm of L1] (L2) {%
...
% Layer 3
\node[layer, below=0.2cm of L2] (L3) {%
...
\begin{tikzpicture}[scale=0.75,
    state/.style={rectangle, draw=Garnet, fill=Garnet!5, text width=1.6cm, minimum height=0.55cm, align=center, font=\scriptsize, line width=0.8pt},
    statew/.style={rectangle, draw=Garnet, fill=Garnet!5, text width=2.0cm, minimum height=0.55cm, align=center, font=\scriptsize, line width=0.8pt},
    arrow/.style={->, >={Latex[length=2mm, width=1.5mm]}, draw=Black70, line width=0.8pt}
]
\node[state] (IDLE) at (0,0) {IDLE};
\node[state] (ACQ) at (2.7,0) {ACQUIRING};
\node[state] (TRACK) at (5.4,0) {TRACKING};
\node[statew] (REACQ) at (5.4,-1.3) {REACQUIRING};
\node[state] (LOST) at (2.7,-1.3) {LOST};
...
\draw[arrow] (REACQ.east) -- ++(0.4,0) |- (TRACK.east);
\end{tikzpicture}
\end{center}
...
\item Inverse-square speed scaling
...
\vspace{0.1em}
{\footnotesize
Constant-velocity model with EMA smoothing:
{\scriptsize
\[
...
\vspace{0.2em}
\textbf{Key Advantage:} Optical zoom provides \emph{true} high-resolution data
\end{columns}
\end{frame}
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth,height=0.46\textheight,keepaspectratio]{image_02.png}
...
\item Predictive slew-to-classification control
\item Parallel GPU/DLA inference (57.4 FPS)
\item Validated on 42-second drone tracking
\end{itemize}
}
...
\item 100\% wide, 99\% PTZ detection rate
\item MJPEG reduces latency by 60--130 ms
\item Zero reacquisitions during tracking
\end{itemize}
}
