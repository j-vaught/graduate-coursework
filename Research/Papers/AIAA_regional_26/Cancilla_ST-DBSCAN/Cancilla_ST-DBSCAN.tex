...
The proposed pipeline consists of four stages: (i) polar-to-Cartesian coordinate conversion, (ii) adaptive ST-DBSCAN clustering with spatial hashing, (iii) gap recovery for track continuity, and (iv) Hungarian assignment for frame-to-frame track association. Each stage is illustrated in Fig.~\ref{fig:pipeline}.
...
The intuition behind this approach follows from the observation that sorted k-NN distances exhibit a characteristic ``elbow'' separating cluster points from noise~\cite{Ertoz2003}. Points within dense clusters have small k-distances, while noise points in sparse regions have large k-distances. Fig.~\ref{fig:kdistance} illustrates this behavior on a representative radar frame: the sorted k-distance curve rises gradually through cluster points, then inflects sharply as it reaches noise. The 90th percentile threshold captures points in the transition region, providing an epsilon that includes most cluster points while excluding sparse outliers.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/kdistance_plot.pdf}
\caption{Sorted k-nearest-neighbor distances for a representative radar frame ($k=5$). The elbow in the curve indicates the transition between dense cluster regions (low k-distance) and sparse noise regions (high k-distance). The 90th percentile threshold (dashed line) provides an adaptive epsilon that separates cluster points from noise.}
\label{fig:kdistance}
\end{figure}
...
\toprule
\textbf{Method} & \textbf{MOTA} & \textbf{MOTP (px)}\textsuperscript{*} & \textbf{Recall} & \textbf{Prec.} & \textbf{FP} & \textbf{IDSW} & \textbf{Tracks} \\
\midrule
...
\bottomrule
\multicolumn{8}{l}{\textsuperscript{*}\footnotesize MOTP is undefined when recall is zero (no matched detections).}
\end{tabular*}
\end{table}
