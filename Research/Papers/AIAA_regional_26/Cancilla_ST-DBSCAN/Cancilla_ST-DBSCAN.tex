...
Early radar tracking systems relied on explicit statistical modeling: CFAR detectors \cite{Rohling1983,Richards2014} characterized clutter as a known distribution and set adaptive thresholds accordingly, while tracking filters like JPDAF \cite{Fortmann1983} and MHT \cite{Reid1979} propagated probabilistic hypotheses about target state \cite{BarShalom2011}. This framework performs well when its assumptions hold, but littoral environments routinely violate them---sea clutter is neither homogeneous nor stationary, and the computational cost of maintaining multiple association hypotheses grows exponentially with target density \cite{Gandhi1988}.

The introduction of DBSCAN \cite{Ester1996} offered a fundamentally different philosophy: rather than modeling clutter explicitly, let the data reveal structure through local density. Points in dense regions belong to clusters; sparse points are noise. This elegance came with a practical limitation---the neighborhood radius $\varepsilon$ had to be chosen manually, and no single value worked well when density varied across the scene. Researchers responded along two fronts. One line of work sought to eliminate the parameter entirely: OPTICS \cite{Ankerst1999} produced hierarchical density orderings, and HDBSCAN \cite{Campello2013} extended this to extract flat clusters automatically. Another line pursued adaptive parameter selection: Ertoz, Steinbach, and Kumar \cite{Ertoz2003} observed that plotting sorted k-nearest-neighbor distances reveals a natural ``elbow'' separating cluster points from noise, and Liu, Zhou, and Wu \cite{Liu2007} used this insight to compute $\varepsilon$ locally. Meanwhile, Birant and Kut \cite{Birant2007} recognized that radar returns have temporal structure and extended DBSCAN into ST-DBSCAN, adding a temporal neighborhood threshold to link detections across successive sweeps \cite{Kisilevich2010}.

Yet a gap persists. The parameter-free methods (OPTICS, HDBSCAN) are too slow for real-time embedded systems. The adaptive methods (VDBSCAN and variants) were not designed for spatiotemporal data and do not address the track fragmentation that occurs when targets briefly disappear---a routine event in maritime radar where wave shadowing and aspect-angle variations cause intermittent dropouts. Track-before-detect methods \cite{Davey2012} handle weak and intermittent targets by integrating energy across scans, but add latency. Deep learning trackers \cite{Bewley2016,Wojke2017} excel on video by exploiting appearance, but radar provides no such features. Our contribution bridges these gaps: we combine adaptive $\varepsilon$ selection with spatiotemporal clustering and explicit gap recovery, all within a real-time embedded implementation.
