...
\textbf{Limitations.} All methods exhibit negative MOTA scores, indicating that false positives and misses exceed correct detections---a consequence of the challenging high-clutter synthetic environment rather than fundamental algorithmic failure. The adaptive epsilon estimation, while theoretically motivated, requires the minimum floor parameter to function on dense radar data, limiting its ``adaptive'' nature. Additionally, these results are obtained on synthetic data with persistent, non-maneuvering targets; performance on real maritime radar with intermittent detections and complex target dynamics may differ. The gap recovery mechanism, designed for intermittent targets, provides limited benefit on this dataset where targets appear consistently.
