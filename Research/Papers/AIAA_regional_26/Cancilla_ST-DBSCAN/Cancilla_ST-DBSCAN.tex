...
Despite decades of research on CFAR detection and multi-target tracking, reliable surface-object tracking from non-coherent marine radar remains an open challenge. Commercial X-band radars -- widely deployed due to their order-of-magnitude cost advantage over military tracking systems -- provide only intensity measurements without Doppler phase history, precluding conventional moving-target indication (MTI) processing for clutter suppression \cite{Skolnik2008,Richards2010}. In coastal and littoral environments, this limitation is compounded by sea clutter that varies rapidly with sea state, wind direction, and bathymetry, exhibiting the heavy-tailed amplitude statistics and spatial heterogeneity that violate the homogeneity assumptions underlying standard CFAR detectors \cite{Ward2006}.
...
Richards, M.~A., Scheer, J.~A., and Holm, W.~A., \textit{Principles of Modern Radar: Basic Principles}, SciTech Publishing, 2010.
