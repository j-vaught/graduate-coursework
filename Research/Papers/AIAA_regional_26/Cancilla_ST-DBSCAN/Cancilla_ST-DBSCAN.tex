...
Despite decades of research on CFAR detection and multi-target tracking, reliable surface-object tracking from non-coherent marine radar remains an open challenge. Commercial X-band radars -- widely deployed due to their order-of-magnitude cost advantage over military tracking systems -- provide only intensity measurements without Doppler phase history, precluding conventional moving-target indication (MTI) processing for clutter suppression \cite{Brennan1973}. In coastal and littoral environments, this limitation is compounded by sea clutter that varies rapidly with sea state, wind direction, and bathymetry, exhibiting the heavy-tailed amplitude statistics and spatial heterogeneity that violate the homogeneity assumptions underlying standard CFAR detectors \cite{Ward1981}.
...
Adaptive clustering methods such as OPTICS \cite{Ankerst1999} and HDBSCAN \cite{Campello2013} address parameter sensitivity by eliminating fixed $\varepsilon$ requirements, but incur computational costs prohibitive for real-time embedded deployment. Track-before-detect approaches \cite{Davey2008} integrate energy across multiple sweeps to improve detection of weak targets, but introduce latency incompatible with the rapid update rates required for collision avoidance and surveillance. Probabilistic data association methods like JPDAF \cite{Fortmann1983} and MHT \cite{Reid1979} maintain association hypotheses through occlusions, but scale poorly with target count and clutter density.
...
\bibitem{Brennan1973}
Brennan, L.~E., and Reed, I.~S., ``Theory of Adaptive Radar,'' \textit{IEEE Transactions on Aerospace and Electronic Systems}, Vol.~AES-9, No.~2, 1973, pp.~237--252.

\bibitem{Ward1981}
Ward, K.~D., ``Compound Representation of High Resolution Sea Clutter,'' \textit{Electronics Letters}, Vol.~17, No.~16, 1981, pp.~561--563.
...
\bibitem{Kalman1960}
Kalman, R.~E., ``A New Approach to Linear Filtering and Prediction Problems,'' \textit{ASME Journal of Basic Engineering}, Vol.~82, No.~1, 1960, pp.~35--45.
...
\bibitem{Davey2008}
Davey, S.~J., Rutten, M.~G., and Cheung, B., ``A Comparison of Detection Performance for Several Track-before-Detect Algorithms,'' \textit{EURASIP Journal on Advances in Signal Processing}, Vol.~2008, 2008, Article ID 428036.
