...
The results reveal fundamental trade-offs between recall, false positive rate, and localization accuracy. Vanilla DBSCAN achieves 98.7\% recall but generates 1208 false positives, yielding 48 tracks of which only 3 correspond to ground truth. Standard ST-DBSCAN produces zero recall because temporal connectivity chains all frames into mega-clusters whose centroids lie far from any instantaneous target position. The proposed adaptive ST-DBSCAN achieves the best MOTA ($-4.44$) and MOTP (1.19 pixels), reducing false positives by 32\% while maintaining 97.4\% recall; however, a minimum epsilon floor ($\varepsilon_{\min} = 20$ pixels) is required to prevent fragmentation in high-density regions. Adding gap recovery further reduces false positives to 695 and eliminates identity switches, though recall drops to 81.2\% due to occasional over-merging.

For safety-critical applications requiring maximum detection, vanilla DBSCAN or adaptive ST-DBSCAN should be used. For systems where false tracks impose significant costs, adaptive ST-DBSCAN with gap recovery provides better precision at acceptable recall loss. Table~\ref{tab:per_object} shows per-object performance: all three persistent objects achieve 100\% track lifetime with sub-2-pixel localization error, while spurious tracks arise primarily from persistent land returns rather than random noise.
