...
The remainder of this paper is organized as follows. Section~II reviews related work. Section~III formalizes the problem. Section~IV details the methodology. Section~V describes implementation. Sections~VI--VII present experiments and results. Section~VIII discusses limitations, and Section~IX concludes.
...
Several approaches address automatic parameter selection. Ertoz, Steinbach, and Kumar \cite{Ertoz2003} proposed using k-nearest-neighbor distances to identify $\varepsilon$ from the inflection point in the k-distance plot. OPTICS \cite{Ankerst1999} and HDBSCAN \cite{Campello2013} eliminate fixed $\varepsilon$ requirements but incur additional computational cost that may be prohibitive for real-time processing.
...
GDBSCAN \cite{Sander1998} allows arbitrary neighborhood predicates; Liu, Zhou, and Wu \cite{Liu2007} compute adaptive $\varepsilon$ from k-distances; Hou, Gao, and Li \cite{Hou2016} combine k-NN graphs with density peaks. Our approach builds on the k-distance method of Liu, Zhou, and Wu, computing neighborhood radii via spatial hashing to accommodate the varying point density in radar data (denser near-range, sparser far-range due to $R^{-4}$ loss).
