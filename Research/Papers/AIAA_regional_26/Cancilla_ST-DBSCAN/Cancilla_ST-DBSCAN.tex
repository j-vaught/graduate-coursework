...
\textbf{Standard ST-DBSCAN} ($\varepsilon_s = 10$, $\varepsilon_t = 2$) produces an unexpected failure mode: zero recall despite generating only 16 false positives. The temporal linkage causes spatiotemporal clusters to span multiple frames, and their centroids drift away from instantaneous target positions as points from different time steps are averaged together. While this yields the fewest tracks (10), none pass within the 50-pixel association threshold. \textit{Strength}: Aggressive false positive suppression. \textit{Weakness}: Centroid drift renders tracking ineffective for point-based evaluation.
