...
The classical approach to radar target tracking combines CFAR detection with probabilistic data association. CFAR detectors \cite{Rohling1983,Richards2014} estimate local clutter power and set adaptive thresholds to maintain specified false alarm rates; ordered-statistic variants handle interfering targets, and adaptive formulations address nonhomogeneous coastal environments \cite{Gandhi1988}. Once detections are extracted, multi-object tracking associates them across time: the Hungarian algorithm \cite{Kuhn1955} provides optimal single-hypothesis assignment, while JPDAF \cite{Fortmann1983} and MHT \cite{Reid1979} maintain probabilistic association hypotheses at the cost of exponential complexity \cite{BarShalom2011}. These methods assume explicit statistical models of clutter and target dynamics, and their performance degrades when model assumptions are violated---a common occurrence in littoral environments with spatially varying sea states.

Density-based clustering emerged as a model-free alternative. DBSCAN \cite{Ester1996} groups points by local density, naturally handling arbitrary cluster shapes and rejecting sparse outliers without requiring prior knowledge of target count or clutter statistics. Subsequent work addressed DBSCAN's sensitivity to its neighborhood radius $\varepsilon$: GDBSCAN \cite{Sander1998} generalized the neighborhood predicate, OPTICS \cite{Ankerst1999} produced density-based orderings without fixed $\varepsilon$, and Ertoz, Steinbach, and Kumar \cite{Ertoz2003} proposed selecting $\varepsilon$ from the inflection point in k-nearest-neighbor distance plots. Birant and Kut \cite{Birant2007} extended DBSCAN to spatiotemporal data with ST-DBSCAN, adding a temporal threshold to link detections across successive frames \cite{Kisilevich2010}. Liu, Zhou, and Wu \cite{Liu2007} computed adaptive $\varepsilon$ from local k-distances, and HDBSCAN \cite{Campello2013} eliminated fixed parameters entirely through hierarchical density estimation. However, OPTICS and HDBSCAN incur computational costs that challenge real-time constraints, and none of these methods explicitly address the track fragmentation caused by intermittent target visibility.

Recent tracking methods have pursued complementary directions. Track-before-detect approaches \cite{Davey2012} integrate energy over multiple scans to detect weak targets, but introduce latency incompatible with rapid surveillance updates. Deep learning trackers such as SORT \cite{Bewley2016} and DeepSORT \cite{Wojke2017} achieve strong performance on video data by exploiting appearance features, but these features are unavailable in radar imagery. The gap remains: no existing method combines adaptive density-based clustering, spatiotemporal track association, explicit dropout recovery, and real-time embedded performance. Our work addresses this gap by extending ST-DBSCAN with k-NN-based parameter adaptation and Union-Find gap recovery, implemented efficiently for embedded deployment.
