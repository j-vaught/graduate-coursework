...
The foundations of modern radar tracking emerged during the Cold War, when Kalman's 1960 optimal filtering theory \cite{BarShalom2011} provided the mathematical framework for state estimation under uncertainty. Through the 1970s and 1980s, this framework evolved into sophisticated multi-target trackers. Reid introduced Multiple Hypothesis Tracking (MHT) in 1979 \cite{Reid1979}, maintaining a tree of possible data associations, while Fortmann and Bar-Shalom developed the Joint Probabilistic Data Association Filter (JPDAF) in 1983 \cite{Fortmann1983}, computing association probabilities without exhaustive enumeration. In parallel, Rohling \cite{Rohling1983} refined CFAR detection to handle clutter with adaptive thresholds. These methods shared a common philosophy of explicit statistical modeling, assuming clutter could be characterized by known distributions and target dynamics by parametric motion models. The assumptions held reasonably well for air surveillance radars tracking aircraft against relatively benign backgrounds, but maritime environments proved more challenging. Sea clutter exhibits heavy-tailed amplitude statistics and rapid spatial variation that violate homogeneity assumptions \cite{Ward2006}, and Gandhi and Kassam \cite{Gandhi1988} showed that CFAR performance degrades substantially in such non-homogeneous backgrounds.

A fundamentally different approach emerged from the data mining community. In 1996, Ester, Kriegel, Sander, and Xu presented DBSCAN at the KDD conference in Portland \cite{Ester1996}, introducing density-based clustering that required no prior knowledge of cluster count or shape. The algorithm's elegance lay in its simplicity. Points in dense regions belonged to clusters, and sparse points were labeled as noise. For radar applications, this offered an appealing alternative to explicit clutter modeling. Rather than specifying what clutter looked like, the algorithm simply grouped coherent returns and discarded the rest. The limitation was practical rather than theoretical. The neighborhood radius $\varepsilon$ had to be chosen manually, and no single value worked well when point density varied across the scene. Ertoz, Steinbach, and Kumar addressed this in 2003 \cite{Ertoz2003} by observing that sorted k-nearest-neighbor distances reveal a natural elbow separating cluster points from noise. Liu, Zhou, and Wu \cite{Liu2007} extended this insight into VDBSCAN, computing $\varepsilon$ locally for each point. Meanwhile, Ankerst and colleagues developed OPTICS in 1999 \cite{Ankerst1999}, producing hierarchical density orderings that eliminated fixed parameters entirely, and Campello's HDBSCAN \cite{Campello2013} later extracted flat clusters automatically from such orderings. Birant and Kut \cite{Birant2007} recognized that radar returns possess temporal structure that purely spatial clustering ignores, and their ST-DBSCAN added a temporal neighborhood threshold to link detections across successive sweeps.

These developments left an important gap. The parameter-free methods run too slowly for real-time embedded systems. The adaptive methods were designed for static datasets and do not address track fragmentation from intermittent target visibility---a routine occurrence in maritime radar where wave shadowing and aspect-angle variations cause brief detection dropouts. Track-before-detect methods \cite{Davey2012} integrate energy across multiple scans to detect weak targets, but introduce latency incompatible with rapid update rates. Deep learning trackers like SORT \cite{Bewley2016} and DeepSORT \cite{Wojke2017} excel on video by exploiting appearance features, but radar provides no such visual information. Our contribution bridges these gaps by combining adaptive $\varepsilon$ selection with spatiotemporal clustering and explicit gap recovery, all within a real-time embedded implementation.
