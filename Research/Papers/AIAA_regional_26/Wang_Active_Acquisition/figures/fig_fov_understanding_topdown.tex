...
\newcommand{\drawQuad}[1]{%
    ...
    \def\blade{\fill[CGrayDark, opacity=0.9] (-0.35, -0.03) rectangle (0.35, 0.03);}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \begin{scope}
        ...
        \draw[CGrayDark, thick, fill=CGrayLight, fill opacity=0.5]
            (FixedOrigin) -- ++(130:7.5) arc (130:50:7.5) -- cycle;
    \end{scope}

    % Zoom FOV (Dark Gray) - Narrow Beam
    \begin{scope}
        \coordinate (PTZOrigin) at (1.0, 0.0);
        \draw[black, thick, fill=black, fill opacity=0.25]
    ...
    % --- Planes ---
    % 1. Original Target Plane (Centered in Zoom FOV) -> BLACK
    \begin{scope}[shift={(2.5, 6.5)}, rotate=75, scale=\planeScale]
        \drawPlane{black}
    \end{scope}

    % 2. Distractor Plane (in Wide FOV, outside Zoom) -> DARK GRAY
    ...
    % 4. Distractor Plane (High center, inside Wide FOV) -> DARK GRAY
    ...
    % --- Quadcopters ---
    % 1. Original Distractor Quad (in Wide FOV, left) -> DARK GRAY
    \begin{scope}[shift={(-2.8, 4.5)}, rotate=-10, scale=\quadScale]
        \drawQuad{CGrayDark}
    \end{scope}

    % 2. Distractor Quad (near Zoom FOV edge, inside Wide) -> DARK GRAY
    ...
    \begin{scope}[shift={(-0.5, 0)}]
        \begin{scope}[rotate=90, scale=0.5]
             ...
             \draw[black, line width=1.5pt] (0, -0.3) -- (0, 0.3);
        \end{scope}
