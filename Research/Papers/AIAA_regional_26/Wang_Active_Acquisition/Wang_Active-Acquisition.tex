...
Classical solutions to the small-object bottleneck involve super-resolution (SR) to reconstruct lost detail from low-resolution inputs \cite{Mahaur2022}. However, reliance on SR for scientific data collection is problematic. First, SR is fundamentally generative; it estimates high-frequency details based on learned priors, creating a risk of hallucination where the model reinforces its own biases \cite{Zhang2019}. Second, high-fidelity generative models required for scientific validity often require $>300$\,ms per frame on edge accelerators \cite{NVIDIA2024}, whereas lightweight models sacrifice reconstruction quality. Our system achieves mechanical slew-and-settle in a comparable time window while obtaining true optical ground truth rather than hallucinated details.

\subsection{Slew-to-Classification vs.\ Continuous Tracking}
Most PTZ tracking literature focuses on the control problem of keeping a target centered in the frame \cite{Chen2015, Tang2017}. This requires mitigating total system latency (video encoding + network + mechanical response), which for IP-based systems frequently ranges from 200--500\,ms \cite{PTZOptics2024}. Our work addresses a different objective we term slew-to-classification. Rather than maintaining persistent lock on one target, the system performs discrete acquisitions of ambiguous candidates to verify their identity.

Existing active perception systems like VIGIA-E \cite{Pedrosa2025} typically optimize for broad area coverage or anomaly detection. Our system instead functions as a sparse query mechanism that identifies specific low-confidence candidates in the wide field and commits the PTZ resource to verifying them individually.

\subsection{Sensor-Driven Labeling}
Reducing manual annotation is a central goal of both semi-supervised learning and active learning. Pseudo-labeling methods such as ASTOD \cite{Vandeghen2023} attempt to retrain models using high-confidence predictions, but this approach often fails in the small-object regime where the detector is consistently uncertain \cite{Sajid2024}. Active learning strategies like PPAL \cite{Yang2024AL} identify informative samples but still require a human loop \cite{MoralesHernandez2023}.

Our slew-to-classification approach creates a fully automated hybrid in which the selection logic of active learning (targeting less confident samples) is paired with PTZ verification instead of a human annotator. While the target is ambiguous at $15 \times 15$ pixels, the zoomed view restores it to a regime ($>100 \times 100$ pixels) where off-the-shelf detectors achieve 99.1\% mAP50 \cite{Reis2023}. By physically bridging the gap between the surveillance view and the high-resolution training distribution, we convert a difficult small-object inference problem into a trivial classification task.
