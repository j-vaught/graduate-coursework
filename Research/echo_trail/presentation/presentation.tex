...
\begin{column}{0.42\textwidth}
  \begin{center}
    \fbox{\includegraphics[width=0.95\textwidth,clip,trim=100 100 100 100]{../paper/data/A1/ppi/noclutter/N_0/frame_0030.png}}
  \end{center}
  \vspace{0.1cm}
  {\small Real PPI output: 1735$\times$1735 px\\grayscale Cartesian image.}
\end{column}
\end{columns}
...
\begin{column}{0.42\textwidth}
  \begin{center}
    \includegraphics[width=0.95\textwidth,clip,trim=50 50 50 50]{a1_scene_01.png}
  \end{center}
  \vspace{0.1cm}
  {\small Scene layout: water mask with color-coded target trajectories.}
\end{column}
\end{columns}
...
\begin{column}{0.42\textwidth}
  \begin{center}
    \fbox{\includegraphics[width=0.46\textwidth,clip,trim=100 100 100 100]{../paper/data/A1/ppi/noclutter/N_0/frame_0010.png}}
    \fbox{\includegraphics[width=0.46\textwidth,clip,trim=100 100 100 100]{../paper/data/A1/ppi/noclutter/N_10/frame_0010.png}}
  \end{center}
  \vspace{0.1cm}
  {\small Left: raw PPI ($N{=}0$). Right: with trails ($N{=}10$).}
\end{column}
\end{columns}
...
\begin{column}{0.48\textwidth}
  ...
  \end{center}
  \vspace{0.1cm}
  {\small Path preview: stationary (white), slow (blue), medium (yellow), fast (red).}
\end{column}
\end{columns}
...
\begin{column}{0.42\textwidth}
  \begin{center}
    \fbox{\includegraphics[width=0.95\textwidth,clip,trim=100 100 100 100]{../paper/data/A1/ppi/clutter/N_10/frame_0030.png}}
  \end{center}
  \vspace{0.1cm}
  {\small Real output: interference streaks (red spokes) overlay target returns (green).}
\end{column}
\end{columns}
...
\begin{column}{0.48\textwidth}
  ...
  \begin{itemize}
    ...
    \item 7 object sub-classes $\rightarrow$ 2 detection classes (stationary, moving)
    \item Labels in YOLO format (class, $x_c$, $y_c$, $w$, $h$)
...
\begin{center}
  \includegraphics[width=0.55\textwidth]{fig5_binary_proportional.pdf}
\end{center}
...
\begin{column}{0.48\textwidth}
  ...
  \begin{itemize}
    ...
    \item \textbf{Fast:} \emphnum{+34.7pp} jump at $N{=}3$ (0.488 $\rightarrow$ 0.836), saturated by $N{=}10$
  \end{itemize}
...
\begin{column}{0.48\textwidth}
  ...
  \begin{itemize}
    \item \textbf{Low interference:} modest benefit over baseline (already strong SNR)
    \item \textbf{Heavy interference:} no-trail baseline degrades significantly; trails help via accumulation
  \end{itemize}

  \vspace{0.3cm}

  However, this benefit is speed-dependent: fast targets gain, slow targets
  may still degrade.

  \vspace{0.2cm}

  Interference inverts the stationary/moving ranking for ALL configurations
  tested in A1--A4.
\end{column}
\end{columns}
...
% ============================================================
% SLIDE 31: SURPRISING FINDINGS
% ============================================================
\begin{frame}{Unexpected Results}

\begin{enumerate}
  \item \textbf{Precision = 1.0 on all moving targets} (clean). Zero false positives;
        all errors are missed detections.

  \vspace{0.1cm}

  \item \textbf{Stationary harder than fast at baseline} ($N{=}0$): stationary
        $\maptf{=}0.339$ vs.\ fast${=}0.488$. Without trails, positional salience matters.

  \vspace{0.1cm}

  \item \textbf{Mono leads on medium-speed targets} in \textit{both} conditions ---
        replicated consistently, mechanism unclear.

  \vspace{0.1cm}

  \item \textbf{Medium-speed tier consistently hardest} across ALL experiments
        ($\sim$0.286 $\maptf$). Displacement too short for directional cue,
        too long for point signature.

  \vspace{0.1cm}

  \item \textbf{Interference inverts stationary/moving ranking} for every parameter
        combination tested.

  \vspace{0.1cm}

  \item \textbf{Twotone overfitting:} peaks highest in training (0.923) but
        overfits; gradient (0.918 peak) generalizes better.
\end{enumerate}

\keypoint{These anomalies reveal that trail rendering interacts with detection in non-obvious ways.}
\end{frame}
