...
\begin{tikzpicture}[node distance=0.6cm and 1.5cm]
  % Task side
  \node[agentbox=garnet, minimum width=2.8cm] (parent1) {Parent};
  \node[workerbox, below left=0.8cm and 1.0cm of parent1, minimum width=2.2cm] (t1) {Task 1};
  \node[workerbox, below=0.8cm of parent1, minimum width=2.2cm] (t2) {Task 2};
  \node[workerbox, below right=0.8cm and 1.0cm of parent1, minimum width=2.2cm] (t3) {Task 3};
  \draw[dataarrow=garnet] (parent1) -- (t1);
  \draw[dataarrow=garnet] (parent1) -- (t2);
  \draw[dataarrow=garnet] (parent1) -- (t3);
  \node[labelnode, above=0.15cm of parent1] {\textbf{Task Model}};
  \node[labelnode, below=0.08cm of t1] {one-shot};
  \node[labelnode, below=0.08cm of t2] {one-shot};
  \node[labelnode, below=0.08cm of t3] {one-shot};

  % Subagent side
  \node[agentbox=garnet, right=5.0cm of parent1, minimum width=2.8cm] (parent2) {Parent};
  \node[agentbox=congaree, below=0.8cm of parent2,
        minimum width=2.2cm] (sub) {Subagent};
  \draw[dataarrow=garnet] (parent2.south) -- ++(0,-0.25) -| ([xshift=-4pt]sub.north);
  \draw[dataarrow=congaree] ([xshift=4pt]sub.north) -- ++(0,0.25) -| (parent2.south);
  \node[labelnode, above=0.15cm of parent2] {\textbf{Subagent Model}};
  \node[labelnode, right=0.15cm of sub] {multi-turn};

  % Divider
  \draw[bk30, dashed, thick] ($(parent1.east)!0.5!(parent2.west)$) ++(0,0.6) -- ++(0,-2.4);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.5cm,
  phase/.style={rectangle, draw=garnet, fill=garnet!8, thick,
    minimum width=\textwidth-2cm, minimum height=0.7cm,
...
\begin{tikzpicture}[node distance=0.8cm and 1.5cm]
  \node[agentbox=garnet, minimum width=6cm, minimum height=1.0cm] (orch) {\large Orchestrator};

  \node[workerbox, below left=1.2cm and 2.5cm of orch, minimum width=2.8cm] (w1) {Worker A};
  \node[workerbox, below=1.2cm of orch, minimum width=2.8cm] (w2) {Worker B};
  \node[workerbox, below right=1.2cm and 2.5cm of orch, minimum width=2.8cm] (w3) {Worker C};

  % Downward arrows
  \draw[dataarrow=garnet, line width=1.2pt] ([xshift=-0.8cm]orch.south) -- ([xshift=0.2cm]w1.north)
    node[midway, left, labelnode, font=\small\sffamily] {task def.};
  \draw[dataarrow=garnet, line width=1.2pt] (orch.south) -- (w2.north)
    node[midway, right, labelnode, font=\small\sffamily] {task def.};
  \draw[dataarrow=garnet, line width=1.2pt] ([xshift=0.8cm]orch.south) -- ([xshift=-0.2cm]w3.north)
    node[midway, right, labelnode, font=\small\sffamily] {task def.};

  % Upward arrows (dashed)
  \draw[dataarrow=atlantic, dashed, line width=1.2pt] ([xshift=-0.2cm]w1.north) -- ([xshift=-1.2cm]orch.south)
    node[midway, right, labelnode, font=\small\sffamily, text=atlantic] {result};
  \draw[dataarrow=atlantic, dashed, line width=1.2pt] ([xshift=0.15cm]w2.north) -- ([xshift=0.15cm]orch.south)
    node[midway, left, labelnode, font=\small\sffamily, text=atlantic] {result};
  \draw[dataarrow=atlantic, dashed, line width=1.2pt] ([xshift=0.2cm]w3.north) -- ([xshift=1.2cm]orch.south)
    node[midway, left, labelnode, font=\small\sffamily, text=atlantic] {result};

  \node[labelnode, below=0.1cm of w2, font=\small\sffamily]
...
\begin{tikzpicture}[node distance=0.6cm and 0.5cm,
  ...
  % Parallel
  \node[tsk=garnet] (p1) {A};
  \node[tsk=garnet, right=0.8cm of p1] (p2) {B};
  \node[tsk=garnet, right=0.8cm of p2] (p3) {C};
  \node[labelnode, above=0.15cm of p2, font=\normalsize\sffamily\bfseries] {\textbf{Parallel}};

  % Linear
  \node[tsk=atlantic, right=2.5cm of p3] (l1) {A};
  \node[tsk=atlantic, right=0.8cm of l1] (l2) {B};
  \node[tsk=atlantic, right=0.8cm of l2] (l3) {C};
  \draw[dataarrow=atlantic, line width=1.2pt] (l1) -- (l2);
  \draw[dataarrow=atlantic, line width=1.2pt] (l2) -- (l3);
  \node[labelnode, above=0.15cm of l2, font=\normalsize\sffamily\bfseries] {\textbf{Linear}};

  % Tree
  \node[tsk=congaree, below=1.2cm of $(p3)!0.5!(l1)$] (t1) {A};
  \node[tsk=congaree, right=1.0cm of t1] (t2) {B};
  \node[tsk=congaree, below=0.7cm of $(t1)!0.5!(t2)$] (t3) {C};
  \draw[dataarrow=congaree, line width=1.2pt] (t1) -- (t3);
  \draw[dataarrow=congaree, line width=1.2pt] (t2) -- (t3);
  \node[labelnode, above=0.15cm of $(t1)!0.5!(t2)$, font=\normalsize\sffamily\bfseries] {\textbf{Tree}};
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.6cm]
  \node[tierbox=garnet, minimum width=\textwidth-3cm, minimum height=1.1cm,
    font=\sffamily] (strat)
    {\textbf{Strategy Layer} --- goals, constraints, success criteria};
  \node[tierbox=atlantic, minimum width=\textwidth-3cm, minimum height=1.1cm,
    font=\sffamily, below=of strat] (plan)
    {\textbf{Planning Layer} --- task decomposition, dependency graph};
  \node[tierbox=congaree, minimum width=\textwidth-3cm, minimum height=1.1cm,
    font=\sffamily, below=of plan] (exec)
    {\textbf{Execution Layer} --- implementation, tool use, results};

  \draw[dataarrow=garnet, line width=1.2pt] (strat) -- (plan)
    node[midway, right, labelnode, font=\small\sffamily] {strategic plan};
  \draw[dataarrow=atlantic, line width=1.2pt] (plan) -- (exec)
    node[midway, right, labelnode, font=\small\sffamily] {task graph};

  % Token annotations
  \node[labelnode, left=0.2cm of strat, font=\small\sffamily] {15--20K};
  \node[labelnode, left=0.2cm of plan, font=\small\sffamily] {40--60K};
  \node[labelnode, left=0.2cm of exec, font=\small\sffamily] {$n \times$ 25--45K};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
  level 1/.style={sibling distance=5.0cm, level distance=1.5cm},
  level 2/.style={sibling distance=2.4cm, level distance=1.5cm},
  every node/.style={agentbox=bk70, minimum width=2.0cm,
    minimum height=0.7cm, font=\small\sffamily, inner sep=3pt},
  edge from parent/.style={draw=bk50, thick, -{Stealth[length=4pt]}}
]
  \node[agentbox=garnet, minimum width=2.8cm, minimum height=0.9cm] {Chief\\Architect}
    child { node[agentbox=atlantic, minimum width=2.4cm] {Backend\\Lead}
      child { node {Auth} }
      child { node {API} }
    }
    child { node[agentbox=atlantic, minimum width=2.4cm] {Frontend\\Lead}
      child { node {UI} }
      child { node {UX} }
    }
    child { node[agentbox=atlantic, minimum width=2.4cm] {DevOps\\Lead}
      child { node {Infra} }
      child { node {CI/CD} }
    };
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.5cm and 1.0cm]
  \node[agentbox=garnet, minimum width=4.0cm, minimum height=1.0cm] (leader) {\large Leader};
  \node[rectangle, draw=bk30, fill=bk10, thick,
    minimum width=8.0cm, minimum height=0.8cm,
    below=0.8cm of leader, font=\sffamily] (queue) {Task Queue};

  \node[workerbox, below left=1.0cm and 2.5cm of queue, minimum width=2.0cm]
    (w1) {W\textsubscript{1}};
  \node[workerbox, below left=1.0cm and 0.3cm of queue, minimum width=2.0cm]
    (w2) {W\textsubscript{2}};
  \node[workerbox, below right=1.0cm and 0.3cm of queue, minimum width=2.0cm]
    (w3) {W\textsubscript{3}};
  \node[workerbox, below right=1.0cm and 2.5cm of queue, minimum width=2.0cm]
    (w4) {W\textsubscript{4}};

  \draw[dataarrow=garnet, line width=1.2pt] (leader) -- (queue);
  \draw[dataarrow=atlantic, line width=1.2pt] (queue) -- (w1);
  \draw[dataarrow=atlantic, line width=1.2pt] (queue) -- (w2);
  \draw[dataarrow=atlantic, line width=1.2pt] (queue) -- (w3);
  \draw[dataarrow=atlantic, line width=1.2pt] (queue) -- (w4);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[agentbox=garnet, minimum width=5.0cm, minimum height=1.0cm] (orch)
    {\large Research Orchestrator};
  \node[workerbox, below left=1.2cm and 2.0cm of orch,
    minimum width=3.0cm] (a1) {Database\\Searcher};
  \node[workerbox, below=1.2cm of orch,
    minimum width=3.0cm] (a2) {Citation\\Analyzer};
  \node[workerbox, below right=1.2cm and 2.0cm of orch,
    minimum width=3.0cm] (a3) {Gap\\Identifier};

  \draw[dataarrow=garnet, line width=1.2pt] (orch) -- (a1);
  \draw[dataarrow=garnet, line width=1.2pt] (orch) -- (a2);
  \draw[dataarrow=garnet, line width=1.2pt] (orch) -- (a3);

  \node[agentbox=horseshoe, below=3.0cm of orch,
    minimum width=5.0cm, minimum height=1.0cm] (syn) {\large Synthesis Agent};
  \draw[dataarrow=atlantic, dashed, line width=1.2pt] (a1) -- (syn);
  \draw[dataarrow=atlantic, dashed, line width=1.2pt] (a2) -- (syn);
  \draw[dataarrow=atlantic, dashed, line width=1.2pt] (a3) -- (syn);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.5cm and 0.5cm]
  \node[agentbox=garnet, minimum width=4.5cm, minimum height=1.0cm] (master) {\large Analysis Master};

  % Data partitions
  \node[rectangle, draw=bk30, fill=bk10,
    minimum width=\textwidth-3cm, minimum height=0.7cm,
    below=0.7cm of master, font=\sffamily] (data)
    {Dataset: partitions $P_1, P_2, \ldots, P_n$};

  \node[workerbox, below left=0.9cm and 2.5cm of data, minimum width=2.0cm]
    (w1) {W\textsubscript{1}\\$P_1$};
  \node[workerbox, below left=0.9cm and 0.3cm of data, minimum width=2.0cm]
    (w2) {W\textsubscript{2}\\$P_2$};
  \node[workerbox, below right=0.9cm and 0.3cm of data, minimum width=2.0cm]
    (w3) {W\textsubscript{3}\\$P_3$};
  \node[workerbox, below right=0.9cm and 2.5cm of data, minimum width=2.0cm]
    (w4) {W\textsubscript{4}\\$P_4$};

  \draw[dataarrow=garnet, line width=1.2pt] (master) -- (data);
  \draw[dataarrow=atlantic, line width=1.2pt] (data) -- (w1);
  \draw[dataarrow=atlantic, line width=1.2pt] (data) -- (w2);
  \draw[dataarrow=atlantic, line width=1.2pt] (data) -- (w3);
  \draw[dataarrow=atlantic, line width=1.2pt] (data) -- (w4);

  \node[agentbox=horseshoe, below=2.8cm of data, minimum width=4.5cm,
    minimum height=1.0cm] (agg) {\large Aggregator};
  \draw[dataarrow=congaree, dashed, line width=1.2pt] (w1) -- (agg);
  \draw[dataarrow=congaree, dashed, line width=1.2pt] (w2) -- (agg);
  \draw[dataarrow=congaree, dashed, line width=1.2pt] (w3) -- (agg);
  \draw[dataarrow=congaree, dashed, line width=1.2pt] (w4) -- (agg);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=0.5cm,
  phase/.style={rectangle, draw=#1, fill=#1!10, thick,
    minimum width=\textwidth-4cm, minimum height=0.9cm,
    font=\sffamily, align=center},
  ...
  \draw[dataarrow=garnet, line width=1.2pt] (r) -- (i);
  \draw[dataarrow=atlantic, line width=1.2pt] (i) -- (a);
  \draw[dataarrow=congaree, line width=1.2pt] (a) -- (w);

  % Annotations
  \node[labelnode, right=0.15cm of r, font=\small\sffamily\itshape] {fan-out};
  \node[labelnode, right=0.15cm of i, font=\small\sffamily\itshape] {3-tier};
  \node[labelnode, right=0.15cm of a, font=\small\sffamily\itshape] {master-worker};
  \node[labelnode, right=0.15cm of w, font=\small\sffamily\itshape] {pipeline};
\end{tikzpicture}
