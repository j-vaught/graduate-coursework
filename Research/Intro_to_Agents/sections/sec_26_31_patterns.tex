...
The orchestrator's role is minimal: it assembles the chain, initiates the first stage, and monitors for failures.
...
The orchestrator establishes the chain and monitors each stage, but the data flows strictly left-to-right through the sequence.
...
\node[agentbox=garnet, minimum width=1.2cm] (orch) {Orchestrator};
...
% Orchestrator coordination (thin dashed)
...
\node[agentbox=garnet, below=3.5cm of orch, minimum width=1.2cm] (orch2) {Orchestrator};
\node[agentbox=garnet, below left=1.5cm and 2cm of orch2, minimum width=1.2cm] (planner) {Planner};

\node[workerbox, below=1.5cm of planner, yshift=0.8cm] (w1) {Worker 1};
\node[workerbox, below=0.3cm of w1] (w2) {Worker 2};
\node[workerbox, below=0.3cm of w2] (w3) {Worker 3};
...
\caption{%
  ...
  The orchestrator coordinates the chain but does not participate in data transformation.
  \textbf{Bottom:} The fan-out/fan-in pattern distributes independent subtasks to parallel workers.
  The planner decomposes the problem, workers execute in parallel, and the synthesizer merges results.
}
...
A planner examines the overall task, identifies subtasks that can proceed without coordination, and distributes them to a set of parallel workers.
Each worker executes its subtask independently---no inter-worker communication is required.
A synthesizer collects all worker outputs and combines them into a unified result.
...
Code reviews fan out file-level analyses to separate workers, then merge findings.
Testing frameworks fan out test suites to parallel executors, then aggregate pass/fail results.
The key enabling assumption is subtask independence: each worker must be able to complete its work without information from other workers.
...
Conversely, correct decomposition yields significant speedup: total latency becomes the maximum worker latency rather than the sum of all subtask latencies.
The pattern also provides natural fault tolerance: if a worker fails, the orchestrator can retry that subtask without affecting other workers.

Figure~\ref{fig:patterns_pipe_fan} (bottom) shows a fan-out/fan-in workflow.
The planner distributes three subtasks to three workers operating in parallel.
Each worker independently completes its assignment and returns results to the synthesizer, which merges them into a final output.
The orchestrator monitors both planning and synthesis but does not participate in subtask execution.

\section{The Master-Worker Pool Pattern}\label{sec:master_worker}

The master-worker pool pattern manages large-scale batch processing through a centralized task queue and a pool of interchangeable workers.
A master agent maintains the queue, adding tasks as they arrive or as prior results generate new work.
Workers pull tasks from the queue, execute them, return results to the master, and immediately request the next task.
Workers are stateless and interchangeable---if one worker fails, another can retry the same task.
...
Analyzing thousands of log files, running extensive test suites, processing batches of data records, or generating multiple variations of a design all fit naturally into the master-worker model.
The pool size can scale dynamically: add workers to increase throughput, remove workers when load decreases.
The master can implement sophisticated scheduling policies: prioritize high-value tasks, load-balance across workers, or retry failed tasks with backoff.
...
For example, a web crawler's master enqueues initial seed URLs; as workers fetch pages, they report discovered links, which the master adds to the queue.
This dynamic task generation continues until no new work is discovered or a resource limit is reached.

Figure~\ref{fig:patterns_pool_feedback} (top) illustrates the master-worker pool.
The master enqueues tasks \texttt{T1}, \texttt{T2}, \texttt{T3}, \texttt{T4}, and so on into a central queue.
Four workers dequeue tasks, execute them, and return results to the master.
...
% TOP HALF: Master-Worker Pool Pattern
...
\node[workerbox, below left=1.5cm and 1cm of queue] (w1) {W1};
\node[workerbox, below=1.5cm of queue, xshift=-0.5cm] (w2) {W2};
\node[workerbox, below=1.5cm of queue, xshift=0.5cm] (w3) {W3};
\node[workerbox, below right=1.5cm and 1cm of queue] (w4) {W4};
...
% Label
\node[left=1.5cm of master, font=\footnotesize\bfseries] {Master-Worker Pool:};
...
\end{tikzpicture}
\caption{%
  \textbf{Top:} The master-worker pool pattern uses a centralized task queue and stateless workers.
  The master enqueues tasks, workers dequeue and execute them, and results flow back to the master.
...
A research pipeline might use fan-out for literature search, a sequential pipeline for data cleaning, a master-worker pool for running experiments, and feedback loops for manuscript writing.
The orchestrator at the top selects and coordinates the appropriate pattern for each phase based on the task structure.
...
The top-level orchestrator manages these transitions, ensuring that failures in one pattern do not silently propagate to dependent patterns.
...
The orchestrator can monitor pattern performance and switch strategies dynamically.
A fan-out phase that discovers subtask dependencies might transition to a pipeline.
A pipeline stage that encounters a large batch might spawn a master-worker pool for that stage alone.
...
The top-level research orchestrator coordinates four phases, each using a different pattern.
...
The experiment phase uses a master-worker pool to run many trials in parallel.
...
\node[workerbox, below left=0.6cm and 0.3cm of lit_plan, minimum width=0.6cm, minimum height=0.4cm] (w1) {W1};
\node[workerbox, below=0.6cm of lit_plan, minimum width=0.6cm, minimum height=0.4cm] (w2) {W2};
\node[workerbox, below right=0.6cm and 0.3cm of lit_plan, minimum width=0.6cm, minimum height=0.4cm] (w3) {W3};
...
% Branch 3: Experiments (Master-Worker)
\node[below right=1.5cm and 0.5cm of top, font=\footnotesize] (exp_label) {\textbf{Experiments (Master-Worker)}};
\node[agentbox=garnet, below=0.3cm of exp_label, minimum width=0.8cm, minimum height=0.5cm] (exp_master) {Master};
\node[workerbox, below left=0.6cm and 0.2cm of exp_master, minimum width=0.6cm, minimum height=0.4cm] (e1) {W1};
\node[workerbox, below right=0.6cm and 0.2cm of exp_master, minimum width=0.6cm, minimum height=0.4cm] (e2) {W2};
...
\end{tikzpicture}
\caption{%
  A composite research workflow combines four patterns under a single orchestrator.
  ...
  The experiment phase employs a master-worker pool for high-throughput execution.
...
Large-scale batch processing with uniform work items benefits from a master-worker pool, which maximizes throughput through dynamic load balancing.
...
Scale reflects the typical number of subtasks: pipelines handle a few stages, fan-out manages moderate parallelism, and master-worker pools scale to hundreds of tasks.
Quality emphasis distinguishes patterns optimized for throughput (master-worker) from those focused on refinement (feedback loop).
...
Fan-out and master-worker patterns add coordination overhead.
...
Master-worker pools batch tasks, trading some latency for throughput.
...
Master-worker pools optimize batch processing.
...
\toprule
\textbf{Criterion} & \textbf{Pipeline} & \textbf{Fan-Out/Fan-In} & \textbf{Master-Worker} & \textbf{Feedback Loop} \\
\midrule
...
The pipeline and fan-out/fan-in patterns are flat---the orchestrator coordinates but does not command.
Hierarchical architectures establish explicit control relationships.
Master-worker pools and feedback loops are hierarchical---the master or planner directs subordinate agents.
