...
The serial topology requires no dedicated figure; it is simply a chain of workers where each stage's output becomes the next stage's input. The orchestrator coordinates handoff timing but does not participate in data transformation. Workers operate independently once launched.
...
\begin{tikzpicture}[node distance=0.8cm and 0.8cm]
  % Orchestrator - wide bar
  \node[agentbox=garnet, minimum width=11cm, minimum height=0.6cm] (orch1) {Orchestrator};

  % Workers row - explicitly centered under orchestrator
  \node[workerbox] at ([yshift=-1.2cm, xshift=-4.5cm]orch1.south) (w11) {W1};
  \node[workerbox] at ([yshift=-1.2cm, xshift=-1.5cm]orch1.south) (w12) {W2};
  \node[workerbox] at ([yshift=-1.2cm, xshift=1.5cm]orch1.south)  (w13) {W3};
  \node[workerbox] at ([yshift=-1.2cm, xshift=4.5cm]orch1.south)  (w14) {W4};
  ...
  % Arrows from workers to barrier
  \foreach \w in {w11,w12,w13,w14} {
...
\end{tikzpicture}
\caption{Full barrier: the orchestrator fans out tasks to all workers and waits for every worker to complete before proceeding to synthesis. The slowest worker determines total execution time.}
...
\centering
\begin{tikzpicture}[node distance=0.8cm and 0.8cm]
  \node[agentbox=garnet, minimum width=11cm, minimum height=0.6cm] (orch2) {Orchestrator};

  \node[workerbox] at ([yshift=-1.2cm, xshift=-4.5cm]orch2.south) (w21) {W1};
  \node[workerbox] at ([yshift=-1.2cm, xshift=-1.5cm]orch2.south) (w22) {W2};
  \node[workerbox] at ([yshift=-1.2cm, xshift=1.5cm]orch2.south)  (w23) {W3};
  \node[workerbox, fill=bk30!30] at ([yshift=-1.2cm, xshift=4.5cm]orch2.south)  (w24) {W4};
...
\end{tikzpicture}
\caption{Partial barrier (k-of-n): the orchestrator proceeds when k workers finish, here 3 of 4. The grayed worker (W4) may still be running but its result is not required (dashed arrow). This reduces sensitivity to slow workers.}
...
\centering
\begin{tikzpicture}[node distance=0.8cm and 0.8cm]
  \node[agentbox=garnet, minimum width=11cm, minimum height=0.6cm] (orch3) {Orchestrator};

  \node[workerbox] at ([yshift=-1.2cm, xshift=-4.5cm]orch3.south) (w31) {W1};
  \node[workerbox] at ([yshift=-1.2cm, xshift=-1.5cm]orch3.south) (w32) {W2};
  \node[workerbox, fill=bk30!30] at ([yshift=-1.2cm, xshift=1.5cm]orch3.south)  (w33) {W3};
  \node[workerbox, fill=bk30!30] at ([yshift=-1.2cm, xshift=4.5cm]orch3.south)  (w34) {W4};
...
\end{tikzpicture}
\caption{Timeout barrier: the orchestrator waits up to a fixed deadline (here 30 seconds), then proceeds with whatever results are available. Grayed workers with dashed arrows have not yet completed. This prioritizes responsiveness over completeness.}
\label{fig:barrier_timeout}
\end{figure*}

Figures~\ref{fig:barrier_full}, \ref{fig:barrier_partial}, and~\ref{fig:barrier_timeout} illustrate the three barrier types. Grayed workers indicate those whose results are not required (or not yet available) for the orchestrator to proceed. The choice of barrier depends on the task's tolerance for incomplete results and its latency requirements.
